...
u8 LoadBgVram(u8 bg, const void *src, u16 size, u16 destOffset, u8 mode)
{
    ...
    if (!IsInvalidBg(bg) && sGpuBgConfigs.configs[bg].visible)
    {
        ...
        {
        case 0x1:
            offset = sGpuBgConfigs.configs[bg].charBaseIndex * 0x4000;
            break;
        case 0x2:
            offset = sGpuBgConfigs.configs[bg].mapBaseIndex * 0x800;
            break;
        default:
            cursor = -1;
            goto end;
        ...
        cursor = RequestDma3Copy(src, (void*)(offset + 0x6000000), size, 0);
...
static void ShowBgInternal(u8 bg)
{
    ...
    if (!IsInvalidBg(bg) && sGpuBgConfigs.configs[bg].visible)
    {
        ...
        sGpuBgConfigs.bgVisibilityAndMode &= (0x0F00 | 0x7);
    }
}
...
{
    SetGpuReg(0x0, (GetGpuReg(0x0) & ~(0x0F00 | 0x7)) | sGpuBgConfigs.bgVisibilityAndMode);
}

void SetTextModeAndHideBgs(void)
{
    SetGpuReg(0x0, GetGpuReg(0x0) & ~(0x0F00 | 0x7));
}
...
static void SetBgAffineInternal(u8 bg, s32 srcCenterX, s32 srcCenterY, s16 dispCenterX, s16 dispCenterY, s16 scaleX, s16 scaleY, u16 rotationAngle)
{
    ...
    SetGpuReg(0x20, dest.pa);
    SetGpuReg(0x28, (s16)(dest.dx));
    SetGpuReg(0x2a, (s16)(dest.dx >> 16));
    SetGpuReg(0x2c, (s16)(dest.dy));
    SetGpuReg(0x2e, (s16)(dest.dy >> 16));
}
...
void InitBgsFromTemplates(u8 bgMode, const struct BgTemplate *templates, u8 numTemplates)
{
    ...
    for (i = 0; i < numTemplates; i++)
    {
        ...
        if (bg < 4)
        {
            ...
            sGpuBgConfigs2[bg].tilemap = ((void *)0);
...
u16 LoadBgTiles(u8 bg, const void* src, u16 size, u16 destOffset)
{
    ...
    cursor = LoadBgVram(bg, src, size, tileOffset, 0x0001);
...
u16 LoadBgTilemap(u8 bg, const void *src, u16 size, u16 destOffset)
{
    u8 cursor = LoadBgVram(bg, src, size, destOffset * 2, 0x0002);
...
u16 Unused_LoadBgPalette(u8 bg, const void *src, u16 size, u16 destOffset)
{
    ...
    if (!IsInvalidBg32(bg))
    {
        ...
        cursor = RequestDma3Copy(src, (void*)(paletteOffset + 0x5000000), size, 0);
...
s32 ChangeBgY_ScreenOff(u8 bg, u32 value, u8 op)
{
    ...
    case 0:
        temp1 = sGpuBgConfigs2[0].bg_y >> 0x8;
        SetGpuReg_ForcedBlank(0x12, temp1);
        break;
    case 1:
        temp1 = sGpuBgConfigs2[1].bg_y >> 0x8;
        SetGpuReg_ForcedBlank(0x16, temp1);
        break;
    case 2:
        if (mode == 0)
        {
            temp1 = sGpuBgConfigs2[2].bg_y >> 0x8;
            SetGpuReg_ForcedBlank(0x1a, temp1);
        ...
        else
        {
            temp1 = sGpuBgConfigs2[2].bg_y >> 0x10;
            temp2 = sGpuBgConfigs2[2].bg_y & 0xFFFF;
            SetGpuReg_ForcedBlank(0x2e, temp1);
            SetGpuReg_ForcedBlank(0x2c, temp2);
        }
        break;
    case 3:
        if (mode == 0)
        {
            temp1 = sGpuBgConfigs2[3].bg_y >> 0x8;
            SetGpuReg_ForcedBlank(0x1e, temp1);
        }
        else if (mode == 2)
        {
            temp1 = sGpuBgConfigs2[3].bg_y >> 0x10;
            temp2 = sGpuBgConfigs2[3].bg_y & 0xFFFF;
            SetGpuReg_ForcedBlank(0x3e, temp1);
            SetGpuReg_ForcedBlank(0x3c, temp2);
        }
        break;
    }
...
void CopyToBgTilemapBuffer(u8 bg, const void *src, u16 mode, u16 destOffset)
{
    ...
    {
        if (mode != 0)
            CpuSet(src, (void *)(sGpuBgConfigs2[bg].tilemap + (destOffset * 2)), 0x00000000 | ((mode)/(16/8) & 0x1FFFFF));
        else
...
bool32 IsTileMapOutsideWram(u8 bg)
{
    if (sGpuBgConfigs2[bg].tilemap > (void*)(0x03000000 + 0x8000))
        return 1;
    else if (sGpuBgConfigs2[bg].tilemap == ((void *)0))
        return 1;
    else
        return 0;
}
