# 0 "src/contest_link.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/contest_link.c"
...
# 6 "src/contest_link.c" 2
# 1 "include/pokemon.h" 1
# 7 "src/contest_link.c" 2
...
static void Task_LinkContest_InitFlags(u8 taskId)
{
    ...
    for (i = 0; i < gNumLinkContestPlayers && (u32)(gLinkPlayers[i].version & 0xFF) - 1 > 2 - 1; i++)
        ;
...
void Task_LinkContest_CommunicateRngRS(u8 taskId)
{
    ...
    case 0:
        if (GetMultiplayerId() == 0)
        {
            if (IsLinkTaskFinished() && LinkContest_SendBlock(&gRngValue, sizeof(gRngValue)) == 1)
                gTasks[taskId].data[0]++;
        }
        else
        {
...
void Task_LinkContest_CommunicateMonIdxs(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            if (LinkContest_SendBlock(&gContestPlayerMonIndex, sizeof(gContestPlayerMonIndex)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
...
void Task_LinkContest_CommunicateMoveSelections(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            ...
            if (LinkContest_SendBlock(&(gContestResources->status)[gContestPlayerMonIndex].currMove, sizeof((gContestResources->status)[gContestPlayerMonIndex].currMove)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
        if (LinkContest_GetBlockReceivedFromAllPlayers())
        {
            ...
            for (i = 0; i < gNumLinkContestPlayers; i++)
                (gContestResources->status)[i].currMove = gBlockRecvBuffer[i][0];
...
void Task_LinkContest_CommunicateAppealsState(u8 taskId)
{
    ...
    case 0:
        if (IsLinkTaskFinished())
        {
            if (LinkContest_SendBlock((gContestResources->status), 4 * sizeof(struct ContestantStatus)) == 1)
                gTasks[taskId].data[0]++;
        }
        break;
    case 1:
        if (LinkContest_GetBlockReceivedFromAllPlayers())
        {
            memcpy((gContestResources->status), gBlockRecvBuffer[gContestLinkLeaderIndex], 4 * sizeof(struct ContestantStatus));
            gTasks[taskId].data[0]++;
        }
        break;
    case 2:
