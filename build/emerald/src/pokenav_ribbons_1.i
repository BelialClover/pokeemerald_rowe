# 0 "src/pokenav_ribbons_1.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokenav_ribbons_1.c"
...
# 9 "src/pokenav_ribbons_1.c" 2
# 1 "include/constants/songs.h" 1
# 10 "src/pokenav_ribbons_1.c" 2
...
{
    [RIBBONS_MON_LIST_FUNC_NONE] = ((void *)0),
...
static u32 BuildPartyMonRibbonList(s32 state)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (!GetMonData(pokemon, 5))
            return 1;
        if (!GetMonData(pokemon, 6) && !GetMonData(pokemon, 4))
        {
            u32 ribbonCount = GetMonData(pokemon, 82);
            if (ribbonCount != 0)
            {
                item.monId = i;
...
static u32 BuildBoxMonRibbonList(s32 state)
{
    ...
    while (boxId < 14)
    {
        while (monId < (6 * 5))
        {
            ...
            {
                u32 ribbonCount = GetBoxMonDataAt(boxId, monId, 82);
                if (ribbonCount != 0)
                {
                    item.boxId = boxId;
                    item.monId = monId;
...
static u32 LoopedTask_OpenRibbonsMonList(s32 state)
{
    ...
    {
    case 0:
        InitBgTemplates(sMonRibbonListBgTemplates, (sizeof(sMonRibbonListBgTemplates)/sizeof(*(sMonRibbonListBgTemplates))));
...
static void sub_81D02B0(s32 windowId, s32 val1, s32 val2)
{
    ...
    *ptr++ = 0xBA;
    ...
    AddTextPrinterParameterized(windowId, 1, strbuf, x, 1, 0xFF, ((void *)0));
}
