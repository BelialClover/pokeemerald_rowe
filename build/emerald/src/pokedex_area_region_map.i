# 0 "src/pokedex_area_region_map.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokedex_area_region_map.c"
...
# 2 "src/pokedex_area_region_map.c" 2
# 1 "include/main.h" 1
...
# 3 "src/pokedex_area_region_map.c" 2
# 1 "include/menu.h" 1
...
# 4 "src/pokedex_area_region_map.c" 2
# 1 "gflib/bg.h" 1
...
# 5 "src/pokedex_area_region_map.c" 2
# 1 "gflib/malloc.h" 1
# 15 "gflib/malloc.h"
extern u8 gHeap[];
...
# 6 "src/pokedex_area_region_map.c" 2
...
# 7 "src/pokedex_area_region_map.c" 2
...
# 8 "src/pokedex_area_region_map.c" 2

static __attribute__((section("ewram_data"))) u8 *sPokedexAreaMapBgNum = ((void *)0);
...
void LoadPokedexAreaMapGfx(const struct PokedexAreaMapTemplate *template)
{
    ...
    if (mode == 0)
    {
        ...
        sub_8199D3C(DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMap_Tilemap, 0, 0, 1), template->offset, 32, 32, 0);
    }
    else
    {
        ...
        sub_8199D3C(DecompressAndCopyTileDataToVram(template->bg, sPokedexAreaMapAffine_Tilemap, 0, 0, 1), template->offset, 64, 64, 1);
    }

    ChangeBgX(template->bg, 0, 0);
    ChangeBgY(template->bg, 0, 0);
    SetBgAttribute(template->bg, BG_ATTR_PALETTEMODE, 1);
    CpuSet(sPokedexAreaMap_Pal, &gPlttBufferUnfaded[0x70], 0x04000000 | ((0x60)/(32/8) & 0x1FFFFF));
...
void FreePokedexAreaMapBgNum(void)
{
    if (sPokedexAreaMapBgNum != ((void *)0))
        { Free(sPokedexAreaMapBgNum); sPokedexAreaMapBgNum = ((void *)0); };
}
