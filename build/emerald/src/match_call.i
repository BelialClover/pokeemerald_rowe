...
{
    u32 i, count;
    for (i = 0, count = 0; i < REMATCH_WALLY_3; i++)
    {
        if (FlagGet(0x15C + i))
            count++;
    }
...
static u8 GetPokedexRatingLevel(u16 numSeen)
{
    ...
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(386), FLAG_GET_CAUGHT))
        numSeen--;
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(385), FLAG_GET_CAUGHT))
        numSeen--;
