...
# 29 "src/naming_screen.c" 2
# 1 "include/constants/rgb.h" 1
# 30 "src/naming_screen.c" 2
...
__attribute__((section("ewram_data"))) static struct NamingScreenData *sNamingScreen = ((void *)0);
extern u16 gKeyRepeatStartDelay;
...
static const u8 sKeyboardChars[KBPAGE_COUNT * 4 * 8] = __(
    "abcdef ."
    "ghijkl ,"
    "mnopqrs "
    "tuvwxyz "
...
static void SpriteCB_InputArrow(struct Sprite *sprite)
{
    ...
    {
        sprite->data[0] = 8;
        sprite->data[1] = (sprite->data[1] + 1) & ((size_t)(sizeof(x) / sizeof((x)[0])) - 1);
    }
    sprite->pos2.x = x[sprite->data[1]];
}
# 1086 "src/naming_screen.c"
static void SpriteCB_Underscore(struct Sprite *sprite)
{
    ...
    {
        sprite->pos2.y = y[sprite->data[1]];
        sprite->data[2]++;
        if (sprite->data[2] > 8)
        {
            sprite->data[1] = (sprite->data[1] + 1) & ((size_t)(sizeof(y) / sizeof((y)[0])) - 1);
            sprite->data[2] = 0;
        }
    }
}
...
static void SetCursorPos(s16 x, s16 y)
{
    ...
    if (x < sPageColumnCounts[CurrentPageToKeyboardId()])
        cursorSprite->pos1.x = sPageColumnXPos[x + CurrentPageToKeyboardId() * 8] + 38;
    else
...
static void NamingScreen_CreateWaldaDadIcon(void)
{
    ...
    spriteId = AddPseudoObjectEvent(19, SpriteCallbackDummy, 56, 37, 0);
    gSprites[spriteId].oam.priority = 3;
    StartSpriteAnim(&gSprites[spriteId], 4);
}
...
{
    return sKeyboardChars[x + y * 8 + CurrentPageToKeyboardId() * 8 * 4];
}
...
static void DrawTextEntry(void)
{
    ...
    for (i = 0; i < maxChars; i++)
    {
        ...
        AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY], 1, temp, i * 8 + x + extraWidth, 1, 0xFF, ((void *)0));
    }
...
static const struct SubspriteTable sSubspriteTable_PageSwapFrame[] =
{
    {(size_t)(sizeof(sSubsprites_PageSwapFrame) / sizeof((sSubsprites_PageSwapFrame)[0])), sSubsprites_PageSwapFrame}
};

static const struct SubspriteTable sSubspriteTable_PageSwapText[] =
{
    {(size_t)(sizeof(sSubsprites_PageSwapText) / sizeof((sSubsprites_PageSwapText)[0])), sSubsprites_PageSwapText},
    {(size_t)(sizeof(sSubsprites_PageSwapText) / sizeof((sSubsprites_PageSwapText)[0])), sSubsprites_PageSwapText},
    {(size_t)(sizeof(sSubsprites_PageSwapText) / sizeof((sSubsprites_PageSwapText)[0])), sSubsprites_PageSwapText}
};
...
static const struct SubspriteTable sSubspriteTable_Button[] =
{
    {(size_t)(sizeof(sSubsprites_Button) / sizeof((sSubsprites_Button)[0])), sSubsprites_Button}
};

static const struct SubspriteTable sSubspriteTable_PCIcon[] =
{
    {(size_t)(sizeof(sSubsprites_PCIcon) / sizeof((sSubsprites_PCIcon)[0])), sSubsprites_PCIcon}
};
