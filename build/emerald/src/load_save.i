...
{
            struct ItemSlot items[80];
            struct ItemSlot keyItems[50];
            struct ItemSlot pokeBalls[31];
            struct ItemSlot TMsHMs[208];
            struct ItemSlot berries[70];
            struct MailStruct mail[16];
            struct ItemSlot medicine[40];
            struct ItemSlot battleItems[100];
            struct ItemSlot powerUp[70];
            struct ItemSlot typeItems[80];
            struct ItemSlot megaStones[47];
};


__attribute__((section("ewram_data"))) struct SaveBlock2 gSaveblock2 = {0};
__attribute__((section("ewram_data"))) u8 gSaveblock2_DMA[128] = {0};

__attribute__((section("ewram_data"))) struct SaveBlock1 gSaveblock1 = {0};
__attribute__((section("ewram_data"))) u8 gSaveblock1_DMA[128] = {0};

__attribute__((section("ewram_data"))) struct PokemonStorage gPokemonStorage = {0};
__attribute__((section("ewram_data"))) u8 gSaveblock3_DMA[128] = {0};

__attribute__((section("ewram_data"))) struct LoadedSaveData gLoadedSaveData = {0};
__attribute__((section("ewram_data"))) u32 gLastEncryptionKey = 0;
...
void CheckForFlashMemory(void)
{
    if (!IdentifyFlash())
    {
        gFlashMemoryPresent = 1;
        InitFlashTimer();
    }
    else
    {
        gFlashMemoryPresent = 0;
    }
}

void ClearSav2(void)
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, &gSaveblock2, 0x00000000 | 0x01000000 | ((sizeof(struct SaveBlock2) + sizeof(gSaveblock2_DMA))/(16/8) & 0x1FFFFF)); };
}

void ClearSav1(void)
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, &gSaveblock1, 0x00000000 | 0x01000000 | ((sizeof(struct SaveBlock1) + sizeof(gSaveblock1_DMA))/(16/8) & 0x1FFFFF)); };
}
...
void MoveSaveBlocks_ResetHeap(void)
{
    ...
    gMain.vblankCallback = ((void *)0);
    gMain.hblankCallback = ((void *)0);
...
void LoadPlayerBag(void)
{
    ...
    for (i = 0; i < 80; i++)
        gLoadedSaveData.typeItems[i] = gSaveBlock1Ptr->bagPocket_TypeItems[i];
...
void SavePlayerBag(void)
{
    ...
    for (i = 0; i < 80; i++)
        gSaveBlock1Ptr->bagPocket_TypeItems[i] = gLoadedSaveData.typeItems[i];
