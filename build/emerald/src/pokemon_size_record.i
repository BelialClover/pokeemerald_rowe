# 0 "src/pokemon_size_record.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_size_record.c"
...
# 5 "src/pokemon_size_record.c" 2
# 1 "include/pokemon.h" 1
# 6 "src/pokemon_size_record.c" 2
...
{
    u16 personality = GetMonData(pkmn, 0);
    u16 hpIV = GetMonData(pkmn, 39) & 0xF;
    u16 attackIV = GetMonData(pkmn, 40) & 0xF;
    u16 defenseIV = GetMonData(pkmn, 41) & 0xF;
    u16 speedIV = GetMonData(pkmn, 42) & 0xF;
    u16 spAtkIV = GetMonData(pkmn, 43) & 0xF;
    u16 spDefIV = GetMonData(pkmn, 44) & 0xF;
...
static u8 CompareMonSize(u16 species, u16 *sizeRecord)
{
    ...
    else
    {
        ...
        if (GetMonData(pkmn, 45) == 1 || GetMonData(pkmn, 11) != species)
        {
            return 1;
        }
        else
...
void GetSeedotSizeRecordInfo(void)
{
    u16 *sizeRecord = GetVarPointer(0x4047);

    GetMonSizeRecordInfo(273, sizeRecord);
}
...
void CompareSeedotSize(void)
{
    ...
    gSpecialVar_Result = CompareMonSize(273, sizeRecord);
}
...
void GetLotadSizeRecordInfo(void)
{
    u16 *sizeRecord = GetVarPointer(0x404F);

    GetMonSizeRecordInfo(270, sizeRecord);
}
...
void CompareLotadSize(void)
{
    ...
    gSpecialVar_Result = CompareMonSize(270, sizeRecord);
}
...
void GiveGiftRibbonToParty(u8 index, u8 ribbonId)
{
    ...
    if (index < 11 && ribbonId < 65)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            if (GetMonData(mon, 11) != 0 && GetMonData(mon, 6) == 0)
            {
