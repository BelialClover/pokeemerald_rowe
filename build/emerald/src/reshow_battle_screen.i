# 0 "src/reshow_battle_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/reshow_battle_screen.c"
...
# 2 "src/reshow_battle_screen.c" 2
...
# 3 "src/reshow_battle_screen.c" 2
...
# 4 "src/reshow_battle_screen.c" 2
...
# 5 "src/reshow_battle_screen.c" 2
# 1 "include/pokemon.h" 1
# 6 "src/reshow_battle_screen.c" 2
# 1 "include/main.h" 1
...
# 7 "src/reshow_battle_screen.c" 2
...
# 8 "src/reshow_battle_screen.c" 2
# 1 "gflib/text.h" 1
# 272 "gflib/text.h"
enum
{
...
# 9 "src/reshow_battle_screen.c" 2
...
# 10 "src/reshow_battle_screen.c" 2
# 1 "gflib/bg.h" 1
...
# 11 "src/reshow_battle_screen.c" 2
...
# 12 "src/reshow_battle_screen.c" 2
# 1 "include/link.h" 1
# 106 "include/link.h"
struct LinkStatus
{
...
# 13 "src/reshow_battle_screen.c" 2
...
# 15 "src/reshow_battle_screen.c" 2
...
# 16 "src/reshow_battle_screen.c" 2
...
# 17 "src/reshow_battle_screen.c" 2
# 1 "include/data.h" 1
...
# 18 "src/reshow_battle_screen.c" 2
...
static void CB2_ReshowBattleScreenAfterMenu(void)
{
    ...
    case 0:
        ...
        break;
    case 1:
        { vu32 tmp = (vu32)(0); CpuFastSet((void *)&tmp, (void*)(0x6000000), 0x01000000 | ((0x18000)/(32/8) & 0x1FFFFF)); };
        break;
    case 2:
    ...
    case 19:
        {
            ...
            opponentBattler = GetBattlerAtPosition(1);
            species = GetMonData(&gEnemyParty[gBattlerPartyIndexes[opponentBattler]], 11);
            SetBattlerShadowSpriteCallback(opponentBattler, species);

            if (IsDoubleBattle())
            {
                opponentBattler = GetBattlerAtPosition(3);
                species = GetMonData(&gEnemyParty[gBattlerPartyIndexes[opponentBattler]], 11);
                SetBattlerShadowSpriteCallback(opponentBattler, species);
            }
...
static void sub_80A95F4(void)
{
    ...
    regBgcnt1 = (struct BGCntrlBitfield *)(&(*(vu16 *)(0x4000000 + 0xa)));
    regBgcnt1->charBaseBlock = 0;

    regBgcnt2 = (struct BGCntrlBitfield *)(&(*(vu16 *)(0x4000000 + 0xc)));
    ...
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != 0)
        {
            if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
                ...
                BattleLoadSubstituteOrMonSpriteGfx(battler, 0);
        }
        else if (gBattleTypeFlags & (1 << 7) && battler == 0)
            DecompressTrainerBackPic(gSaveBlock2Ptr->playerGender, battler);
        else if (gBattleTypeFlags & (1 << 9) && battler == 0)
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != 0)
        {
            if (GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 57) == 0)
                return;
            if (gBattleScripting.monCaught)
                return;

            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 11), GetBattlerPosition(battler), GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 89));
            ...
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetFormSpeciesId(GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 11), GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 89));

            StartSpriteAnim(&gSprites[gBattlerSpriteIds[battler]], gBattleMonForms[battler]);
            if (gBattleSpritesDataPtr->battlerData[battler].transformSpecies == 351)
                gSprites[gBattlerSpriteIds[battler]].anims = gMonFrontAnimsPtrTable[351];
        }
        else if (gBattleTypeFlags & (1 << 7) && battler == 0)
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock2Ptr->playerGender, GetBattlerPosition(0));
        ...
        {
            if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 57) == 0)
                return;

            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 11), GetBattlerPosition(battler), GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 89));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, GetBattlerSpriteCoord(battler, 2), posY, GetBattlerSpriteSubpriority(battler));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = battler;
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetFormSpeciesId(GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 11), GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 89));
...
static void CreateHealthboxSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (gBattleTypeFlags & (1 << 7) && battler == 0)
        ...
        if (GetBattlerPosition(battler) == 3 || GetBattlerPosition(battler) == 2)
            DummyBattleInterfaceFunc(gHealthboxSpriteIds[battler], 1);
        else
            DummyBattleInterfaceFunc(gHealthboxSpriteIds[battler], 0);
