# 0 "src/berry_tag_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/berry_tag_screen.c"
...
# 7 "src/berry_tag_screen.c" 2
# 1 "include/item.h" 1
# 8 "src/berry_tag_screen.c" 2
# 1 "include/item_use.h" 1
# 9 "src/berry_tag_screen.c" 2
# 1 "include/main.h" 1
...
# 11 "src/berry_tag_screen.c" 2


# 1 "include/task.h" 1
# 14 "src/berry_tag_screen.c" 2
# 1 "include/menu_helpers.h" 1
# 15 "src/berry_tag_screen.c" 2
...
# 29 "src/berry_tag_screen.c" 2
# 1 "include/constants/berry.h" 1
# 30 "src/berry_tag_screen.c" 2
# 1 "include/constants/items.h" 1
# 31 "src/berry_tag_screen.c" 2
# 1 "include/constants/rgb.h" 1
# 32 "src/berry_tag_screen.c" 2
# 1 "include/constants/songs.h" 1
# 33 "src/berry_tag_screen.c" 2
...
struct BerryTagScreenStruct
{
    ...
    u16 berryId;
    u8 berrySpriteId;
    u8 flavorCircleIds[5];
    u16 gfxState;
};


static __attribute__((section("ewram_data"))) struct BerryTagScreenStruct *sBerryTag = ((void *)0);
...
static bool8 LoadBerryTagGfx(void)
{
    ...
    case 3:
        ...
        {
            for (i = 0; i < (size_t)(sizeof(sBerryTag->tilemapBuffers[1]) / sizeof((sBerryTag->tilemapBuffers[1])[0])); i++)
...
static void HandleInitWindows(void)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sWindowTemplates) / sizeof((sWindowTemplates)[0])) - 1; i++)
        PutWindowTilemap(i);
...
static void AddBerryTagTextToBg0(void)
{
    ...
    FillWindowPixelBuffer(WIN_BERRY_TAG, ((15) | ((15) << 4)));
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, gText_SizeSlash, 0, 1, 0xFF, ((void *)0));
    if (berry->size != 0)
    {
        u32 inches, fraction;
        ...
        AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, gStringVar4, 0x28, 1, 0, ((void *)0));
    }
    else
    {
        AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, gText_ThreeMarks, 0x28, 1, 0, ((void *)0));
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, gText_FirmSlash, 0, 0x11, 0xFF, ((void *)0));
    if (berry->firmness != 0)
        AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, sBerryFirmnessStrings[berry->firmness - 1], 0x28, 0x11, 0, ((void *)0));
    else
        AddTextPrinterParameterized(WIN_SIZE_FIRM, 1, gText_ThreeMarks, 0x28, 0x11, 0, ((void *)0));
}

static void PrintBerryDescription1(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, 1, berry->description1, 0, 1, 0, ((void *)0));
}

static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, 1, berry->description2, 0, 0x11, 0, ((void *)0));
}
...
static void SetFlavorCirclesVisiblity(void)
{
    ...
    if (berry->spicy)
        gSprites[sBerryTag->flavorCircleIds[0]].invisible = 0;
    else
        gSprites[sBerryTag->flavorCircleIds[0]].invisible = 1;

    if (berry->dry)
        gSprites[sBerryTag->flavorCircleIds[1]].invisible = 0;
    else
        gSprites[sBerryTag->flavorCircleIds[1]].invisible = 1;

    if (berry->sweet)
        gSprites[sBerryTag->flavorCircleIds[2]].invisible = 0;
    else
        gSprites[sBerryTag->flavorCircleIds[2]].invisible = 1;

    if (berry->bitter)
        gSprites[sBerryTag->flavorCircleIds[3]].invisible = 0;
    else
        gSprites[sBerryTag->flavorCircleIds[3]].invisible = 1;

    if (berry->sour)
        gSprites[sBerryTag->flavorCircleIds[4]].invisible = 0;
    else
        gSprites[sBerryTag->flavorCircleIds[4]].invisible = 1;
}
...
static void Task_HandleInput(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        u16 arrowKeys = ({(gMain.newAndRepeatedKeys) & (((0x0010 | 0x0020 | 0x0040 | 0x0080)));});
        if (arrowKeys == 0x0040)
            TryChangeDisplayedBerry(taskId, -1);
        else if (arrowKeys == 0x0080)
            TryChangeDisplayedBerry(taskId, 1);
...
static void TryChangeDisplayedBerry(u8 taskId, s8 toMove)
{
    ...
    if (newPocketPosition < 46 && BagGetItemIdByPocketPosition(7, newPocketPosition) != 0)
    {
        if (toMove < 0)
            data[1] = 2;
        else
...
static void HandleBagCursorPositionChange(s8 toMove)
{
    ...
    if (toMove > 0)
    {
        if (*cursorPos < 4 || BagGetItemIdByPocketPosition(7, *scrollPos + 8) == 0)
            *cursorPos += toMove;
        else
            *scrollPos += toMove;
    }
    ...
    sBerryTag->berryId = ItemIdToBerryType(BagGetItemIdByPocketPosition(7, *scrollPos + *cursorPos));
}
