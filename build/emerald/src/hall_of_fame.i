# 0 "src/hall_of_fame.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/hall_of_fame.c"
...
# 5 "src/hall_of_fame.c" 2

# 1 "include/pokemon.h" 1
# 7 "src/hall_of_fame.c" 2
# 1 "gflib/text.h" 1
# 272 "gflib/text.h"
enum
{
...
# 14 "src/hall_of_fame.c" 2
# 1 "include/constants/songs.h" 1
# 15 "src/hall_of_fame.c" 2
...
# 21 "src/hall_of_fame.c" 2
# 1 "include/constants/game_stat.h" 1
# 22 "src/hall_of_fame.c" 2
# 1 "include/util.h" 1
...
# 31 "src/hall_of_fame.c" 2
# 1 "include/menu.h" 1
# 32 "src/hall_of_fame.c" 2
...
# 35 "src/hall_of_fame.c" 2
# 1 "include/confetti_util.h" 1
...
# 36 "src/hall_of_fame.c" 2
# 1 "include/constants/rgb.h" 1
# 37 "src/hall_of_fame.c" 2
...
static __attribute__((section("ewram_data"))) u32 sHofFadePalettes = 0;
static __attribute__((section("ewram_data"))) struct HallofFameTeam *sHofMonPtr = ((void *)0);
static __attribute__((section("ewram_data"))) struct HofGfx *sHofGfxPtr = ((void *)0);
...
static const s16 sHallOfFame_MonHalfTeamPositions[6 / 2][4] =
{
    {120, 234, 120, 64},
    {326, 244, 56, 64},
    {-86, 244, 184, 64}
...
static void Task_Hof_InitMonData(u8 taskId)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        {
            sHofMonPtr->mon[i].species = GetMonData(&gPlayerParty[i], 65);
            sHofMonPtr->mon[i].tid = GetMonData(&gPlayerParty[i], 1);
            sHofMonPtr->mon[i].personality = GetMonData(&gPlayerParty[i], 0);
            sHofMonPtr->mon[i].lvl = GetMonData(&gPlayerParty[i], 56);
...
static void Task_Hof_InitTeamSaveData(u8 taskId)
{
    ...
    {
        if (Save_LoadGameData(SAVE_HALL_OF_FAME) != 1)
            memset(gDecompressionBuffer, 0, 0x2000);
    }

    for (i = 0; i < 30; i++, lastSavedTeam++)
    {
    ...
    if (i >= 30)
    {
        ...
        afterTeam++;
        for (i = 0; i < 30 - 1; i++, beforeTeam++, afterTeam++)
        {
...
static void Task_Hof_TrySaveData(u8 taskId)
{
    ...
    if (TrySavingData(SAVE_HALL_OF_FAME) == 0xFF && gDamagedSaveSectors != 0)
    {
        ...
        if (sHofGfxPtr != ((void *)0))
            { Free(sHofGfxPtr); sHofGfxPtr = ((void *)0); };
        if (sHofMonPtr != ((void *)0))
            { Free(sHofMonPtr); sHofMonPtr = ((void *)0); };
...
static void Task_Hof_DisplayMon(u8 taskId)
{
    ...
    if (currMon->species == 898 + 308 + 1)
        destY += 10;

    spriteId = CreatePicSprite2(currMon->species, currMon->tid, currMon->personality, 1, startX, startY, currMonId, 0xFFFF, 0);
    gSprites[spriteId].data[1] = destX;
    gSprites[spriteId].data[2] = destY;
    gSprites[spriteId].data[0] = 0;
    gSprites[spriteId].data[7] = currMon->species;
...
static void Task_Hof_TryDisplayAnotherMon(u8 taskId)
{
    ...
    else
    {
        ...
        if (gTasks[taskId].data[1] < 6 - 1 && currMon[1].species != 0)
        {
            gTasks[taskId].data[1]++;
            BeginNormalPaletteFade(sHofFadePalettes, 0, 12, 12, ((16) | ((29) << 5) | ((24) << 10)));
...
static void Task_Hof_PaletteFadeAndPrintWelcomeText(u8 taskId)
{
    ...
    BeginNormalPaletteFade(0xFFFF0000, 0, 0, 0, ((0) | ((0) << 5) | ((0) << 10)));
    for (i = 0; i < 6; i++)
    {
...
static void Task_Hof_WaitToDisplayPlayer(u8 taskId)
{
    ...
    {
        gTasks[taskId].data[3]++;
        SetGpuReg(0x52, gTasks[taskId].data[3] * 256);
    }
}
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].data[4] = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId_Debug(gSaveBlock2Ptr->playerGender, 1), 1, 120, 72, 6, 0xFFFF);
...
static void Task_Hof_WaitAndPrintPlayerInfo(u8 taskId)
{
    ...
    else
    {
        ...
        DrawDialogueFrame(0, 0);
        AddTextPrinterParameterized2(0, 1, gText_LeagueChamp, 0, ((void *)0), 2, 1, 3);
        CopyWindowToVram(0, 3);
...
static void Task_Hof_HandlePaletteOnExit(u8 taskId)
{
    CpuSet(gPlttBufferFaded, gPlttBufferUnfaded, 0x00000000 | ((0x400)/(16/8) & 0x1FFFFF));
    BeginNormalPaletteFade(0xFFFFFFFF, 8, 0, 0x10, ((0) | ((0) << 5) | ((0) << 10)));
...
static void Task_Hof_HandleExit(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            u8 spriteId = gTasks[taskId].data[i + 5];
            if (spriteId != 0xFF)
            {
...
static void Task_HofPC_CopySaveData(u8 taskId)
{
    ...
    else
    {
        ...
        CpuSet(gDecompressionBuffer, sHofMonPtr, 0x00000000 | ((0x2000)/(16/8) & 0x1FFFFF));
        savedTeams = sHofMonPtr;
        for (i = 0; i < 30; i++, savedTeams++)
        {
        ...
        if (i < 30)
            ...
            gTasks[taskId].data[0] = 30 - 1;

        gTasks[taskId].data[1] = GetGameStat(10);
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    BlendPalettes(0xFFFF0000, 0xC, ((16) | ((29) << 5) | ((24) << 10)));
...
static void Task_HofPC_PrintMonInfo(u8 taskId)
{
    ...
    for (i = 0; i < 6; i++)
    {
        u16 spriteId = gTasks[taskId].data[i + 5];
        if (spriteId != 0xFF)
            gSprites[spriteId].oam.priority = 1;
    }

    currMonID = gTasks[taskId].data[gTasks[taskId].data[2] + 5];
    gSprites[currMonID].oam.priority = 0;
    sHofFadePalettes = (0x10000 << gSprites[currMonID].oam.paletteNum) ^ 0xFFFF0000;
    BlendPalettesUnfaded(sHofFadePalettes, 0xC, ((16) | ((29) << 5) | ((24) << 10)));
...
static void Task_HofPC_HandleInput(u8 taskId)
{
    ...
    else if (({(gMain.newKeys) & (0x0040);}) && gTasks[taskId].data[2] != 0)
    {
        gTasks[taskId].data[2]--;
        gTasks[taskId].func = Task_HofPC_PrintMonInfo;
    }
    else if (({(gMain.newKeys) & (0x0080);}) && gTasks[taskId].data[2] < gTasks[taskId].data[4] - 1)
    {
        gTasks[taskId].data[2]++;
        gTasks[taskId].func = Task_HofPC_PrintMonInfo;
    }
...
static void Task_HofPC_PrintDataIsCorrupted(u8 taskId)
{
    sub_8198180(gText_AButtonExit, 8, 1);
    DrawDialogueFrame(0, 0);
    AddTextPrinterParameterized2(0, 1, gText_HOFCorrupted, 0, ((void *)0), 2, 1, 3);
    CopyWindowToVram(0, 3);
...
static void HallOfFame_PrintPlayerInfo(u8 unused1, u8 unused2)
{
    ...
    PutWindowTilemap(1);
    DrawStdFrameWithCustomTileAndPalette(1, 0, 0x21D, 0xD);
...
static void ClearVramOamPltt_LoadHofPal(void)
{
    ...
    oamOffset = 0x7000000;
    oamSize = 0x400;
    { vu32 tmp = (vu32)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(oamOffset); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((oamSize)/(32/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; };

    plttOffset = 0x5000000;
    plttSize = 0x400;
    { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(plttOffset); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((plttSize)/(16/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; };
...
{
    ResetBgsAndClearDma3BusyFlags(0);
    InitBgsFromTemplates(0, sHof_BgTemplates, (size_t)(sizeof(sHof_BgTemplates) / sizeof((sHof_BgTemplates)[0])));
...
static void SpriteCB_GetOnScreenAndAnimate(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        if (species == 898 + 308 + 1)
            DoMonFrontSpriteAnimation(sprite, species, 1, 3);
        else
            DoMonFrontSpriteAnimation(sprite, species, 0, 3);
    }
}
# 1374 "src/hall_of_fame.c"
static void SpriteCB_HofConfetti(struct Sprite* sprite)
{
...
static bool8 CreateHofConfettiSprite(void)
{
    ...
    StartSpriteAnim(sprite, Random() % (size_t)(sizeof(sAnims_Confetti) / sizeof((sAnims_Confetti)[0])));
    ...
    case 1:
        if (data[1] != 0 && data[1] % 3 == 0)
        {
            ...
            id = ConfettiUtil_AddNew(&sOamData_Confetti,
                              ...
                              Random() % 240,
                              -(Random() % 8),
                              Random() % (size_t)(sizeof(sAnims_Confetti) / sizeof((sAnims_Confetti)[0])),
                              id);
            if (id != 0xFF)
            {
