...
# 6 "src/battle_util2.c" 2
# 1 "include/pokemon.h" 1
# 7 "src/battle_util2.c" 2
...
# 10 "src/battle_util2.c" 2
# 1 "include/constants/abilities.h" 1
# 11 "src/battle_util2.c" 2
...
void AllocateBattleResources(void)
{
    ...
    {
        u16 currSecretBaseId = VarGet(0x4054);
...
void FreeBattleResources(void)
{
    ...
    gFieldStatuses = 0;
    if (gBattleResources != ((void *)0))
    {
        { Free(gBattleStruct); gBattleStruct = ((void *)0); };

        { Free(gBattleResources->secretBase); gBattleResources->secretBase = ((void *)0); };
        { Free(gBattleResources->flags); gBattleResources->flags = ((void *)0); };
        { Free(gBattleResources->battleScriptsStack); gBattleResources->battleScriptsStack = ((void *)0); };
        { Free(gBattleResources->battleCallbackStack); gBattleResources->battleCallbackStack = ((void *)0); };
        { Free(gBattleResources->beforeLvlUp); gBattleResources->beforeLvlUp = ((void *)0); };
        { Free(gBattleResources->ai); gBattleResources->ai = ((void *)0); };
        { Free(gBattleResources->battleHistory); gBattleResources->battleHistory = ((void *)0); };
        { Free(gBattleResources->AI_ScriptsStack); gBattleResources->AI_ScriptsStack = ((void *)0); };
        { Free(gBattleResources); gBattleResources = ((void *)0); };

        { Free(gLinkBattleSendBuffer); gLinkBattleSendBuffer = ((void *)0); };
        { Free(gLinkBattleRecvBuffer); gLinkBattleRecvBuffer = ((void *)0); };

        { Free(gUnknown_0202305C); gUnknown_0202305C = ((void *)0); };
        { Free(gUnknown_02023060); gUnknown_02023060 = ((void *)0); };
    }
}
...
void AdjustFriendshipOnBattleFaint(u8 battlerId)
{
    ...
    if (gBattleTypeFlags & (1 << 0))
    {
        ...
        opposingBattlerId = GetBattlerAtPosition(1);
        opposingBattlerId2 = GetBattlerAtPosition(3);
...
u32 sub_805725C(u8 battlerId)
{
        ...
        case 0:
            if (gBattleMons[battlerId].status1 & (1 << 0 | 1 << 1 | 1 << 2))
            {
                ...
                else
                {
                    ...
                    if (gBattleMons[battlerId].ability == 48)
                        toSub = 2;
                    else
                        toSub = 1;

                    if ((gBattleMons[battlerId].status1 & (1 << 0 | 1 << 1 | 1 << 2)) < toSub)
                        gBattleMons[battlerId].status1 &= ~((1 << 0 | 1 << 1 | 1 << 2));
                    else
    ...
    } while (gBattleCommunication[0x0] != 2 && effect == 0);
