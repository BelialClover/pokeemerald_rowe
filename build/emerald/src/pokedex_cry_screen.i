# 0 "src/pokedex_cry_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokedex_cry_screen.c"
...
# 11 "src/pokedex_cry_screen.c" 2
# 30 "src/pokedex_cry_screen.c"
...
static __attribute__((section("ewram_data"))) struct PokedexCryScreen *sDexCryScreen = ((void *)0);
static __attribute__((section("ewram_data"))) u8 *sCryWaveformWindowTiledata = ((void *)0);
static __attribute__((section("ewram_data"))) struct PokedexCryMeterNeedle *sCryMeterNeedle = ((void *)0);
...
static const u16 sCryMeter_Tilemap[] = INCBIN_U16("graphics/pokedex/cry_meter_map.bin");
...
bool8 LoadCryWaveformWindow(struct CryScreenWindow *window, u8 windowId)
{
    ...
    case 0:
        ...
        ShiftWaveformOver(windowId, -8 * window->xPos, 1);
        for (i = 0; i < 224; i++)
            CopyToWindowPixelBuffer(windowId, sCryScreenBg_Gfx, 32, i);
...
{
    if (gMPlayInfo_BGM.status & 0x80000000 && !sDexCryScreen->cryOverrideCountdown)
    {
        if (!sDexCryScreen->cryRepeatDelay)
        {
...
static void BufferCryWaveformSegment(void)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sDexCryScreen->cryWaveformBuffer) / sizeof((sDexCryScreen->cryWaveformBuffer)[0])); i++)
...
static void AdvancePlayhead(u8 windowId)
{
    ...
    ShiftWaveformOver(windowId, sDexCryScreen->playhead, 0);
    sDexCryScreen->playhead += 2;
    offset = (sDexCryScreen->playhead / 8 + sDexCryScreen->playStartPos + 1) % 32;
    for (i = 0; i < 7; i++)
        CopyToWindowPixelBuffer(windowId, sCryScreenBg_Gfx, 32, offset + (i * 32));
}
...
static void DrawWaveformSegment(u8 position, u8 amplitude)
{
    ...
    if (y > sDexCryScreen->waveformPreviousY)
    {
        ...
        do
        {
            offset = sWaveformOffsets[(position & ((1 << 3) - 1))][y] + (position >> 3) * 32;
...
void FreeCryScreen(void)
{
    ...
    { Free(sDexCryScreen); sDexCryScreen = ((void *)0); };
    { Free(sCryMeterNeedle); sCryMeterNeedle = ((void *)0); };
}
...
static void SpriteCB_CryMeterNeedle(struct Sprite *sprite)
{
    ...
    gSprites[sCryMeterNeedle->spriteId].oam.affineMode = 1;
...
{
    u16 rotation = (32 - offset) & 0xFF;


    if (rotation > 32 && rotation < (u8)-32)
        rotation = (u8)-32;
