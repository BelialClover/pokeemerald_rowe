...
# 36 "src/scrcmd.c" 2
# 1 "include/rotating_tile_puzzle.h" 1
...
__attribute__((section("ewram_data"))) const u8 *gUnknown_020375C0 = ((void *)0);
static __attribute__((section("ewram_data"))) u32 gUnknown_020375C4 = 0;
static __attribute__((section("ewram_data"))) u16 sPauseCounter = 0;
static __attribute__((section("ewram_data"))) u16 sMovingNpcId = 0;
static __attribute__((section("ewram_data"))) u16 sMovingNpcMapBank = 0;
static __attribute__((section("ewram_data"))) u16 sMovingNpcMapId = 0;
...
bool8 ScrCmd_fadescreenswapbuffers(struct ScriptContext *ctx)
{
    ...
    switch (mode)
    {
        ...
        case 3:
        default:
            CpuSet(gPlttBufferUnfaded, gPaletteDecompressionBuffer, 0x04000000 | (((0x200 * 2))/(32/8) & 0x1FFFFF));
            FadeScreen(mode, 0);
            break;
        case 0:
        case 2:
            CpuSet(gPaletteDecompressionBuffer, gPlttBufferUnfaded, 0x04000000 | (((0x200 * 2))/(32/8) & 0x1FFFFF));
            FadeScreen(mode, 0);
            break;
    }
...
bool8 ScrCmd_createvobject(struct ScriptContext *ctx)
{
    ...
    u8 objectEventId = (*(ctx->scriptPtr++));
    u16 x = VarGet(ScriptReadHalfword(ctx));
...
bool8 ScrCmd_messageautoscroll(struct ScriptContext *ctx)
{
    ...
    gTextFlags.forceMidTextSpeed = 1;
...
bool8 ScrCmd_braillemessage(struct ScriptContext *ctx)
{
    ...
    FillWindowPixelBuffer(gBrailleWindowId, ((1) | ((1) << 4)));
...
bool8 ScrCmd_givemon(struct ScriptContext *ctx)
{
    ...
    u32 ability = ScriptReadWord(ctx);
...
bool8 ScrCmd_setwildbattle(struct ScriptContext *ctx)
{
    ...
    u8 level2 = (*(ctx->scriptPtr++));
    u16 item2 = ScriptReadHalfword(ctx);

    if(species2 == 0)
    {
        CreateScriptedWildMon(species, level, item);
        gIsScriptedWildDouble = 0;
    }
    else
    {
        CreateScriptedDoubleWildMon(species, level, item, species2, level2, item2);
        gIsScriptedWildDouble = 1;
    }
...
{
    u8 treeId = (*(ctx->scriptPtr++));
    u8 berry = (*(ctx->scriptPtr++));
    u8 growthStage = (*(ctx->scriptPtr++));

    if (berry == 0)
        PlantBerryTree(treeId, 0, growthStage, 0);
    else
        PlantBerryTree(treeId, berry, growthStage, 0);
    return 0;
}
...
bool8 ScrCmd_givecustommon(struct ScriptContext *ctx)
{
    ...
    u8 level = (*(ctx->scriptPtr++));
    u16 item = ScriptReadHalfword(ctx);
    u8 ball = (*(ctx->scriptPtr++));
    u8 nature = (*(ctx->scriptPtr++));
    u8 abilityNum = (*(ctx->scriptPtr++));
    u8 hpEv = (*(ctx->scriptPtr++));
    u8 atkEv = (*(ctx->scriptPtr++));
    u8 defEv = (*(ctx->scriptPtr++));
    u8 speedEv = (*(ctx->scriptPtr++));
    u8 spAtkEv = (*(ctx->scriptPtr++));
    u8 spDefEv = (*(ctx->scriptPtr++));
    u8 hpIv = (*(ctx->scriptPtr++));
    u8 atkIv = (*(ctx->scriptPtr++));
    u8 defIv = (*(ctx->scriptPtr++));
    u8 speedIv = (*(ctx->scriptPtr++));
    u8 spAtkIv = (*(ctx->scriptPtr++));
    u8 spDefIv = (*(ctx->scriptPtr++));
    ...
    bool8 isShiny = (*(ctx->scriptPtr++));

    u8 evs[6] = {hpEv, atkEv, defEv, speedEv, spAtkEv, spDefEv};
    u8 ivs[6] = {hpIv, atkIv, defIv, speedIv, spAtkIv, spDefIv};
...
bool8 ScrCmd_setmonobedient(struct ScriptContext *ctx)
{
    ...
    SetMonData(&gPlayerParty[partyIndex], 80, &obedient);
    return 0;
}
