...
# 14 "src/list_menu.c" 2
# 1 "include/constants/songs.h" 1
# 15 "src/list_menu.c" 2
...
__attribute__((section("ewram_data"))) struct ScrollArrowsTemplate gTempScrollArrowTemplate = {0};
    ...
    case 1:
        ...
        if (sMysteryGiftLinkMenu.state == 2)
        {
            ...
            else
            {
                ...
                {
                case 0:
                    ClearStdWindowAndFrame(sMysteryGiftLinkMenu.windowId, 0);
                    break;
                case 2:
                case 1:
                    ClearStdWindowAndFrame(sMysteryGiftLinkMenu.windowId, 0);
        ...
        break;
    case 2:
        DestroyListMenuTask(sMysteryGiftLinkMenu.listTaskId, ((void *)0), ((void *)0));
...
s32 ListMenu_ProcessInput(u8 listTaskId)
{
    ...
    {
        ListMenuChangeSelection(list, 1, 1, 0);
        return -1;
    }
    ...
    else
    {
        ...
        case 1:
            ...
            leftButton = gMain.newAndRepeatedKeys & 0x0020;
            rightButton = gMain.newAndRepeatedKeys & 0x0010;
            break;
        case 2:

            leftButton = gMain.newAndRepeatedKeys & 0x0200;
            rightButton = gMain.newAndRepeatedKeys & 0x0100;
            break;
        }

        if (leftButton)
        {
            ListMenuChangeSelection(list, 1, list->template.maxShowed, 0);
            return -1;
        }
        else if (rightButton)
        {
            ListMenuChangeSelection(list, 1, list->template.maxShowed, 1);
            return -1;
        }
        else
        {
...
void RedrawListMenu(u8 listTaskId)
{
    ...
    FillWindowPixelBuffer(list->template.windowId, ((list->template.fillValue) | ((list->template.fillValue) << 4)));
...
s32 ListMenuTestInput(struct ListMenuTemplate *template, u32 scrollOffset, u32 selectedRow, u16 keys, u16 *newScrollOffset, u16 *newSelectedRow)
{
    ...
    if (keys == 0x0040)
        ListMenuChangeSelection(&list, 0, 1, 0);
    if (keys == 0x0080)
        ListMenuChangeSelection(&list, 0, 1, 1);

    if (newScrollOffset != ((void *)0))
        *newScrollOffset = list.scrollOffset;
    if (newSelectedRow != ((void *)0))
...
static void ListMenuPrintEntries(struct ListMenu *list, u16 startIndex, u16 yOffset, u16 count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (list->template.itemPrintFunc != ((void *)0))
...
static void ListMenuScroll(struct ListMenu *list, u8 count, bool8 movingDown)
{
    ...
    else
    {
        ...
        if (!movingDown)
        {
            ...
            ScrollWindow(list->template.windowId, 1, count * yMultiplier, ((list->template.fillValue) | ((list->template.fillValue) << 4)));
        ...
        else
        {
            ...
            ScrollWindow(list->template.windowId, 0, count * yMultiplier, ((list->template.fillValue) | ((list->template.fillValue) << 4)));
...
static void ListMenuCallSelectionChangedCallback(struct ListMenu *list, u8 onInit)
{
    if (list->template.moveCursorFunc != ((void *)0))
...
static u8 AddScrollIndicatorArrowObject(u8 arrowDir, u8 x, u8 y, u16 tileTag, u16 palTag)
{
    ...
    gSprites[spriteId].data[1] = sScrollIndicatorTemplates[arrowDir].animNum;
    gSprites[spriteId].data[2] = sScrollIndicatorTemplates[arrowDir].bounceDir;
    gSprites[spriteId].data[3] = sScrollIndicatorTemplates[arrowDir].multiplier;
    gSprites[spriteId].data[4] = sScrollIndicatorTemplates[arrowDir].frequency;
    gSprites[spriteId].data[5] = 0;

    return spriteId;
}
# 1069 "src/list_menu.c"
...
static void Task_ScrollIndicatorArrowPair(u8 taskId)
{
    ...
    if (currItem == data->fullyUpThreshold && currItem != 0xFFFF)
        gSprites[data->topSpriteId].invisible = 1;
    else
        gSprites[data->topSpriteId].invisible = 0;

    if (currItem == data->fullyDownThreshold)
        gSprites[data->bottomSpriteId].invisible = 1;
    else
        gSprites[data->bottomSpriteId].invisible = 0;
}
...
void Task_ScrollIndicatorArrowPairOnMainMenu(u8 taskId)
{
    ...
    if (data[15])
    {
        gSprites[scrollData->topSpriteId].invisible = 0;
        gSprites[scrollData->bottomSpriteId].invisible = 1;
    }
    else
    {
        gSprites[scrollData->topSpriteId].invisible = 1;
        gSprites[scrollData->bottomSpriteId].invisible = 0;
    }
}
