# 0 "src/battle_anim_normal.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_anim_normal.c"
...
# 5 "src/battle_anim_normal.c" 2
# 1 "include/task.h" 1
# 6 "src/battle_anim_normal.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 7 "src/battle_anim_normal.c" 2
# 1 "include/constants/rgb.h" 1
# 8 "src/battle_anim_normal.c" 2
...
static void AnimTask_FlashAnimTagWithColor_Step2(u8 taskId)
{
    ...
    {
        selectedPalettes = 1 << (IndexOfSpritePaletteTag(gTasks[taskId].data[7]) + 16);
        BeginNormalPaletteFade(selectedPalettes, 0, 0, 0, ((0) | ((0) << 5) | ((0) << 10)));
        DestroyAnimVisualTask(taskId);
    }
}
...
void AnimTask_InvertScreenColor(u8 taskId)
{
    ...
    if (gBattleAnimArgs[0] & 0x8 && IsBattlerAlive(((gBattleAnimTarget) ^ 2)))
        selectedPalettes |= (0x10000 << ((gBattleAnimTarget) ^ 2));
 if (gBattleAnimArgs[0] & 0x10 && IsBattlerAlive(((gBattleAnimAttacker) ^ 2)))
        selectedPalettes |= (0x10000 << ((gBattleAnimAttacker) ^ 2));
...
void AnimHitSplatRandom(struct Sprite *sprite)
{
    ...
    if (!InitSpritePosToAnimBattler(gBattleAnimArgs[0], sprite, 0))
        return;
