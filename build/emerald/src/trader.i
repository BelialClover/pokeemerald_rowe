...
void CreateAvailableDecorationsMenu(u8 taskId)
{
    ...
    DrawStdFrameWithCustomTileAndPalette(data[3], 0, 0x214, 14);
    for (i = 0; i < 4; i++)
    {
        if (trader->decorations[i] > 120)
            AddTextPrinterParameterized(data[3], 1, gText_FiveMarks, 8, 16 * i + 1, 255, ((void *)0));
        else
            AddTextPrinterParameterized(data[3], 1, gDecorations[trader->decorations[i]].name, 8, 16 * i + 1, 255, ((void *)0));
    }
    AddTextPrinterParameterized(data[3], 1, gText_Exit, 8, 16 * i + 1, 255, ((void *)0));
...
void Task_BufferDecorSelectionAndCloseWindow(u8 taskId, u8 decorationId)
{
    ...
    ClearStdWindowAndFrameToTransparent(data[3], 0);
...
void ScrSpecial_TraderDoDecorationTrade(void)
{
    ...
    trader->language[gSpecialVar_0x8005] = (2);
