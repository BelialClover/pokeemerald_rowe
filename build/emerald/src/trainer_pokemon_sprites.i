# 0 "src/trainer_pokemon_sprites.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/trainer_pokemon_sprites.c"
...
# 2 "src/trainer_pokemon_sprites.c" 2
...
# 4 "src/trainer_pokemon_sprites.c" 2
# 1 "gflib/malloc.h" 1
# 15 "gflib/malloc.h"
extern u8 gHeap[];
...
# 5 "src/trainer_pokemon_sprites.c" 2
...
# 6 "src/trainer_pokemon_sprites.c" 2
...
# 7 "src/trainer_pokemon_sprites.c" 2
...
# 8 "src/trainer_pokemon_sprites.c" 2
# 1 "include/data.h" 1
...
# 9 "src/trainer_pokemon_sprites.c" 2
# 1 "include/pokemon.h" 1
# 10 "src/trainer_pokemon_sprites.c" 2
...
# 11 "src/trainer_pokemon_sprites.c" 2
...
static __attribute__((section("ewram_data"))) struct SpriteTemplate sCreatingSpriteTemplate = {};
static __attribute__((section("ewram_data"))) struct PicData sSpritePics[8] = {};
...
static bool16 DecompressPic(u16 formSpeciesId, u32 personality, bool8 isFrontPic, u8 *dest, bool8 isTrainer, bool8 ignoreDeoxys)
{
    if (!isTrainer)
    {
        if (isFrontPic)
        {
            LoadSpecialPokePic_DontHandleDeoxys(&gMonFrontPicTable[formSpeciesId], dest, formSpeciesId, personality, isFrontPic);
        }
        else
        {
            LoadSpecialPokePic_DontHandleDeoxys(&gMonBackPicTable[formSpeciesId], dest, formSpeciesId, personality, isFrontPic);
        }
    }
    else
    {
        if (isFrontPic)
            DecompressPicFromTable_DontHandleDeoxys(&gTrainerFrontPicTable[formSpeciesId], dest, formSpeciesId);
        else
            DecompressPicFromTable_DontHandleDeoxys(&gTrainerBackPicTable[formSpeciesId], dest, formSpeciesId);
    }
    return 0;
}
...
{
    return DecompressPic(species, personality, isFrontPic, dest, isTrainer, 0);
}

static void LoadPicPaletteByTagOrSlot(u16 formSpeciesId, u32 otId, u32 personality, u8 paletteSlot, u16 paletteTag, bool8 isTrainer)
{
    if (!isTrainer)
    {
        if (paletteTag == 0xFFFF)
        {
            ...
            LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(formSpeciesId, otId, personality), 0x100 + paletteSlot * 0x10, 0x20);
        }
        else
        {
            sCreatingSpriteTemplate.paletteTag = paletteTag;
            LoadCompressedSpritePalette(GetMonSpritePalStructFromOtIdPersonality(formSpeciesId, otId, personality));
        }
    }
    else
    {
        ...
        {
            sCreatingSpriteTemplate.paletteTag = 0xFFFF;
            LoadCompressedPalette(gTrainerFrontPicPaletteTable[formSpeciesId].data, 0x100 + paletteSlot * 0x10, 0x20);
        }
        else
        {
            sCreatingSpriteTemplate.paletteTag = paletteTag;
            LoadCompressedSpritePalette(&gTrainerFrontPicPaletteTable[formSpeciesId]);
        }
    }
}
...
{
    return CreatePicSprite(species, otId, personality, isFrontPic, x, y, paletteSlot, paletteTag, isTrainer, 0);
}
...
u16 CreatePicSprite2(u16 species, u32 otId, u32 personality, u8 flags, s16 x, s16 y, u8 paletteSlot, u16 paletteTag, u8 formId)
{
    ...
    if (DecompressPic(formSpeciesId, personality, flags, framePics, 0, 0))
    {
    ...
    sCreatingSpriteTemplate.tileTag = 0xFFFF;
    sCreatingSpriteTemplate.anims = gMonFrontAnimsPtrTable[formSpeciesId];
    sCreatingSpriteTemplate.images = images;
    if (flags2 == 0x01)
    {
    ...
    sCreatingSpriteTemplate.callback = DummyPicSpriteCallback;
    LoadPicPaletteByTagOrSlot(formSpeciesId, otId, personality, paletteSlot, paletteTag, 0);
    spriteId = CreateSprite(&sCreatingSpriteTemplate, x, y, 0);
    if (paletteTag == 0xFFFF)
    {
        gSprites[spriteId].oam.paletteNum = paletteSlot;
...
{
    if (DecompressPic(species, personality, isFrontPic, (u8 *)GetWindowAttribute(windowId, WINDOW_TILE_DATA), 0, 0))
    {
        return 0xFFFF;
    }
...
static u16 CreateTrainerCardSprite(u16 species, u32 otId, u32 personality, bool8 isFrontPic, u16 destX, u16 destY, u8 paletteSlot, u8 windowId, bool8 isTrainer)
{
    ...
    framePics = Alloc(4 * 0x800);
    if (framePics && !DecompressPic(species, personality, isFrontPic, framePics, isTrainer, 0))
    {
...
{
    return CreatePicSprite(species, otId, personality, isFrontPic, x, y, paletteSlot, paletteTag, 0, ignoreDeoxys);
}

u16 CreateMonPicSprite_HandleDeoxys(u16 species, u32 otId, u32 personality, bool8 isFrontPic, s16 x, s16 y, u8 paletteSlot, u16 paletteTag)
{
    return CreateMonPicSprite(species, otId, personality, isFrontPic, x, y, paletteSlot, paletteTag, 0);
}
...
{
    return sub_818D65C(species, otId, personality, isFrontPic, paletteSlot, windowId, 0);
}
...
{
    return CreateTrainerCardSprite(species, otId, personality, isFrontPic, destX, destY, paletteSlot, windowId, 0);
}

u16 CreateTrainerPicSprite(u16 species, bool8 isFrontPic, s16 x, s16 y, u8 paletteSlot, u16 paletteTag)
{
    return CreatePicSprite_HandleDeoxys(species, 0, 0, isFrontPic, x, y, paletteSlot, paletteTag, 1);
}
...
{
    return sub_818D65C(species, 0, 0, isFrontPic, paletteSlot, windowId, 1);
}

u16 CreateTrainerCardTrainerPicSprite(u16 species, bool8 isFrontPic, u16 destX, u16 destY, u8 paletteSlot, u8 windowId)
{
    return CreateTrainerCardSprite(species, 0, 0, isFrontPic, destX, destY, paletteSlot, windowId, 1);
}
