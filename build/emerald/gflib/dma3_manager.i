# 0 "gflib/dma3_manager.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "gflib/dma3_manager.c"
...
# 3 "gflib/dma3_manager.c" 2
# 11 "gflib/dma3_manager.c"
...
void ClearDma3Requests(void)
{
    ...
    for (i = 0; i < 128; i++)
    {
        ...
        gDma3Requests[i].src = ((void *)0);
        gDma3Requests[i].dest = ((void *)0);
    }

    gDma3ManagerLocked = 0;
}
...
void ProcessDma3Requests(void)
{
    ...
    while (gDma3Requests[gDma3RequestCursor].size != 0)
    {
        ...
        if (bytesTransferred > 40 * 1024)
            return;
        if (*(u8 *)(0x4000000 + 0x6) > 224)
            return;

        switch (gDma3Requests[gDma3RequestCursor].mode)
        {
        case 1:
            { const void *_src = gDma3Requests[gDma3RequestCursor].src; void *_dest = gDma3Requests[gDma3RequestCursor].dest; u32 _size = gDma3Requests[gDma3RequestCursor].size; while (1) { if (_size <= 0x1000) { { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(_src); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((_size)/(32/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; break; } { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(_src); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0400 | 0x0000 | 0x0000) << 16 | ((0x1000)/(32/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; _src += 0x1000; _dest += 0x1000; _size -= 0x1000; } }

                                                                    ;
            break;
        case 2:
            { void *_dest = gDma3Requests[gDma3RequestCursor].dest; u32 _size = gDma3Requests[gDma3RequestCursor].size; while (1) { if (_size <= 0x1000) { { vu32 tmp = (vu32)(gDma3Requests[gDma3RequestCursor].value); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((_size)/(32/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; }; break; } { vu32 tmp = (vu32)(gDma3Requests[gDma3RequestCursor].value); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((0x1000)/(32/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; }; _dest += 0x1000; _size -= 0x1000; } }
        ...
        case 3:
            ...
            break;
        case 4:
            { void *_dest = gDma3Requests[gDma3RequestCursor].dest; u32 _size = gDma3Requests[gDma3RequestCursor].size; while (1) { if (_size <= 0x1000) { { vu16 tmp = (vu16)(gDma3Requests[gDma3RequestCursor].value); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((_size)/(16/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; }; break; } { vu16 tmp = (vu16)(gDma3Requests[gDma3RequestCursor].value); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); u32 eval_src = (u32)(&tmp); u32 eval_dst = (u32)(_dest); u32 eval_ctl = (u32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((0x1000)/(16/8))); register u32 r_src asm("r0") = eval_src; register u32 r_dst asm("r1") = eval_dst; register u32 r_ctl asm("r2") = eval_ctl; asm volatile("stmia %0!, {%1, %2, %3}" : "+l" (dmaRegs) : "l" (r_src), "l" (r_dst), "l" (r_ctl) : "memory"); }; }; _dest += 0x1000; _size -= 0x1000; } }
...
s16 RequestDma3Copy(const void *src, void *dest, u16 size, u8 mode)
{
    ...
    while (i < 128)
    {
        if (gDma3Requests[cursor].size == 0)
        {
