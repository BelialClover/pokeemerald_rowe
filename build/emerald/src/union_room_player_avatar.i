# 0 "src/union_room_player_avatar.c"
# 0 "<built-in>"
# 0 "<command-line>"
...
static __attribute__((section("ewram_data"))) struct UnionRoomObject * sUnionObjWork = ((void *)0);
static __attribute__((section("ewram_data"))) u32 sUnionObjRefreshTimer = 0;
...
static const u8 sUnionRoomObjGfxIds[2][8 + 2] = {
    [0] = {
        33,
        44,
        31,
...
static const s16 sUnionRoomPlayerCoords[8][2] = {
    { 4, 6},
    {13, 8},
    {10, 6},
    { 1, 8},
...
static bool32 AnimateUnionRoomPlayerDespawn(s8 * state, u32 playerIdx, struct UnionRoomObject * ptr)
{
    ...
    {
    case 0:
        if (SetUnionRoomPlayerEnterExitMovement(playerIdx, sMovement_UnionPlayerExit) == 1)
        {
...
static bool32 AnimateUnionRoomPlayerSpawn(s8 * state, u32 playerIdx, struct UnionRoomObject * ptr)
{
    ...
    case 0:
        ...
        if (IsUnionRoomPlayerFacingTileAt(playerIdx, 0, x, y) == 1)
        {
            break;
        }
...
static void AnimateUnionRoomPlayer(u32 playerIdx, struct UnionRoomObject * ptr)
{
    ...
    case 2:
        if (!IsUnionRoomPlayerInvisible(playerIdx, 0) && ptr->schedAnim == 2)
        {
            ptr->state = 0;
            ptr->animState = 0;
            RemoveUnionRoomPlayerObjectEvent(playerIdx);
        ...
        else if (AnimateUnionRoomPlayerSpawn(&ptr->animState, playerIdx, ptr) == 1)
        {
            ptr->state = 1;
        }
        break;
...
static u8 CreateTask_AnimateUnionRoomPlayers(void)
{
    if (FuncIsActiveTask(Task_AnimateUnionRoomPlayers) == 1)
        return 16;
    else
...
void CreateGroupMemberSpritesInvisible(u8 * spriteIds, s32 playerIdx)
{
    ...
    {
        s32 id = (5 * playerIdx + direction);
        spriteIds[id] = CreateObjectSprite(65,
                                           id - (64 - 8),
                                           ...
                                           3, 1);
        SetObjectEventSpriteInvisibility(id - (64 - 8), 1);
    }
}
...
void SetTilesAroundUnionRoomPlayersPassable(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        for (direction = 0; direction <= 4; direction++)
        {
            ...
            MapGridSetMetatileImpassabilityAt(x, y, 0);
        }
    }
}
...
{
    return IsObjectEventSpriteInvisible((5 * playerIdx + direction) - (64 - 8));
}
...
static void SpawnGroupMember(u32 playerIdx, u32 direction, u8 graphicsId, struct GFtgtGname * gname)
{
    s32 x, y;
    s32 id = (5 * playerIdx + direction);
    if (IsUnionRoomPlayerInvisible(playerIdx, direction) == 1)
    {
        SetObjectEventSpriteInvisibility(id - (64 - 8), 0);
        SetObjectEventSpriteAnim(id - (64 - 8), 1);
    }
    SetObjectEventSpriteGraphics(id - (64 - 8), graphicsId);
    ...
    MapGridSetMetatileImpassabilityAt(x, y, 1);
}

static void DespawnGroupMember(u32 playerIdx, u32 direction)
{
    s32 x, y;
    SetObjectEventSpriteAnim((5 * playerIdx + direction) - (64 - 8), 2);
    GetUnionRoomPlayerFacingCoords(playerIdx, direction, &x, &y);
    MapGridSetMetatileImpassabilityAt(x, y, 0);
}
...
static void AssembleGroup(u32 playerIdx, struct GFtgtGname * gname)
{
    ...
    if (IsObjectEventSpriteInvisible((5 * playerIdx + 0) - (64 - 8)) == 1)
    {
        if (IsUnionRoomPlayerFacingTileAt(playerIdx, 0, x, y) == 1 || IsUnionRoomPlayerFacingTileAt(playerIdx, 0, x2, y2) == 1)
        {
            return;
        }
    ...
    for (i = 1; i < 5; i++)
    {
        ...
        else if (IsUnionRoomPlayerFacingTileAt(playerIdx, i, x, y) == 0 && IsUnionRoomPlayerFacingTileAt(playerIdx, i, x2, y2) == 0)
        {
...
static void UpdateUnionRoomPlayerSprites(struct WirelessLink_URoom *uroom)
{
    ...
    sUnionObjRefreshTimer = 0;
    for (i = 0, r4 = uroom->field_0->arr; i < 8; i++)
    {
...
bool32 TryInteractWithUnionRoomMember(struct UnkStruct_Main0 *main0, s16 *directionPtr, s16 *playerIdxPtr, u8 *spriteIds)
{
    ...
    for (i = 0, r4 = main0->arr; i < 8; i++)
    {
        for (direction = 0; direction <= 4; direction++)
        {
            s32 id = (5 * i + direction);
            ...
            if (IsObjectEventSpriteInvisible(id - (64 - 8)))
            {
                continue;
            }
            if (IsObjectEventSpriteAnimating(id - (64 - 8)))
            {
                continue;
            }
            if (r4[i].groupScheduledAnim != 1)
...
{
    TurnObjectEventSprite(5 * playerIdx - (64 - 8) + currDirection, newDirection);
