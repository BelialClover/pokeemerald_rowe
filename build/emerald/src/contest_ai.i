...
void ContestAI_ResetAI(u8 contestantAI)
{
    ...
    (*gContestResources->aiData).aiFlags = gContestMons[(*gContestResources->aiData).contestantId].aiFlags;
}
...
u8 ContestAI_GetActionToUse(void)
{
    ...
    while (1)
    {
        ...
        u8 moveIdx = Random() & (4 - 1);
        u8 score = (*gContestResources->aiData).moveScores[moveIdx];
        int i;
        for (i = 0; i < 4; i++)
        {
    ...
    while ((*gContestResources->aiData).aiState != CONTESTAI_FINISHED)
    {
        switch((*gContestResources->aiData).aiState)
        {
            ...
            case CONTESTAI_SETTING_UP:
                gAIScriptPtr = gContestAI_ScriptsTable[(*gContestResources->aiData).currentAIFlag];
            ...
            case CONTESTAI_PROCESSING:
                ...
                {
                    (*gContestResources->aiData).moveScores[(*gContestResources->aiData).nextMoveIndex] = 0;
...
static void ContestAICmd_score(void)
{
    s16 score = (*gContestResources->aiData).moveScores[(*gContestResources->aiData).nextMoveIndex] + (s8)gAIScriptPtr[1];
    ...
    (*gContestResources->aiData).moveScores[(*gContestResources->aiData).nextMoveIndex] = score;
...
static void ContestAICmd_get_appeal_num(void)
{
    (*gContestResources->aiData).scriptResult = (*gContestResources->contest).appealNumber;
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_user_order(void)
{
    (*gContestResources->aiData).scriptResult = (*gContestResources->appealResults).turnOrder[(*gContestResources->aiData).contestantId];
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_get_preliminary_points(void)
{
    (*gContestResources->aiData).scriptResult = gContestMonRound1Points[(*gContestResources->aiData).contestantId];
    gAIScriptPtr += 1;
}
...
static void ContestAICmd_if_combo_starter(void)
{
    ...
    if ((*gContestResources->aiData).scriptResult)
        gAIScriptPtr = (u8*) ((gAIScriptPtr + 0)[0] | ((gAIScriptPtr + 0)[1] << 8) | ((gAIScriptPtr + 0)[2] << 16) | ((gAIScriptPtr + 0)[3] << 24));
    else
        gAIScriptPtr += 4;
}
...
static void ContestAICmd_get_condition(void)
{
    ...
    (*gContestResources->aiData).scriptResult = (gContestResources->status)[contestant].condition / 10;
    gAIScriptPtr += 2;
}
...
static void ContestAICmd_get_used_combo_starter(void)
{
    ...
    if (IsContestantAllowedToCombo(contestant))
        result = gContestMoves[(gContestResources->status)[contestant].prevMove].comboStarterId ? 1 : 0;
...
static void ContestAICmd_get_completed_combo(void)
{
    ...
    (*gContestResources->aiData).scriptResult = (gContestResources->status)[contestant].completedComboFlag;
    gAIScriptPtr += 2;
}
...
static void ContestAICmd_get_points_diff(void)
{
    ...
    (*gContestResources->aiData).scriptResult = (gContestResources->status)[contestant].pointTotal - (gContestResources->status)[(*gContestResources->aiData).contestantId].pointTotal;
    gAIScriptPtr += 2;
}
...
static void ContestAICmd_get_preliminary_points_diff(void)
{
    ...
    (*gContestResources->aiData).scriptResult = gContestMonRound1Points[contestant] - gContestMonRound1Points[(*gContestResources->aiData).contestantId];
    gAIScriptPtr += 2;
}
...
static void ContestAICmd_get_used_moves_effect(void)
{
    ...
    u16 move = (*gContestResources->contest).moveHistory[round][contestant];
...
static void ContestAICmd_get_used_moves_excitement(void)
{
    u8 contestant = GetContestantIdByTurn(gAIScriptPtr[1]);
    u8 round = gAIScriptPtr[2];
    s8 result = (*gContestResources->contest).excitementHistory[round][contestant];
...
static void ContestAICmd_save_result(void)
{
    (*gContestResources->aiData).vars[gAIScriptPtr[1]] = (*gContestResources->aiData).scriptResult;
    gAIScriptPtr += 2;
}

static void ContestAICmd_setvar(void)
{
    (*gContestResources->aiData).vars[gAIScriptPtr[1]] = ((gAIScriptPtr + 2)[0] | ((gAIScriptPtr + 2)[1] << 8));
    gAIScriptPtr += 4;
}
...
static void ContestAICmd_add(void)
{
    ...
    (*gContestResources->aiData).vars[gAIScriptPtr[1]] += ((s8)gAIScriptPtr[2] | gAIScriptPtr[3] << 8);
    gAIScriptPtr += 4;
}

static void ContestAICmd_addvar(void)
{
    (*gContestResources->aiData).vars[gAIScriptPtr[1]] += (*gContestResources->aiData).vars[gAIScriptPtr[2]];
    gAIScriptPtr += 3;
}
...
static void ContestAICmd_if_less_than(void)
{
    if ((*gContestResources->aiData).vars[gAIScriptPtr[1]] < ((gAIScriptPtr + 2)[0] | ((gAIScriptPtr + 2)[1] << 8)))
...
static void ContestAICmd_if_less_than_var(void)
{
    if ((*gContestResources->aiData).vars[gAIScriptPtr[1]] < ((*gContestResources->aiData).vars[gAIScriptPtr[2]]))
...
static void ContestAICmd_if_random_less_than(void)
{
    ...
    if ((Random() & 0xFF) < (*gContestResources->aiData).vars[gAIScriptPtr[1]])
...
static void ContestAICmd_check_user_has_move(void)
{
    int hasMove = 0;
    int i;
    u16 targetMove = ((gAIScriptPtr + 1)[0] | ((gAIScriptPtr + 1)[1] << 8));
    ...
    (*gContestResources->aiData).scriptResult = hasMove;
    gAIScriptPtr += 3;
}
