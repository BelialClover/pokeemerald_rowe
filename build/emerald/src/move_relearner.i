# 0 "src/move_relearner.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/move_relearner.c"
...
# 24 "src/move_relearner.c" 2
# 1 "include/task.h" 1
# 25 "src/move_relearner.c" 2
# 1 "include/constants/rgb.h" 1
# 26 "src/move_relearner.c" 2
# 1 "include/constants/songs.h" 1
# 27 "src/move_relearner.c" 2
# 155 "src/move_relearner.c"
static __attribute__((section("ewram_data"))) struct
{
    u8 state;
    u8 heartSpriteIds[16];
    u16 movesToLearn[(40 > 50 ? 40 : 50)];
    u8 partyMon;
    u8 moveSlot;
    struct ListMenuItem menuItems[(40 > 50 ? 40 : 50)];
    u8 numMenuChoices;
    u8 numToShowAtOnce;
    u8 moveListMenuTask;
...
{
    [0] = sHeartSprite_AppealEmptyFrame,
    [1] = sHeartSprite_AppealFullFrame,
    [2] = sHeartSprite_JamEmptyFrame,
    [3] = sHeartSprite_JamFullFrame,
};
...
static void InitMoveRelearnerBackgroundLayers(void)
{
    ...
    InitBgsFromTemplates(0, sMoveRelearnerMenuBackgroundTemplates, (size_t)(sizeof(sMoveRelearnerMenuBackgroundTemplates) / sizeof((sMoveRelearnerMenuBackgroundTemplates)[0])));
...
static void DoMoveRelearnerMain(void)
{
    ...
    case 0:
        ...
        HideHeartSpritesAndShowTeachMoveText(0);
    ...
    case 9:
        {
            ...
            {
                if (sMoveRelearnerMenuSate.showContestInfo == 0)
                {
                    sMoveRelearnerStruct->state = 3;
                }
                else if (sMoveRelearnerMenuSate.showContestInfo == 1)
                {
    ...
    case 28:
        ...
        if (sMoveRelearnerMenuSate.showContestInfo == 0)
        {
            HideHeartSpritesAndShowTeachMoveText(1);
        }
        else if (sMoveRelearnerMenuSate.showContestInfo == 1)
        {
            ShowTeachMoveText(1);
    ...
    case 29:
        if (!gPaletteFade.active)
        {
            ...
            {
                u16 moveId = GetMonData(&gPlayerParty[sMoveRelearnerStruct->partyMon], 13 + sMoveRelearnerStruct->moveSlot);
...
static void FreeMoveRelearnerResources(void)
{
    ...
    { Free(sMoveRelearnerStruct); sMoveRelearnerStruct = ((void *)0); };
    ResetSpriteData();
    FreeAllSpritePalettes();
}
...
static void HideHeartSpritesAndShowTeachMoveText(bool8 onlyHideSprites)
{
    ...
    for (i = 0; i < 16; i++)
    {
        gSprites[sMoveRelearnerStruct->heartSpriteIds[i]].invisible = 1;
    }
    ...
    if (!onlyHideSprites)
    {
        ...
        FillWindowPixelBuffer(3, 0x11);
        AddTextPrinterParameterized(3, 1, gStringVar4, 0, 1, 0, ((void *)0));
    }
}
...
static void HandleInput(bool8 showContest)
{
    ...
    {
    case -1:
        if (!(({(gMain.newKeys) & (0x0020 | 0x0010);})) && !GetLRKeysPressed())
        {
            break;
        }
        ...
        if (showContest == 0)
        {
            PutWindowTilemap(1);
            sMoveRelearnerStruct->state = 5;
            sMoveRelearnerMenuSate.showContestInfo = 1;
        }
        else
        {
            PutWindowTilemap(0);
...
static void CreateLearnableMovesList(void)
{
    ...
    GetMonData(&gPlayerParty[sMoveRelearnerStruct->partyMon], 2, nickname);
...
void MoveRelearnerShowHideHearts(s32 moveId)
{
    ...
    if (!sMoveRelearnerMenuSate.showContestInfo || moveId == -2)
    {
        for (i = 0; i < 16; i++)
        {
            gSprites[sMoveRelearnerStruct->heartSpriteIds[i]].invisible = 1;
    ...
    else
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            gSprites[sMoveRelearnerStruct->heartSpriteIds[i]].invisible = 0;
        }
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            gSprites[sMoveRelearnerStruct->heartSpriteIds[i + 8]].invisible = 0;
        }
    }
}
