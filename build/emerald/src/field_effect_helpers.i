# 0 "src/field_effect_helpers.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/field_effect_helpers.c"
...
# 2 "src/field_effect_helpers.c" 2
...
# 3 "src/field_effect_helpers.c" 2
...
# 4 "src/field_effect_helpers.c" 2
...
# 5 "src/field_effect_helpers.c" 2
...
# 6 "src/field_effect_helpers.c" 2
...
# 7 "src/field_effect_helpers.c" 2
...
# 8 "src/field_effect_helpers.c" 2
...
# 9 "src/field_effect_helpers.c" 2
...
# 11 "src/field_effect_helpers.c" 2
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 13 "src/field_effect_helpers.c" 2
# 1 "include/constants/field_effects.h" 1
# 14 "src/field_effect_helpers.c" 2
# 1 "include/constants/songs.h" 1
# 15 "src/field_effect_helpers.c" 2
...
void SetUpReflection(struct ObjectEvent *objectEvent, struct Sprite *sprite, bool8 stillReflection)
{
    ...
    reflectionSprite->affineAnimBeginning = 1;
    ...
    if (!stillReflection)
        reflectionSprite->oam.affineMode = 1;
}
...
static void UpdateObjectReflectionSprite(struct Sprite *reflectionSprite)
{
    ...
    else
    {
        ...
        if (objectEvent->hideReflection == 1)
            reflectionSprite->invisible = 1;


        if (reflectionSprite->data[7] == 0)
        {



            reflectionSprite->oam.matrixNum = 0;
            if (mainSprite->oam.matrixNum & 0x08)
...
static void UpdateAshFieldEffect_Step1(struct Sprite *sprite)
{
    ...
    gObjectEvents[gPlayerAvatar.objectEventId].triggerGroundEffectsOnMove = 1;
    sprite->data[0] = 2;
}
...
void sub_81556E8(struct ObjectEvent *objectEvent, struct Sprite *sprite)
{
    ...
    if (spriteY == 0 && (x != sprite->data[6] || y != sprite->data[7]))
    {
        ...
        sprite->data[6] = x;
        sprite->data[7] = y;
        for (i = 1; i <= 4; i++, x = sprite->data[6], y = sprite->data[7])
        {
            MoveCoords(i, &x, &y);
...
# 1373 "src/field_effect_helpers.c"
...
static bool8 AnimateRayquazaInFigure8(struct Sprite *sprite)
{
    ...
    case 0:
        ...
        break;
    case 1:
        sprite->pos2.x -= GetFigure8XOffset((72 - 1) - sprite->data[6]);
        sprite->pos2.y += GetFigure8YOffset((72 - 1) - sprite->data[6]);
        break;
    case 2:
        sprite->pos2.x -= GetFigure8XOffset(sprite->data[6]);
        sprite->pos2.y += GetFigure8YOffset(sprite->data[6]);
...
void UpdateRayquazaSpotlightEffect(struct Sprite *sprite)
{
    ...
    {
        case 0:
            SetGpuReg(0x12, 240 / 2 - (sprite->data[0] / 3));
            if (sprite->data[0] == 96)
            {
                for (i = 0; i < 3; i++)
                {
        ...
        case 8:
            for (i = 0; i < 15; i++)
            {
                for (j = 12; j < 18; j++)
                {
                    ((u16*)((0x6000000 + (0x800 * (31)))))[i * 32 + j] = 0;
                }
            }
            SetGpuReg(0x12, 0);
...
# 1549 "src/field_effect_helpers.c"
