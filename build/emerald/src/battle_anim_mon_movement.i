# 0 "src/battle_anim_mon_movement.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_anim_mon_movement.c"
...
# 2 "src/battle_anim_mon_movement.c" 2
...
# 3 "src/battle_anim_mon_movement.c" 2
...
# 4 "src/battle_anim_mon_movement.c" 2

# 1 "include/task.h" 1
# 6 "src/battle_anim_mon_movement.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 7 "src/battle_anim_mon_movement.c" 2
...
# 94 "src/battle_anim_mon_movement.c"
...
# 157 "src/battle_anim_mon_movement.c"
void AnimTask_ShakeMon2(u8 taskId)
{
    ...
    else if (gBattleAnimArgs[0] != 8)
    {
        ...
        {
        case 4:
            battlerId = GetBattlerAtPosition(0);
            break;
        case 5:
            battlerId = GetBattlerAtPosition(2);
            break;
        case 6:
        ...
        if (IsBattlerSpriteVisible(battlerId) == 0)
            destroy = 1;
...
# 256 "src/battle_anim_mon_movement.c"
...
# 366 "src/battle_anim_mon_movement.c"
...
# 420 "src/battle_anim_mon_movement.c"
...
# 555 "src/battle_anim_mon_movement.c"
static void SlideMonToOffset(struct Sprite *sprite)
{
    u8 battler;
    u8 monSpriteId;
...
# 647 "src/battle_anim_mon_movement.c"
...
# 761 "src/battle_anim_mon_movement.c"
...
static void AnimTask_SwayMonStep(u8 taskId)
{
    ...
    {
        if (GetBattlerSide(gTasks[taskId].data[5]) == 0)
        {
...
# 1075 "src/battle_anim_mon_movement.c"
void AnimTask_RotateVertically(u8 taskId)
{
    ...
    gTasks[taskId].data[5] = ((GetBattlerSide(GetAnimBattlerId(gBattleAnimArgs[0]))) == 0);
    gTasks[taskId].data[6] = gTasks[taskId].data[5] ? 0x1FFF : 0x7FFE;
    gTasks[taskId].data[1] = gBattleAnimArgs[1];
...
static void AnimTask_RotateVerticallyStep(u8 taskId)
{
    ...
    {
    case 0:
        gTasks[taskId].data[2] = ((abs(gTasks[taskId].data[2] + gTasks[taskId].data[1])) < (gTasks[taskId].data[6]) ? (abs(gTasks[taskId].data[2] + gTasks[taskId].data[1])) : (gTasks[taskId].data[6]));
        SetSpriteRotScale(gTasks[taskId].data[0], 0x100, 0x100, gTasks[taskId].data[2]);
        if (gTasks[taskId].data[5])
            SetBattlerSpriteYOffsetFromRotation(gTasks[taskId].data[0]);

        if (gTasks[taskId].data[2] == gTasks[taskId].data[6])
            gTasks[taskId].data[3]++;
        break;
    case 1:
        if (++gTasks[taskId].data[4] >= 75)
            gTasks[taskId].data[3]++;
        break;
    case 2:
        if (gTasks[taskId].data[2] < gTasks[taskId].data[1])
            gTasks[taskId].data[2] = 0;
        else
            gTasks[taskId].data[2] = gTasks[taskId].data[2] - gTasks[taskId].data[1];
        SetSpriteRotScale(gTasks[taskId].data[0], 0x100, 0x100, gTasks[taskId].data[2]);
        if (gTasks[taskId].data[5])
            SetBattlerSpriteYOffsetFromRotation(gTasks[taskId].data[0]);
        ...
        break;
    case 3:
        ResetSpriteRotScale(gTasks[taskId].data[0]);
        DestroyAnimVisualTask(taskId);
        break;
    }
}
