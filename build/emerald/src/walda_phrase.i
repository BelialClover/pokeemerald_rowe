# 0 "src/walda_phrase.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/walda_phrase.c"
...
# 6 "src/walda_phrase.c" 2
# 1 "include/main.h" 1
# 7 "src/walda_phrase.c" 2
# 1 "gflib/text.h" 1
# 272 "gflib/text.h"
enum
{
...
{
    0xBC, 0xBD, 0xBE, 0xC0, 0xC1, 0xC2, 0xC4, 0xC5, 0xC6, 0xC7, 0xC8, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE, 0xD0, 0xD1, 0xD4,
    0xD6, 0xD7, 0xD8, 0xDA, 0xDB, 0xDC, 0xDE, 0xDF, 0xE1, 0xE2, 0xE4, 0xE5, 0xE7
};
...
static u8 GetLetterTableId(u8 letter)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sWaldaLettersTable) / sizeof((sWaldaLettersTable)[0])); i++)
    {
        if (sWaldaLettersTable[i] == letter)
            return i;
    }

    return (size_t)(sizeof(sWaldaLettersTable) / sizeof((sWaldaLettersTable)[0]));
}

static bool32 TryCalculateWallpaper(u16* backgroundClr, u16 *foregroundClr, u8 *iconId, u8 *patternId, u16 trainerId, u8 *phrase)
{
    s32 i;
    __attribute__((aligned(2))) u8 array[12];
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        if (charsByTableId[i] == (size_t)(sizeof(sWaldaLettersTable) / sizeof((sWaldaLettersTable)[0])))
            return 0;
    }
