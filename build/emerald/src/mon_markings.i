# 0 "src/mon_markings.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/mon_markings.c"
...
# 8 "src/mon_markings.c" 2
# 1 "include/constants/songs.h" 1
# 9 "src/mon_markings.c" 2
# 1 "include/sound.h" 1
...
static __attribute__((section("ewram_data"))) struct PokemonMarkMenu *sMenu = ((void *)0);
...
void sub_811F918(void)
{
    ...
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, sMenu->menuWindowSpriteTiles, 0x00000000 | 0x01000000 | ((sizeof(sMenu->menuWindowSpriteTiles))/(16/8) & 0x1FFFFF)); };
}
...
bool8 sub_811F960(void)
{
    ...
    {
    case 0:
        CpuFastSet(sMenu->frameTiles, dest, ((32)/(32/8) & 0x1FFFFF));
        for (i = 0; i < 6; i++)
        {
            CpuFastSet(sMenu->frameTiles + 32, dest + 32 * (i + 1), ((32)/(32/8) & 0x1FFFFF));
        }
        CpuFastSet(sMenu->frameTiles + 32 * 2, dest + 32 * 7, ((32)/(32/8) & 0x1FFFFF));
        sMenu->tileLoadState++;
        break;
    default:
        CpuFastSet(sMenu->frameTiles + 32 * 3, dest, ((32)/(32/8) & 0x1FFFFF));
        for (i = 0; i < 6; i++)
        {
            CpuFastSet(sMenu->frameTiles + 32 * 4, dest + 32 * (i + 1), ((32)/(32/8) & 0x1FFFFF));
        }
        CpuFastSet(sMenu->frameTiles + 32 * 5, dest + 32 * 7, ((32)/(32/8) & 0x1FFFFF));
        sMenu->tileLoadState++;
        break;
    case 13:
        CpuFastSet(sMenu->frameTiles + 32 * 6, dest, ((32)/(32/8) & 0x1FFFFF));
        for (i = 0; i < 6; i++)
        {
            CpuFastSet(sMenu->frameTiles + 32 * 7, dest + 32 * (i + 1), ((32)/(32/8) & 0x1FFFFF));
        }
        CpuFastSet(sMenu->frameTiles + 32 * 8, dest + 32 * 7, ((32)/(32/8) & 0x1FFFFF));
        sMenu->tileLoadState++;
        return 0;
    case 14:
        return 0;
...
void sub_811FAF8(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        sMenu->menuWindowSprites[i] = ((void *)0);
    }
    for (i = 0; i < 4; i++)
    {
...
static void sub_811FC80(s16 x, s16 y, u16 baseTileTag, u16 basePaletteTag)
{
    ...
    if (spriteId != 64)
    {
        ...
        sMenu->menuTextSprite->oam.shape = (((2 << 2) | (0)) & 0x03);
        sMenu->menuTextSprite->oam.size = ((((2 << 2) | (0)) >> 2) & 0x03);
        StartSpriteAnim(sMenu->menuTextSprite, 9);
        sMenu->menuTextSprite->pos1.x = x + 32;
        sMenu->menuTextSprite->pos1.y = y + 80;
        CalcCenterToCornerVec(sMenu->menuTextSprite, (((2 << 2) | (1)) & 0x03), ((((2 << 2) | (1)) >> 2) & 0x03), 0);
    }
    else
    {
        sMenu->menuTextSprite = ((void *)0);
