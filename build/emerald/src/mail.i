...
static __attribute__((section("ewram_data"))) struct MailRead *sMailRead = ((void *)0);
...
void ReadMail(struct MailStruct *mail, void (*callback)(void), bool8 flag)
{
    ...
    sMailRead = AllocZeroed((1) * (sizeof(*sMailRead)));
    sMailRead->language = 2;
    sMailRead->playerIsSender = 1;
    sMailRead->parserSingle = CopyEasyChatWord;
    sMailRead->parserMultiple = ConvertEasyChatWordsToString;
    if (((mail->itemId == 132 || mail->itemId == 133 || mail->itemId == 134 || mail->itemId == 135 || mail->itemId == 136 || mail->itemId == 137 || mail->itemId == 138 || mail->itemId == 139 || mail->itemId == 140 || mail->itemId == 141 || mail->itemId == 142 || mail->itemId == 143)))
    {
        sMailRead->mailType = mail->itemId - 132;
    }
    else
    {
    ...
    if (species >= 1 && species < 898 + 308 + 1)
    {
...
static void CB2_InitMailRead(void)
{
    ...
    } while (MenuHelpers_LinkSomething() != 1);
}
...
static void sub_8121B1C(void)
{
    ...
    for (i = 0; i < sMailRead->layout->numSubStructs; i ++)
    {
        if (sMailRead->strbuf[i][0] == 0xFF || sMailRead->strbuf[i][0] == 0x00)
        {
            continue;
        }
...
static void CB2_ExitMailReadFreeVars(void)
{
    if (!UpdatePaletteFade())
    {
        ...
        { Free(sMailRead); sMailRead = ((void *)0); };
    }
}
