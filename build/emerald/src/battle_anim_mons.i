...
u8 GetBattlerSpriteCoord(u8 battlerId, u8 coordType)
{
    ...
    case BATTLER_COORD_X_2:
        retVal = sBattlerCoords[((gBattleTypeFlags & (1 << 0)))][GetBattlerPosition(battlerId)].x;
        break;
    case BATTLER_COORD_Y:
        retVal = sBattlerCoords[((gBattleTypeFlags & (1 << 0)))][GetBattlerPosition(battlerId)].y;
        break;
...
u8 GetBattlerSpriteFinal_Y(u8 battlerId, u16 species, bool8 a3)
{
    ...
    y = offset + sBattlerCoords[((gBattleTypeFlags & (1 << 0)))][GetBattlerPosition(battlerId)].y;
    if (a3)
    {
        if (GetBattlerSide(battlerId) == 0)
            y += 8;
...
void InitSpritePosToAnimAttackerPartner(struct Sprite *sprite, bool8 respectMonPicOffsets)
{
    if (!respectMonPicOffsets)
    {
        sprite->pos1.x = GetBattlerSpriteCoord2(((gBattleAnimAttacker) ^ 2), BATTLER_COORD_X);
        sprite->pos1.y = GetBattlerSpriteCoord2(((gBattleAnimAttacker) ^ 2), BATTLER_COORD_Y);
    }
    else
    {
        sprite->pos1.x = GetBattlerSpriteCoord2(((gBattleAnimAttacker) ^ 2), BATTLER_COORD_X_2);
        sprite->pos1.y = GetBattlerSpriteCoord2(((gBattleAnimAttacker) ^ 2), BATTLER_COORD_Y_PIC_OFFSET);
    }
    SetAnimSpriteInitialXOffset(sprite, gBattleAnimArgs[0]);
    sprite->pos1.y += gBattleAnimArgs[1];
}
...
u8 sub_80A8394(u16 species, bool8 isBackpic, u8 a3, s16 x, s16 y, u8 subpriority, u32 personality, u32 trainerId, u32 battlerId, u32 a10, u8 formId)
{
    ...
    if (gMonSpritesGfxPtr != ((void *)0) && gMonSpritesGfxPtr->buffer == ((void *)0))
    ...
    RequestDma3Copy(gMonSpritesGfxPtr->buffer, (void *)((0x6000000 + 0x10000) + (sheet * 0x20)), 0x800, 1);
    { Free(gMonSpritesGfxPtr->buffer); gMonSpritesGfxPtr->buffer = ((void *)0); };

    if (!isBackpic)
        spriteId = CreateSprite(&sUnknown_08525F90[a3], x, y + gMonFrontPicCoords[formSpeciesId].y_offset, subpriority);
    else
        spriteId = CreateSprite(&sUnknown_08525F90[a3], x, y + gMonBackPicCoords[formSpeciesId].y_offset, subpriority);
