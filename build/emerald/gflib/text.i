...
__attribute__((section("ewram_data"))) struct TextPrinter gTempTextPrinter = {0};
__attribute__((section("ewram_data"))) struct TextPrinter gTextPrinters[32] = {0};
...
const struct FontInfo gFontInfos[] =
{
    ...
    { ((void *)0), 0x8, 0x8, 0x0, 0x0, 0x0, 0x1, 0x2, 0xF }
};
...
u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    {
    case 0:
        if ((({(gMain.heldKeys) & (0x0001 | 0x0002);})) && subStruct->hasPrintBeenSpedUp)
            textPrinter->delayCounter = 0;

        if (textPrinter->delayCounter && textPrinter->textSpeed)
        {
            textPrinter->delayCounter--;
            if (gTextFlags.canABSpeedUpPrint && (({(gMain.newKeys) & (0x0001 | 0x0002);})))
            {
    ...
    case 4:
        if (textPrinter->scrollDistance)
        {
            ...
            {
                ScrollWindow(textPrinter->printerTemplate.windowId, 0, speed, ((textPrinter->printerTemplate.bgColor) | ((textPrinter->printerTemplate.bgColor) << 4)));
...
u8 RenderTextFont9(u8 *pixels, u8 fontId, u8 *str)
{
    ...
    do
    {
        ...
        default:
            ...
            CpuSet(gUnknown_03002F90.unk0, pixels, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            CpuSet(gUnknown_03002F90.unk40, pixels + 0x20, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            pixels += 0x40;
            break;
        }
    }
