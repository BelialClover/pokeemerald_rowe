# 0 "src/battle_anim_ice.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_anim_ice.c"
...
# 10 "src/battle_anim_ice.c" 2

# 1 "include/task.h" 1
# 12 "src/battle_anim_ice.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 13 "src/battle_anim_ice.c" 2
# 1 "include/constants/battle_anim.h" 1
# 14 "src/battle_anim_ice.c" 2
# 1 "include/constants/rgb.h" 1
# 15 "src/battle_anim_ice.c" 2
...
{
    {.frame = {.xScale = 0xCE, .yScale = 0xCE, .rotation = 0, .duration = 0}},
...
{
    {.frame = {.xScale = 0x118, .yScale = 0x118, .rotation = 0, .duration = 0}},
    {.type = 0x7FFF},
};
...
{
    {.frame = {.xScale = 0x1C0, .yScale = 0x1C0, .rotation = 0, .duration = 0}},
    {.type = 0x7FFF},
};
...
static void AnimTask_OverlayFogTiles(u8 taskId)
{
    ...
    case 0:
        ...
        SetGpuReg(0x52, (((17 - gTasks[taskId].data[11]) << 8) | (gTasks[taskId].data[11])));
...
static void InitPoisonGasCloudAnim(struct Sprite *sprite)
{
    ...
    if ((gBattlerPositions[gBattleAnimTarget] & 1) == 0)
    {
        ...
        if ((sprite->data[7] & 0x8000) && (gBattlerPositions[gBattleAnimAttacker] & 1) == 0)
...
static void MovePoisonGasCloud(struct Sprite *sprite)
{
    ...
    case 0:
        ...
        if (sprite->data[0] <= 0)
        {
            ...
            if (IsContest())
                sprite->data[5] = 80;
            else if (((gBattlerPositions[gBattleAnimTarget]) & 1) != 0)
                sprite->data[5] = 204;
            else
                sprite->data[5] = 80;
