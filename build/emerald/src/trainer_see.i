...
# 7 "src/trainer_see.c" 2
# 1 "include/pokemon.h" 1
# 8 "src/trainer_see.c" 2
...
__attribute__((section("ewram_data"))) u8 gApproachingTrainerId = 0;
...
static u8 GetTrainerApproachDistance(struct ObjectEvent *trainerObj)
{
    ...
    {
        for (i = 0; i < (size_t)(sizeof(sDirectionalApproachDistanceFuncs) / sizeof((sDirectionalApproachDistanceFuncs)[0])); i++)
        {
...
static void TrainerApproachPlayer(struct ObjectEvent *trainerObj, u8 range)
{
    ...
    task->data[3] = range;
    task->data[7] = gApproachingTrainers[gNoOfApproachingTrainers].objectEventId;
}
...
static void Task_RunTrainerSeeFuncList(u8 taskId)
{
    ...
    struct ObjectEvent *trainerObj = &gObjectEvents[task->data[7]];
...
u8 GetChosenApproachingTrainerObjectEventId(u8 arrayId)
{
    if (arrayId >= (size_t)(sizeof(gApproachingTrainers) / sizeof((gApproachingTrainers)[0])))
        return 0;
    else if (arrayId == 0)
        return gApproachingTrainers[0].objectEventId;
    else
...
void PlayerFaceTrainerAfterBattle(void)
{
    ...
    if (gTrainerApproachedPlayer == 1)
    {
        ...
        ScriptMovement_StartObjectMovementScript(0xFF, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup, gPostBattleMovementScript);
    }
    else
    {
