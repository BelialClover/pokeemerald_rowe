...
__attribute__((section("ewram_data"))) static u16 gBackupMapData[0x2800] = {0};
__attribute__((section("ewram_data"))) struct MapHeader gMapHeader = {0};
__attribute__((section("ewram_data"))) struct Camera gCamera = {0};
__attribute__((section("ewram_data"))) static struct ConnectionFlags gMapConnectionFlags = {0};
__attribute__((section("ewram_data"))) static u32 sFiller_02037344 = 0;
...
void InitBattlePyramidMap(bool8 setPlayerPosition)
{
    { vu32 tmp = (vu32)(0x03ff03ff); CpuFastSet((void *)&tmp, gBackupMapData, 0x01000000 | ((sizeof(gBackupMapData))/(32/8) & 0x1FFFFF)); };
...
static void InitMapLayoutData(struct MapHeader *mapHeader)
{
    ...
    { vu32 tmp = (vu32)(((0x03ff) << 16) | (0x03ff)); CpuFastSet((void *)&tmp, (gBackupMapData), 0x01000000 | (((sizeof(gBackupMapData)))/(32/8) & 0x1FFFFF)); };
...
u32 MapGridGetMetatileIdAt(int x, int y)
{
    ...
    else
    {
        ...
        i = (x + 1) & 1;
        i += ((y + 1) & 1) * 2;
        block = mapLayout->border[i] | 0x0C00;
    }
    if (block == 0x03FF)
    {
        border = gMapHeader.mapLayout->border;
...
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, gSaveBlock1Ptr->mapView, 0x00000000 | 0x01000000 | ((sizeof(gSaveBlock1Ptr->mapView))/(16/8) & 0x1FFFFF)); };
}
...
int GetMapBorderIdAt(int x, int y)
{
    ...
    else
    {
        ...
        j = (x + 1) & 1;
        j += ((y + 1) & 1) * 2;
        block2 = 0x0C00 | mapLayout->border[j];
        if (block2 == 0x03FF)
        {
            goto fail;
        }
...
struct MapConnection *sub_8088950(u8 direction, int x, int y)
{
    ...
    for (i = 0; i < count; i++, connection++)
    {
        if (connection->direction == direction && sub_80889A8(direction, x, y, connection) == 1)
            return connection;
    }
    return ((void *)0);
}
...
struct MapConnection *GetConnectionAtCoords(s16 x, s16 y)
{
    ...
    else
    {
        ...
        for (i = 0; i < count; i++, connection++)
        {
            ...
            if (sub_8088A4C(connection, x - 7, y - 7) == 1)
            {
                return connection;
            }
        }
...
static bool8 SkipCopyingMetatileFromSavedMap(u16* mapMetatilePtr, u16 mapWidth, u8 yMode)
{
    ...
    if (IsLargeBreakableDecoration(*mapMetatilePtr & 0x03FF, yMode) == 1)
        return 1;
    return 0;
}
...
void LoadTilesetPalette(struct Tileset const *tileset, u16 destOffset, u16 size)
{
    ...
    if (tileset)
    {
        ...
        else if (tileset->isSecondary == 1)
        {
            ...
            LoadPaletteDayNight(((u16*)tileset->palettes) + (6 * 16), destOffset, size);
...
{
    LoadTilesetPalette(mapLayout->primaryTileset, 0, 6 * 16 * 2);
}

void LoadSecondaryTilesetPalette(struct MapLayout const *mapLayout)
{
    LoadTilesetPalette(mapLayout->secondaryTileset, 6 * 16, (13 - 6) * 16 * 2);
}
