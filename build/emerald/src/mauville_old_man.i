# 0 "src/mauville_old_man.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/mauville_old_man.c"
...
# 3 "src/mauville_old_man.c" 2
# 1 "include/constants/songs.h" 1
# 4 "src/mauville_old_man.c" 2
# 1 "include/constants/easy_chat.h" 1
# 5 "src/mauville_old_man.c" 2
# 1 "include/constants/event_objects.h" 1
# 6 "src/mauville_old_man.c" 2
...
# 24 "src/mauville_old_man.c" 2
# 1 "include/constants/mauville_old_man.h" 1
# 25 "src/mauville_old_man.c" 2
...
static __attribute__((section("ewram_data"))) u16 sUnknownBardRelated = 0;
static __attribute__((section("ewram_data"))) struct MauvilleManStoryteller * sStorytellerPtr = ((void *)0);
static __attribute__((section("ewram_data"))) u8 sStorytellerWindowId = 0;
...
{
    u16 *scriptResult = &gSpecialVar_Result;
    struct MauvilleManBard *bard = &gSaveBlock1Ptr->oldMan.bard;
...
static void BardSing(struct Task *task, struct BardSong *song)
{
    switch (task->data[0])
    {
        ...
        case 2:
        {
            ...
            GetWordPhonemes(song, (((word) & 3) + (((word) / 8) & 1)));
            song->currWord++;
...
static void Task_BardSong(u8 taskId)
{
    ...
    switch (task->data[0])
    {
        ...
        case 2:
        {
            ...
            if (!task->data[5])
                sUnknownBardRelated = (((bard->songLyrics[task->data[4]]) % 4) + (((bard->songLyrics[task->data[4]]) / 8) & 1));
            else
                sUnknownBardRelated = (((bard->temporaryLyrics[task->data[4]]) % 4) + (((bard->temporaryLyrics[task->data[4]]) / 8) & 1));
...
{
    bool32 isRuby = (version == 1 || version == 2);
...
static void StorytellerSetup(void)
{
    ...
    sStorytellerPtr->alreadyRecorded = 0;
    for (i = 0; i < 4; i++)
    {
...
static const struct Story *GetStoryByStat(u32 stat)
{
    ...
    for (i = 0; i < (int)(size_t)(sizeof(sStorytellerStories) / sizeof((sStorytellerStories)[0])); i++)
    {
        if (sStorytellerStories[i].stat == stat)
            return &sStorytellerStories[i];
    }
    return &sStorytellerStories[(size_t)(sizeof(sStorytellerStories) / sizeof((sStorytellerStories)[0])) - 1];
}
...
static const struct UnknownStruct_0859F288 sStorytellerStuff = {
    (size_t)(sizeof(sStorytellerStories) / sizeof((sStorytellerStories)[0])),
...
static bool8 StorytellerInitializeRandomStat(void)
{
    ...
    ScrambleStatList(arr, (size_t)(sizeof(sStorytellerStories) / sizeof((sStorytellerStories)[0])));
    for (i = 0; i < (s32)(size_t)(sizeof(sStorytellerStories) / sizeof((sStorytellerStories)[0])); i++)
    {
        ...
        {
            sStorytellerPtr->alreadyRecorded = 1;
...
static void PrintStoryList(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (gameStatID == 0)
            break;
        AddTextPrinterParameterized(sStorytellerWindowId, 1, GetStoryTitleByStat(gameStatID), 8, 16 * i + 1, 0xFF, ((void *)0));
    }
    AddTextPrinterParameterized(sStorytellerWindowId, 1, gText_Exit, 8, 16 * i + 1, 0xFF, ((void *)0));
...
bool8 ScrSpecial_StorytellerUpdateStat(void)
{
    ...
    if (HasTrainerStatIncreased(sSelectedStory) == 1)
    {
...
bool8 ScrSpecial_HasStorytellerAlreadyRecorded(void)
{
    ...
    if (sStorytellerPtr->alreadyRecorded == 0)
        return 0;
    else
        return 1;
}
