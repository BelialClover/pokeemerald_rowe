...
__attribute__((section("ewram_data"))) struct MartInfo gMartInfo = {0};
__attribute__((section("ewram_data"))) struct ShopData *gShopDataPtr = ((void *)0);
__attribute__((section("ewram_data"))) struct ListMenuItem *gUnknown_02039F74 = ((void *)0);
__attribute__((section("ewram_data"))) u8 (*gUnknown_02039F78)[16] = {0};
__attribute__((section("ewram_data"))) u8 gMartPurchaseHistoryId = 0;
__attribute__((section("ewram_data"))) struct ItemSlot gMartPurchaseHistory[3] = {0};
...
static u8 CreateShopMenu(u8 martType)
{
    ...
    if (martType == MART_TYPE_NORMAL)
    {
        ...
        winTemplate.width = GetMaxWidthInMenuTable(sShopMenuActions_BuySellQuit, (size_t)(sizeof(sShopMenuActions_BuySellQuit) / sizeof((sShopMenuActions_BuySellQuit)[0])));
        gMartInfo.windowId = AddWindow(&winTemplate);
        gMartInfo.menuActions = sShopMenuActions_BuySellQuit;
        numMenuItems = (size_t)(sizeof(sShopMenuActions_BuySellQuit) / sizeof((sShopMenuActions_BuySellQuit)[0]));
    }
    else
    {
        struct WindowTemplate winTemplate;
        winTemplate = sShopMenuWindowTemplates[1];
        winTemplate.width = GetMaxWidthInMenuTable(sShopMenuActions_BuyQuit, (size_t)(sizeof(sShopMenuActions_BuyQuit) / sizeof((sShopMenuActions_BuyQuit)[0])));
        gMartInfo.windowId = AddWindow(&winTemplate);
        gMartInfo.menuActions = sShopMenuActions_BuyQuit;
        numMenuItems = (size_t)(sizeof(sShopMenuActions_BuyQuit) / sizeof((sShopMenuActions_BuyQuit)[0]));
    }
...
static u8 GetNumberOfBadges(void)
{
    ...
    for (badgeFlag = (((0x500 + 864 - 1) + 1) + 0x7); badgeFlag < (((0x500 + 864 - 1) + 1) + 0x7) + (1 + (((0x500 + 864 - 1) + 1) + 0xE) - (((0x500 + 864 - 1) + 1) + 0x7)); badgeFlag++)
    {
...
static void BuyMenuPrintPriceInList(u8 windowId, s32 item, u8 y)
{
    ...
    if (item != -2)
    {
        ...
        if (ItemId_GetPocket(item) == 9 && (CheckBagHasItem(item, 1) || CheckPCHasItem(item, 1)))
...
static void BuyMenuInitBgs(void)
{
    ...
    InitBgsFromTemplates(0, sShopBuyMenuBgTemplates, (size_t)(sizeof(sShopBuyMenuBgTemplates) / sizeof((sShopBuyMenuBgTemplates)[0])));
    ...
    SetGpuReg(0x1e, 0);
    SetGpuReg(0x50, 0);
    SetGpuReg(0x0, 0x0000 | 0x1000 | 0x0040);
    ShowBg(0);
    ShowBg(1);
    ShowBg(2);
    ShowBg(3);
...
static void BuyMenuDrawMapBg(void)
{
    ...
    for (j = 0; j < 10; j++)
    {
        for (i = 0; i < 15; i++)
        {
            ...
            if (BuyMenuCheckForOverlapWithMenuBg(i, j) == 1)
            ...
            {
                BuyMenuDrawMapMetatile(i, j, (u16*)mapLayout->secondaryTileset->metatiles + ((metatile - 512) * 8), metatileLayerType);
            }
        }
    }
}
...
static void BuyMenuDrawObjectEvents(void)
{
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        if (BuyMenuCheckIfObjectEventOverlapsMenuBg(gShopDataPtr->viewportObjects[i]) == 1)
        {
...
{
    if (!BuyMenuCheckForOverlapWithMenuBg(object[X_COORD], object[Y_COORD] + 2) && object[LAYER_TYPE] != 0x01)
    {
        return 1;
    }
    else
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        s32 itemId = ListMenu_ProcessInput(data[7]);
        ListMenuGetScrollAndRow(data[7], &gShopDataPtr->scrollOffset, &gShopDataPtr->selectedRow);
        ...
        default:
            ...
            if (ItemId_GetPocket(itemId) == 9 && (CheckBagHasItem(itemId, 1) || CheckPCHasItem(itemId, 1)))
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    u16 quantityInBag = CountTotalItemQuantityInBag(data[5]);
    u16 maxQuantity;

    DrawStdFrameWithCustomTileAndPalette(3, 0, 1, 13);
    ...
    data[1] = 1;
    DrawStdFrameWithCustomTileAndPalette(4, 0, 1, 13);
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    if (AdjustQuantityAccordingToDPadInput(&data[1], gShopDataPtr->maxQuantity) == 1)
    {
        gShopDataPtr->totalCost = (ItemId_GetPrice(data[5]) >> GetPriceReduction(1)) * data[1];
        BuyMenuPrintItemQuantityAndPrice(taskId);
    }
    else
    {
        if (({(gMain.newKeys) & (0x0001);}))
        {
            ...
            ConvertIntToDecimalStringN(gStringVar2, data[1], STR_CONV_MODE_LEFT_ALIGN, 2);
...
static void BuyMenuTryMakePurchase(u8 taskId)
{
    ...
    if (gMartInfo.martType == MART_TYPE_NORMAL)
    {
        if (AddBagItem(data[5], data[1]) == 1)
        {
            GetSetItemObtained(data[5], FLAG_SET_OBTAINED);
            RedrawListMenu(data[7]);
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId)
{
    ...
    {
        PlaySE(5);
        if ((ItemId_GetPocket(data[5]) == 3) && data[1] > 9 && AddBagItem(12, data[1] / 10) == 1)
        {
            if (data[1] > 19)
            {
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    FillWindowPixelBuffer(4, ((1) | ((1) << 4)));
    PrintMoneyAmount(4, 32, 1, gShopDataPtr->totalCost, 0xFF);
    ConvertIntToDecimalStringN(gStringVar1, data[1], STR_CONV_MODE_LEADING_ZEROS, 2);
...
static void RecordItemPurchase(u8 taskId)
{
    ...
    for (i = 0; i < 3; i++)
    {
        if (gMartPurchaseHistory[i].itemId == data[5] && gMartPurchaseHistory[i].quantity != 0)
        {
            if (gMartPurchaseHistory[i].quantity + data[1] > 255)
            {
                gMartPurchaseHistory[i].quantity = 255;
            }
            else
            {
                gMartPurchaseHistory[i].quantity += data[1];
            }
            return;
        }
    }
    ...
    {
        gMartPurchaseHistory[gMartPurchaseHistoryId].itemId = data[5];
        gMartPurchaseHistory[gMartPurchaseHistoryId].quantity = data[1];
