# 0 "src/dewford_trend.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/dewford_trend.c"
...
# 11 "src/dewford_trend.c" 2
# 1 "include/constants/easy_chat.h" 1
# 12 "src/dewford_trend.c" 2
...
void InitDewfordTrend(void)
{
    ...
    for (i = 0; i < 5; i++)
    {
        gSaveBlock1Ptr->easyChatPairs[i].words[0] = GetRandomEasyChatWordFromGroup(0xa);

        if (Random() & 1)
            gSaveBlock1Ptr->easyChatPairs[i].words[1] = GetRandomEasyChatWordFromGroup(0xc);
        else
            gSaveBlock1Ptr->easyChatPairs[i].words[1] = GetRandomEasyChatWordFromGroup(0xd);
        ...
        else
        {
            ...
            for (i = 0; i < players; i++)
            {
                for (j = 0; j < 5; j++)
                {
                    ...
                    {
                        foo_of_buffer2 = (struct EasyChatPair *)((u32)buffer2 + (foo * 8));
