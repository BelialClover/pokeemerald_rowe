...
# 2 "src/option_menu.c" 2
# 1 "include/option_menu.h" 1



void CB2_InitOptionMenu(void);
# 3 "src/option_menu.c" 2
# 1 "include/main.h" 1
...
# 9 "src/option_menu.c" 2
# 1 "include/task.h" 1
# 10 "src/option_menu.c" 2
# 1 "gflib/malloc.h" 1
# 15 "gflib/malloc.h"
extern u8 gHeap[];
...
# 19 "src/option_menu.c" 2
# 1 "include/constants/rgb.h" 1
# 20 "src/option_menu.c" 2
...
} static const sItemFunctions[MENUITEM_COUNT] =
{
 ...
 [MENUITEM_MUSIC] = {Music_DrawChoices, Music_ProcessInput},
    ...
    [MENUITEM_CANCEL] = {((void *)0), ((void *)0)},
};


__attribute__((section("ewram_data"))) static struct OptionMenu *sOptions = ((void *)0);
...
{
    if (sItemFunctions[id].drawChoices != ((void *)0))
...
void CB2_InitOptionMenu(void)
{
    ...
    case 1:
        ...
        InitBgsFromTemplates(0, sOptionMenuBgTemplates, (size_t)(sizeof(sOptionMenuBgTemplates) / sizeof((sOptionMenuBgTemplates)[0])));
        ResetBgPositions();
...
static void ScrollMenu(int direction)
{
    ...
    ScrollWindow(WIN_OPTIONS, direction, 16, ((0) | ((0) << 4)));

    FillWindowPixelRect(WIN_OPTIONS, ((1) | ((1) << 4)), 0, 16 * pos, 26 * 8, 16);

    DrawChoices(menuItem, pos * 16, 0xFF);
    AddTextPrinterParameterized(WIN_OPTIONS, 1, sOptionMenuItemsNames[menuItem], 8, (pos * 16) + 1, 0xFF, ((void *)0));
    CopyWindowToVram(WIN_OPTIONS, 2);
}

static void ScrollAll(int direction)
{
    int i, y, menuItem, pos;
    int scrollCount = MENUITEM_COUNT - 7;

    ScrollWindow(WIN_OPTIONS, direction, 16 * scrollCount, ((0) | ((0) << 4)));
    ...
    FillWindowPixelRect(WIN_OPTIONS, ((1) | ((1) << 4)), 0, y, 26 * 8, 16 * scrollCount);
...
static void Task_OptionMenuProcessInput(u8 taskId)
{
    ...
    else if (({(gMain.newKeys) & (0x0080);}))
    {
        ...
        {
            if (++sOptions->menuCursor >= MENUITEM_COUNT)
            {
...
static void HighlightOptionMenuItem(int cursor)
{
    SetGpuReg(0x40, (((16) << 8) | (224)));
    SetGpuReg(0x44, (((cursor * 16 + 40) << 8) | (cursor * 16 + 56)));
}
...
static void DrawOptionMenuChoice(const u8 *text, u8 x, u8 y, u8 style, u8 textSpeed)
{
    ...
    dst[i] = 0xFF;
    AddTextPrinterParameterized(WIN_OPTIONS, 1, dst, x, y + 1, textSpeed, ((void *)0));
}
...
static void HpBar_DrawChoices(int selection, int y, u8 textSpeed)
{
    if (selection < 10)
    {
        u8 textPlus[] = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}+1{0x77}{0x77}{0x77}{0x77}{0x77}");
        textPlus[7] = 0xA1 + selection;
...
static void FourOptions_DrawChoices(const u8 *const *const strings, int selection, int y, u8 textSpeed)
{
    ...
    FillWindowPixelRect(WIN_OPTIONS, ((1) | ((1) << 4)), 104, y, 26 * 8 - 104, 16);
    CopyWindowToVram(WIN_OPTIONS, 2);
...
static void DrawTextOption(void)
{
    FillWindowPixelBuffer(WIN_TEXT_OPTION, ((1) | ((1) << 4)));
    AddTextPrinterParameterized(WIN_TEXT_OPTION, 1, gText_Option, 8, 1, 0, ((void *)0));
...
static void DrawOptionMenuTexts(void)
{
    ...
    FillWindowPixelBuffer(WIN_OPTIONS, ((1) | ((1) << 4)));
    for (i = 0; i < 7; i++)
        AddTextPrinterParameterized(WIN_OPTIONS, 1, sOptionMenuItemsNames[i], 8, (i * 16) + 1, 0, ((void *)0));
