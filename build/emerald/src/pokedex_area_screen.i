# 0 "src/pokedex_area_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokedex_area_screen.c"
...
# 10 "src/pokedex_area_screen.c" 2
...
struct PokeDexAreaScreen
{
              ...
              u16 state;
              u16 species;
              struct PokeDexAreaScreenMapIdentity overworldAreasWithMons[0x40];
...
static __attribute__((section("ewram_data"))) struct PokeDexAreaScreen *sPokedexAreaScreen = ((void *)0);
...
static const u16 sSpeciesHiddenFromAreaScreen[] = { 360 };
...
{
    {349, ((34 | (0 << 8)) >> 8), ((34 | (0 << 8)) & 0xFF)},
    {898 + 308 + 1}
};

static const u16 sLandmarkData[][2] =
{
    {0x55, (((0x500 + 864 - 1) + 1) + 0x5E)},
    {0x44, (((0x500 + 864 - 1) + 1) + 0x47)},
    {0xD2, (((0x500 + 864 - 1) + 1) + 0x7D)},
    {0xC7, 0x78},
    {0xD1, (((0x500 + 864 - 1) + 1) + 0x7E)},
...
static bool8 DrawAreaGlow(void)
{
    ...
    case 3:
        ...
        {
            CpuSet(sAreaGlow_Pal, gPlttBufferUnfaded + 0xA0, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
...
static void FindMapsWithMon(u16 species)
{
    ...
    if (species != roamer->species)
    {
        ...
        for (i = 0; i < (size_t)(sizeof(sSpeciesHiddenFromAreaScreen) / sizeof((sSpeciesHiddenFromAreaScreen)[0])); i++)
        {
            if (sSpeciesHiddenFromAreaScreen[i] == species)
                return;
        }

        for (i = 0; sFeebasData[i][0] != 898 + 308 + 1; i++)
        {
...
static void SetSpecialMapHasMon(u16 mapGroup, u16 mapNum)
{
    ...
    if (sPokedexAreaScreen->numSpecialAreas < 0x20)
    {
        ...
        if (regionMapSectionId < 0xD5)
        {
            for (i = 0; i < (size_t)(sizeof(sMovingRegionMapSections) / sizeof((sMovingRegionMapSections)[0])); i++)
            {
                if (regionMapSectionId == sMovingRegionMapSections[i])
                    return;
            }

            for (i = 0; sLandmarkData[i][0] != 0xD5; i++)
            {
...
{
    if (GetRegionMapSectionId(info->mapGroup, info->mapNum) == 0xD2)
    {
...
static void BuildAreaGlowTilemap(void)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sPokedexAreaScreen->areaGlowTilemap) / sizeof((sPokedexAreaScreen->areaGlowTilemap)[0])); i++)
...
static void Task_ShowPokedexAreaScreen(u8 taskId)
{
    switch (gTasks[taskId].data[0])
    {
        ...
        case 8:
            ...
            break;
        case 9:
            BeginNormalPaletteFade(0xFFFFFFEB, 0, 16, 0, ((0) | ((0) << 5) | ((0) << 10)));
            break;
        case 10:
            SetGpuReg(0x50, (1 << 0) | (1 << 6) | (1 << 8) | (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13)));
            StartAreaGlow();
            ShowBg(2);
            ShowBg(3);
            SetGpuRegBits(0x0, 0x1000);
            break;
        case 11:
...
static void Task_HandlePokedexAreaScreenInput(u8 taskId)
{
    ...
    case 3:
        ...
        { Free(sPokedexAreaScreen); sPokedexAreaScreen = ((void *)0); };
        return;
    }
...
static void CreateAreaUnknownSprites(void)
{
    ...
    {
        for (i = 0; i < 3; i++)
            sPokedexAreaScreen->areaUnknownSprites[i] = ((void *)0);
    }
    else
    {
        for (i = 0; i < 3; i++)
