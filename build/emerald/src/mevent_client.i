# 0 "src/mevent_client.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/mevent_client.c"
...
# 10 "src/mevent_client.c" 2

__attribute__((section("ewram_data"))) struct mevent_client * s_mevent_client_ptr = ((void *)0);
...
static void mevent_client_init(struct mevent_client * svr, u32 sendPlayerNo, u32 recvPlayerNo)
{
    ...
    svr->mainseqno = 0;
    svr->flag = 0;
    svr->sendBuffer = AllocZeroed(0x400);
    svr->recvBuffer = AllocZeroed(0x400);
    svr->cmdBuffer = AllocZeroed(0x400);
...
{
    memcpy(svr->cmdBuffer, svr->recvBuffer, 0x400);
    svr->cmdidx = 0;
}

static void mevent_client_send_word(struct mevent_client * svr, u32 ident, u32 word)
{
    { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, svr->sendBuffer, 0x04000000 | 0x01000000 | ((0x400)/(32/8) & 0x1FFFFF)); };
...
static u32 mainseq_0(struct mevent_client * svr)
{
    ...
    memcpy(svr->cmdBuffer, gUnknown_082F2598, 0x400);
    svr->cmdidx = 0;
    svr->mainseqno = 4;
    svr->flag = 0;
    return 0;
...
static u32 mainseq_4(struct mevent_client * svr)
{
    ...
    case 18:
        ...
        break;
    case 21:
        memcpy(gDecompressionBuffer, svr->recvBuffer, 0x400);
        svr->mainseqno = 7;
        svr->flag = 0;
        break;
    }
