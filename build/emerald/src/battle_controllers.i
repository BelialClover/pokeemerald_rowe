# 0 "src/battle_controllers.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_controllers.c"
...
# 6 "src/battle_controllers.c" 2
# 1 "include/battle_message.h" 1
# 7 "src/battle_controllers.c" 2
...
# 12 "src/battle_controllers.c" 2
# 1 "include/pokemon.h" 1
# 13 "src/battle_controllers.c" 2
...
__attribute__((section("ewram_data"))) struct UnusedControllerStruct gUnknown_02022D0C = {};
static __attribute__((section("ewram_data"))) u8 sBattleBuffersTransferData[0x100] = {};
...
void SetUpBattleVarsAndBirchZigzagoon(void)
{
 ...
 BattleAI_SetupAIData(0xF);
    ...
    {
        ZeroEnemyPartyMons();
        CreateMon(&gEnemyParty[0], 263, 2, 32, 0, 0, 0, 0, 0);
        i = 0;
    ...
    else
    {
        ...
        if (gBattleTypeFlags & (1 << 24))
        {
            ...
            else if (gBattleTypeFlags & (1 << 6))
            {
                ...
                for (var = gUnknown_0203C7B4, i = 0; i < 4; i++)
                {
    ...
    if (!(gBattleTypeFlags & (1 << 6)))
    {
        for (i = 0; i < gBattlersCount; i++)
        {
            for (j = 0; j < 6; j++)
            {
                if (i < 2)
                {
                    if (((gBattlerPositions[i]) & 1) == 0)
                    {
                        if (GetMonData(&gPlayerParty[j], 57) != 0
                         && GetMonData(&gPlayerParty[j], 65) != 0
                         && GetMonData(&gPlayerParty[j], 65) != 898 + 308 + 1
                    ...
                    else
                    {
                        if (GetMonData(&gEnemyParty[j], 57) != 0
                         ...
                         && GetMonData(&gEnemyParty[j], 65) != 898 + 308 + 1
...
void PrepareBufferDataTransferLink(u8 bufferId, u16 size, u8 *data)
{
    ...
    if (gTasks[sLinkSendTaskId].data[14] + alignedSize + LINK_BUFF_DATA + 1 > 0x1000)
    {
...
void sub_8033648(void)
{
    ...
    if (gReceivedRemoteLinkPlayers != 0 && (gBattleTypeFlags & (1 << 5)))
    {
        DestroyTask_RfuIdle();
