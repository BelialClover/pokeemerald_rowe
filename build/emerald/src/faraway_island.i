# 0 "src/faraway_island.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/faraway_island.c"
...
# 11 "src/faraway_island.c" 2
# 1 "include/constants/metatile_behaviors.h" 1
# 12 "src/faraway_island.c" 2
...
static __attribute__((section("ewram_data"))) u8 sGrassSpriteId = 0;
...
u32 GetMewMoveDirection(void)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sMewDirectionCandidates) / sizeof((sMewDirectionCandidates)[0])); i++)
    ...
    for (i = 0; i < (size_t)(sizeof(sFarawayIslandRockCoords) / sizeof((sFarawayIslandRockCoords)[0])); i++)
    {
...
static u8 GetValidMewMoveDirection(u8 ignoredDir)
{
    ...
    if (CanMewMoveToCoords(mew->currentCoords.x, mew->currentCoords.y - 1) == 1 && ignoredDir != 2)
    {
        sMewDirectionCandidates[count] = 2;
        count++;
    }

    if (CanMewMoveToCoords(mew->currentCoords.x + 1, mew->currentCoords.y) == 1 && ignoredDir != 4)
    {
        sMewDirectionCandidates[count] = 4;
        count++;
    }

    if (CanMewMoveToCoords(mew->currentCoords.x, mew->currentCoords.y + 1) == 1 && ignoredDir != 1)
    {
        sMewDirectionCandidates[count] = 1;
        count++;
    }

    if (CanMewMoveToCoords(mew->currentCoords.x - 1, mew->currentCoords.y) == 1 && ignoredDir != 3)
    {
...
{
    u16 steps = VarGet(0x403A);
    if (gSaveBlock1Ptr->location.mapNum == ((57 | (26 << 8)) & 0xFF)
     && gSaveBlock1Ptr->location.mapGroup == ((57 | (26 << 8)) >> 8))
    {
        steps++;
        if (steps >= 9999)
            VarSet(0x403A, 0);
...
bool8 IsMewPlayingHideAndSeek(void)
{
    if (gSaveBlock1Ptr->location.mapNum == ((57 | (26 << 8)) & 0xFF)
     && gSaveBlock1Ptr->location.mapGroup == ((57 | (26 << 8)) >> 8))
    {
        if (FlagGet(0x1CA) != 1 && FlagGet(0x2CE) != 1)
            return 1;
    }
