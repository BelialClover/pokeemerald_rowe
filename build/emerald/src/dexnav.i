...
__attribute__((section("ewram_data"))) static struct DexNavSearch *sDexNavSearchDataPtr = ((void *)0);
__attribute__((section("ewram_data"))) static struct DexNavGUI *sDexNavUiDataPtr = ((void *)0);
__attribute__((section("ewram_data"))) static u8 *sBg1TilemapBuffer = ((void *)0);
__attribute__((section("ewram_data"))) bool8 gDexnavBattle = 0;
...
static const u8 sFontColor_Black[3] = {0x0, 0x2, 0x3};
static const u8 sFontColor_White[3] = {0x0, 0x1, 0x2};
...
static const struct CompressedSpriteSheet sNoDataIconSpriteSheet = {sNoDataGfx, (32 * 32) / 2, 55130};
static const struct CompressedSpriteSheet sCapturedAllPokemonSpriteSheet = {sCapturedAllMonsTiles, (8 * 8) / 2, 0x4002};

static const struct CompressedSpriteSheet sPotentialStarSpriteSheet = {sPotentialStarGfx, (8 * 8) / 2, 0x4010};

static const struct CompressedSpriteSheet sOwnedIconSpriteSheet = {sOwnedIconGfx, (8 * 8) / 2, 0x4003};
static const struct CompressedSpriteSheet sHiddenMonIconSpriteSheet = {sHiddenMonIconGfx, (8 * 8) / 2, 0x4006};
...
static void DrawDexNavSearchMonIcon(u16 species, u8 *dst, bool8 owned)
{
 ...
 u16 formId = GetFormIdFromFormSpeciesId(species);
    ...
    *dst = spriteId;
    ...
    if (owned)
        sDexNavSearchDataPtr->ownedIconSpriteId = CreateSprite(&sOwnedIconTemplate, 28 + 6, GetSearchWindowY() + 4, 0);
}
...
static void AddSearchWindow(u8 width)
{
    ...
    FillWindowPixelBuffer(sDexNavSearchDataPtr->windowId, ((1) | ((1) << 4)));
    PutWindowTilemap(sDexNavSearchDataPtr->windowId);
    CopyWindowToVram(sDexNavSearchDataPtr->windowId, 3);

    DrawStdFrameWithCustomTileAndPalette(sDexNavSearchDataPtr->windowId, 1, 0x214, 14);
}
...
static void AddSearchWindowText(u16 species, u8 proximity, u8 searchLevel, bool8 hidden)
{
    ...
    if (hidden)
    {
        ...
        AddTextPrinterParameterized3(sDexNavSearchDataPtr->windowId, 0, (28 + 4), 0, sSearchFontColor, 0xFF, gStringVar4);
        return;
    }
    else
    {
        StringCopy(gStringVar1, gSpeciesNames[species]);
        AddTextPrinterParameterized3(sDexNavSearchDataPtr->windowId, 0, (28 + 4), 0, sSearchFontColor, 0xFF, gStringVar1);
    }
    ...
    if (proximity <= 4)
    {
        ...
        if (searchLevel > 1 && sDexNavSearchDataPtr->moves[0])
        {
            ...
            AddTextPrinterParameterized3(windowId, 0, (((28 + 4) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (10))) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6)), 0, sSearchFontColor, 0xFF, gStringVar4);
        }
        ...
        if (searchLevel > 2)
        {
            ...
            if (sDexNavSearchDataPtr->heldItem)
            {
                ...
                AddTextPrinterParameterized3(windowId, 0, (28 + 4), 12, sSearchFontColor, 0xFF, gStringVar4);
            }
        }
    }
    ...
    AddTextPrinterParameterized3(windowId, 0, ((((28 + 4) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (10))) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6)) + 90) - 16, 12, sSearchFontColor, 0xFF, gStringVar4);
...
static void RemoveDexNavWindowAndGfx(void)
{
    ...
    if (sDexNavSearchDataPtr->iconSpriteId != 64)
    ...
    if (sDexNavSearchDataPtr->eyeSpriteId != 64)
        DestroySprite(&gSprites[sDexNavSearchDataPtr->eyeSpriteId]);
    if (sDexNavSearchDataPtr->ownedIconSpriteId != 64)
        DestroySprite(&gSprites[sDexNavSearchDataPtr->ownedIconSpriteId]);
    if (sDexNavSearchDataPtr->exclamationSpriteId != 64)
        DestroySprite(&gSprites[sDexNavSearchDataPtr->exclamationSpriteId]);

    for (i = 0; i < (sizeof(sDexNavSearchDataPtr->starSpriteIds)/sizeof(*(sDexNavSearchDataPtr->starSpriteIds))); i++)
    {
        if (sDexNavSearchDataPtr->starSpriteIds[i] != 64)
            DestroySprite(&gSprites[sDexNavSearchDataPtr->starSpriteIds[i]]);
    }

    FreeSpriteTilesByTag(0xd750);
    FreeSpriteTilesByTag(0x4003);
    FreeSpriteTilesByTag(0x4005);
    FreeSpriteTilesByTag(0x4006);
    FreeSpriteTilesByTag(0x4010);
    FreeSpritePaletteByTag(0xd750);
...
static bool8 DexNavPickTile(u8 environment, u8 areaX, u8 areaY, bool8 smallScan)
{
    ...
    s16 topX = gSaveBlock1Ptr->pos.x - 0 + (smallScan * 5);
    s16 topY = gSaveBlock1Ptr->pos.y - 0 + (smallScan * 5);
    s16 botX = topX + areaX;
    s16 botY = topY + areaY;
    u8 i;
    bool8 nextIter;
    ...
    while (topY < botY)
    {
        ...
        topY++;
        topX = gSaveBlock1Ptr->pos.x - 0 + (smallScan * 5);
    }
...
static bool8 TryStartHiddenMonFieldEffect(u8 environment, u8 xSize, u8 ySize, bool8 smallScan)
{
    ...
    if (DexNavPickTile(environment, xSize, ySize, smallScan))
    {
        ...
        case 0:
            ...
            else if (IsMapTypeIndoors(currMapType))
            {
                if (MetatileBehavior_IsTallGrass(metatileBehaviour))
                    fldEffId = 19;
                else if (MetatileBehavior_IsLongGrass(metatileBehaviour))
                    fldEffId = 20;
            ...
            else
            {
                if (MetatileBehavior_IsTallGrass(metatileBehaviour))
                    ...
                    fldEffId = 21;
                else if (MetatileBehavior_IsMountain(metatileBehaviour))
                    fldEffId = 67;
                else
                    fldEffId = 23;
            }
        ...
        if (fldEffId != 0)
        {
            ...
            if (sDexNavSearchDataPtr->fldEffSpriteId == 64)
                return 0;
...
{
    *dst = CreateSprite(&sHeldItemTemplate, 28 + 6, GetSearchWindowY() + 18, 0);
    if (*dst != 64)
        gSprites[*dst].invisible = 1;
}
...
static void Task_SetUpDexNavSearch(u8 taskId)
{
    ...
    sDexNavSearchDataPtr->itemSpriteId = 64;
    sDexNavSearchDataPtr->eyeSpriteId = 64;
    sDexNavSearchDataPtr->starSpriteIds[0] = 64;
    sDexNavSearchDataPtr->starSpriteIds[1] = 64;
    sDexNavSearchDataPtr->starSpriteIds[2] = 64;
    sDexNavSearchDataPtr->ownedIconSpriteId = 64;
    sDexNavSearchDataPtr->exclamationSpriteId = 64;
    ...
    FlagSet((((0x500 + 864 - 1) + 1) + 0x22));
    gPlayerAvatar.creeping = 1;
...
static void Task_InitDexNavSearch(u8 taskId)
{
    struct Task *task = &gTasks[taskId];
    u8 searchLevel;
    u16 species = task->data[2];
    u8 environment = task->data[3];
    ...
    sDexNavSearchDataPtr->isHiddenMon = (environment == 2) ? 1 : 0;
    ...
    if (sDexNavSearchDataPtr->monLevel == 255 || !TryStartHiddenMonFieldEffect(sDexNavSearchDataPtr->environment, 12, 12, 0))
    {
        Free(sDexNavSearchDataPtr);
        FreeMonIconPalettes();
    ...
    sDexNavSearchDataPtr->hiddenSearch = 0;
    task->data[4] = 1;
...
static void DexNavDrawPotentialStars(u8 potential, u8* dst)
{
    ...
    for (i = 0; i < (sizeof(sDexNavSearchDataPtr->starSpriteIds)/sizeof(*(sDexNavSearchDataPtr->starSpriteIds))); i++)
    {
        spriteId = 64;
        if (potential > i)
            spriteId = CreateSprite(&sPotentialStarTemplate, 28 - 20, GetSearchWindowY() + 4 + (i * 8), 0);
...
static void DexNavUpdateDirectionArrow(void)
{
    ...
    FillWindowPixelRect(windowId, ((1) | ((1) << 4)), ((((28 + 4) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (10))) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6)) + 90), 0, 12, 12);
    ...
    AddTextPrinterParameterized3(windowId, 1, ((((28 + 4) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * (10))) + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH) * 6)) + 90), 0, sSearchFontColor, 0xFF, str);
...
static void DexNavDrawIcons(void)
{
    ...
    DrawSearchWindow(species, sDexNavSearchDataPtr->potential, 0);
...
{
    u8 taskId;
    u16 val = VarGet(0x40F7);

    if (FlagGet((((0x500 + 864 - 1) + 1) + 0x22)) || (val & 0x3FFF) == 0)
        return 0;
...
static void Task_RevealHiddenMon(u8 taskId)
{
    ...
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_SEEN))
    {
        ...
        DrawSearchWindow(species, sDexNavSearchDataPtr->potential, 1);
        DrawDexNavSearchMonIcon(species, &sDexNavSearchDataPtr->iconSpriteId, 0);

        index = IndexOfSpritePaletteTag(gSprites[sDexNavSearchDataPtr->iconSpriteId].template->paletteTag);
        CpuSet(&gPlttBufferUnfaded[0x100 + index * 16], sDexNavSearchDataPtr->palBuffer, 0x00000000 | ((32)/(16/8) & 0x1FFFFF));
...
static void Task_DexNavSearch(u8 taskId)
{
    ...
    if (sDexNavSearchDataPtr->proximity > 20)
    {
...
# 1150 "src/dexnav.c"
    ...
    if (task->data[0] != sDexNavSearchDataPtr->proximity)
    {
        ...
        task->data[0] = sDexNavSearchDataPtr->proximity;
    }
...
static void DexNavUpdateSearchWindow(u8 proximity, u8 searchLevel)
{
    ...
    if (sDexNavSearchDataPtr->itemSpriteId != 64)
        gSprites[sDexNavSearchDataPtr->itemSpriteId].invisible = 1;
    if (sDexNavSearchDataPtr->starSpriteIds[0] != 64)
        gSprites[sDexNavSearchDataPtr->starSpriteIds[0]].invisible = 1;
    if (sDexNavSearchDataPtr->starSpriteIds[1] != 64)
        gSprites[sDexNavSearchDataPtr->starSpriteIds[1]].invisible = 1;
    ...
    if (proximity <= 4)
    {
        if (searchLevel > 2 && sDexNavSearchDataPtr->heldItem)
        {
            ...
            if (sDexNavSearchDataPtr->itemSpriteId != 64)
                gSprites[sDexNavSearchDataPtr->itemSpriteId].invisible = 0;
        }

        if (searchLevel > 4)
        {
            if (sDexNavSearchDataPtr->starSpriteIds[0] != 64)
                gSprites[sDexNavSearchDataPtr->starSpriteIds[0]].invisible = 0;

            if (sDexNavSearchDataPtr->starSpriteIds[1] != 64)
                gSprites[sDexNavSearchDataPtr->starSpriteIds[1]].invisible = 0;

            if (sDexNavSearchDataPtr->starSpriteIds[2] != 64)
                gSprites[sDexNavSearchDataPtr->starSpriteIds[2]].invisible = 0;
        }
    }
}
...
static u8 DexNavTryGenerateMonLevel(u16 species, u8 environment)
{
 ...
 u8 level = getMinWildPokemonLevel();

    if (levelBase == 255 || levelBase == 1)
        return 255;
...
static u8 DexNavGetAbilityNum(u16 species, u8 searchLevel)
{
    ...
    if (genAbility && gBaseStats[species].abilityHidden != 0 && GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
    {
   ...
   if (landMonsNightInfo == ((void *)0))
    ...
    case 2:
        ...
        if (hiddenMonsInfo->encounterRate == 1)
            sDexNavSearchDataPtr->environment = 1;
        else
            sDexNavSearchDataPtr->environment = 0;
        break;
    default:
        return 255;
    }
...
static bool8 DexNav_InitBgs(void)
{
    ...
    if (sBg1TilemapBuffer == ((void *)0))
        return 0;

    memset(sBg1TilemapBuffer, 0, 0x800);
    ResetBgsAndClearDma3BusyFlags(0);
    InitBgsFromTemplates(0, sDexNavMenuBgTemplates, (sizeof(sDexNavMenuBgTemplates)/sizeof(*(sDexNavMenuBgTemplates))));
...
static void UpdateCursorPosition(void)
{
    ...
    {
    case 0:
        x = 30 + (24 * sDexNavUiDataPtr->cursorCol);
        y = 35;
        sDexNavUiDataPtr->environment = 1;
        break;
    case 1:
        x = 20 + (24 * sDexNavUiDataPtr->cursorCol);
        y = 72;
        sDexNavUiDataPtr->environment = 0;
        break;
    case 2:
        x = 20 + (24 * sDexNavUiDataPtr->cursorCol);
        y = (72 + 28);
        ...
        break;
    case 3:
        x = 52 + (24 * sDexNavUiDataPtr->cursorCol);
        y = 138;
        sDexNavUiDataPtr->environment = 2;
        break;
    default:
        return;
    }
...
static void CB1_InitDexNavSearch(void)
{
    ...
    if (!gPaletteFade.active && !ScriptContext2_IsEnabled() && gMain.callback2 == CB2_Overworld)
    {
        ...
        taskId = CreateTask(Task_InitDexNavSearch, 0);
        gTasks[taskId].data[2] = gSpecialVar_0x8000;
        gTasks[taskId].data[3] = gSpecialVar_0x8001;
    }
}
...
static void DexNavLoadEncounterData(void)
{
 ...
 if (IsCurrentlyDay())
    ...
    if (waterMonsInfo != ((void *)0) && waterMonsInfo->encounterRate != 0)
    {
        ...
        {
            species = waterMonsInfo->wildPokemon[i].species;
            if (species != 0 && !SpeciesInArray(species, 1))
    ...
    if (hiddenMonsInfo != ((void *)0))
    {
        ...
        {
            species = hiddenMonsInfo->wildPokemon[i].species;
            if (species != 0 && !SpeciesInArray(species, 2))
...
static const u8 sMoveTypeToOamPaletteNum[19] =
{
    [0] = 13,
    [1] = 13,
    [2] = 14,
...
static void PrintCurrentSpeciesInfo(void)
{
    ...
    FillWindowPixelBuffer(WINDOW_INFO, ((0x0) | ((0x0) << 4)));


    if (species == 0)
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, 5, sFontColor_Black, 0, sText_DexNav_NoInfo);
    else
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, 5, sFontColor_Black, 0, gSpeciesNames[species]);
    ...
    if (species == 0)
    {
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((5 + 24) + 24), sFontColor_Black, 0, sText_DexNav_NoInfo);
    }
    else
    {
        ConvertIntToDecimalStringN(gStringVar4, gSaveBlock1Ptr->dexNavSearchLevels[dexNum], 0, 4);
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((5 + 24) + 24), sFontColor_Black, 0, gStringVar4);
    }
    ...
    if (species == 0)
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, (((5 + 24) + 24) + 24), sFontColor_Black, 0, sText_DexNav_NoInfo);
    else
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, (((5 + 24) + 24) + 24), sFontColor_Black, 0, gStringVar4);


    if (species == 0)
    {
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((((5 + 24) + 24) + 24) + 24), sFontColor_Black, 0, sText_DexNav_NoInfo);
    }
    else if (GetSetPokedexFlag(dexNum, FLAG_GET_CAUGHT))
    {
        if (gBaseStats[species].abilityHidden != 0)
            AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((((5 + 24) + 24) + 24) + 24), sFontColor_Black, 0, gAbilityNames[gBaseStats[species].abilityHidden]);
        else
            AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((((5 + 24) + 24) + 24) + 24), sFontColor_Black, 0, gText_None);
    }
    else
    {
        AddTextPrinterParameterized3(WINDOW_INFO, 0, 0, ((((5 + 24) + 24) + 24) + 24), sFontColor_Black, 0, sText_DexNav_CaptureToSee);
    }
...
static void PrintMapName(void)
{
    ...
    AddTextPrinterParameterized3(WINDOW_REGISTERED, 1, 108 +
      GetStringRightAlignXOffset(1, gStringVar3, 16 * GetFontAttribute(1, FONTATTR_MAX_LETTER_WIDTH)), 0, sFontColor_White, 0, gStringVar3);
    CopyWindowToVram(WINDOW_REGISTERED, 3);
}

static void PrintSearchableSpecies(u16 species)
{
    FillWindowPixelBuffer(WINDOW_REGISTERED, ((0x0) | ((0x0) << 4)));
    PutWindowTilemap(WINDOW_REGISTERED);
    if (species == 0)
    {
        AddTextPrinterParameterized3(WINDOW_REGISTERED, 1, 0, 0, sFontColor_White, 0xFF, sText_DexNav_PressRToRegister);
    }
    else
    {
        StringCopy(gStringVar1, gSpeciesNames[species]);
        StringExpandPlaceholders(gStringVar4, sText_DexNav_SearchForRegisteredSpecies);
        AddTextPrinterParameterized3(WINDOW_REGISTERED, 1, 0, 0, sFontColor_White, 0xFF, gStringVar4);
    }
...
static bool8 DexNav_DoGfxSetup(void)
{
    ...
    case 4:
        ...
        break;
    case 5:
        if (DexNav_LoadGraphics() == 1)
            gMain.state++;
        break;
    case 6:
        DexNav_InitWindows();
        sDexNavUiDataPtr->cursorRow = 1;
        sDexNavUiDataPtr->cursorCol = 0;
        sDexNavUiDataPtr->environment = 0;
        gMain.state++;
        break;
    case 7:
        PrintSearchableSpecies(VarGet(0x40F7) & 0x3FFF);
    ...
    case 8:
        ...
        gTasks[taskId].data[3] = sDexNavUiDataPtr->environment;
        gMain.state++;
        break;
    case 9:
...
{
    if ((sDexNavUiDataPtr = AllocZeroed(sizeof(struct DexNavGUI))) == ((void *)0))
    {
...
static void Task_DexNavMain(u8 taskId)
{
    ...
    else if (({(gMain.newKeys) & (0x0040);}))
    {
        ...
        {
            sDexNavUiDataPtr->cursorRow = 3;
            if (sDexNavUiDataPtr->cursorCol >= 3)
                sDexNavUiDataPtr->cursorCol = (3 - 1);
        }
        else
        {
            if (sDexNavUiDataPtr->cursorRow == 1 && sDexNavUiDataPtr->cursorCol == (6 - 1))
                sDexNavUiDataPtr->cursorCol = (5 - 1);
    ...
    {
        if (sDexNavUiDataPtr->cursorRow == 3)
        {
            sDexNavUiDataPtr->cursorRow = 0;
        }
        else if (sDexNavUiDataPtr->cursorRow == 2)
        {
            if (sDexNavUiDataPtr->cursorCol >= 3)
                sDexNavUiDataPtr->cursorCol = (3 - 1);
    ...
    else if (({(gMain.newKeys) & (0x0020);}))
    {
        if (sDexNavUiDataPtr->cursorCol == 0)
        {
            ...
            case 3:
                ...
                break;
            default:
                sDexNavUiDataPtr->cursorCol = (6 - 1);
                break;
            }
        }
        else
    ...
    else if (({(gMain.newKeys) & (0x0010);}))
    {
        ...
        {
        case 0:
            if (sDexNavUiDataPtr->cursorCol == (5 - 1))
                sDexNavUiDataPtr->cursorCol = 0;
            else
                sDexNavUiDataPtr->cursorCol++;
            break;
        case 3:
            if (sDexNavUiDataPtr->cursorCol == (3 - 1))
                sDexNavUiDataPtr->cursorCol = 0;
            else
                sDexNavUiDataPtr->cursorCol++;
            break;
        default:
            if (sDexNavUiDataPtr->cursorCol == (6 - 1))
                sDexNavUiDataPtr->cursorCol = 0;
            else
                sDexNavUiDataPtr->cursorCol++;
            break;
    ...
    else if (({(gMain.newKeys) & (0x0001);}))
    {
        ...
        else
        {
            ...
            gSpecialVar_0x8002 = (sDexNavUiDataPtr->cursorRow == 3) ? 1 : 0;
            PlaySE(112);
            BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 16, ((0) | ((0) << 5) | ((0) << 10)));
...
bool8 TryFindHiddenPokemon(void)
{
    u16 *stepPtr = GetVarPointer(0x40F8);
    u32 attempts = 0;
    u16 currSteps;
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < 60))
    {
        ...
        sDexNavSearchDataPtr->species = species;
        sDexNavSearchDataPtr->hiddenSearch = 1;
        sDexNavSearchDataPtr->environment = environment;
        sDexNavSearchDataPtr->monLevel = DexNavTryGenerateMonLevel(species, environment);
        if (sDexNavSearchDataPtr->monLevel == 255)
        {
            Free(sDexNavSearchDataPtr);
            return 0;
        }
        ...
        gTasks[taskId].data[2] = sDexNavSearchDataPtr->species;
...
static void DrawHiddenSearchWindow(u8 width)
{
    ...
    AddTextPrinterParameterized3(sDexNavSearchDataPtr->windowId, 0, 28 + 4, 0, sSearchFontColor, 0xFF, sText_ThreeQmarks);
    ...
    AddTextPrinterParameterized3(sDexNavSearchDataPtr->windowId, 0, 28 + 4, 12, sSearchFontColor, 0xFF, gStringVar4);
    CopyWindowToVram(sDexNavSearchDataPtr->windowId, 2);
}
...
static void DexNavDrawHiddenIcons(void)
{
    ...
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
        sDexNavSearchDataPtr->ownedIconSpriteId = CreateSprite(&sOwnedIconTemplate, 28 + 6, GetSearchWindowY() + 2, 0);

    if (sDexNavSearchDataPtr->isHiddenMon)
        sDexNavSearchDataPtr->exclamationSpriteId = CreateSprite(&sHiddenMonIconTemplate, 28 + 34, GetSearchWindowY() + 8, 0);
}
...
void TryIncrementSpeciesSearchLevel(u16 dexNum)
{
    if (gMapHeader.regionMapSectionId != 0x3A && gSaveBlock1Ptr->dexNavSearchLevels[dexNum] < 255)
