...
# 4 "src/battle_interface.c" 2
# 1 "include/pokemon.h" 1
# 5 "src/battle_interface.c" 2
...
# 13 "src/battle_interface.c" 2
# 1 "include/constants/songs.h" 1
# 14 "src/battle_interface.c" 2
...
# 18 "src/battle_interface.c" 2
# 1 "include/battle_message.h" 1
# 19 "src/battle_interface.c" 2
...
# 24 "src/battle_interface.c" 2
# 1 "include/constants/battle_anim.h" 1
# 25 "src/battle_interface.c" 2
# 1 "include/constants/rgb.h" 1
# 26 "src/battle_interface.c" 2
# 1 "include/battle_debug.h" 1
# 27 "src/battle_interface.c" 2
# 1 "include/constants/battle_config.h" 1
# 28 "src/battle_interface.c" 2
# 1 "include/data.h" 1
...
static const struct SpriteTemplate sHealthbarSpriteTemplates[4] =
{
    {
        .tileTag = 0xD704,
        .paletteTag = 0xD704,
...
{
    {(size_t)(sizeof(sUnknown_0832C258) / sizeof((sUnknown_0832C258)[0])), sUnknown_0832C258},
    {(size_t)(sizeof(sUnknown_0832C260) / sizeof((sUnknown_0832C260)[0])), sUnknown_0832C260}
};
...
{
    {(size_t)(sizeof(sStatusSummaryBar_Subsprites_0) / sizeof((sStatusSummaryBar_Subsprites_0)[0])), sStatusSummaryBar_Subsprites_0}
};

static const struct SubspriteTable sUnknown_0832C2CC[] =
{
    {(size_t)(sizeof(sUnknown_0832C2AC) / sizeof((sUnknown_0832C2AC)[0])), sUnknown_0832C2AC}
};
...
{
    [PAL_STATUS_PSN] = ((24) | ((12) << 5) | ((24) << 10)),
    [PAL_STATUS_PAR] = ((23) | ((23) << 5) | ((3) << 10)),
    [PAL_STATUS_SLP] = ((20) | ((20) << 5) | ((17) << 10)),
    [PAL_STATUS_FRZ] = ((17) | ((22) << 5) | ((28) << 10)),
    [PAL_STATUS_BRN] = ((28) | ((14) << 5) | ((10) << 10)),
};

static const struct WindowTemplate sHealthboxWindowTemplate = {0, 0, 0, 8, 2, 0, 0};
...
{
    sMegaTriggerGfx, sizeof(sMegaTriggerGfx), 0xD777
};
static const struct SpritePalette sSpritePalette_MegaTrigger =
{
    sMegaTriggerPal, 0xD777
};
...
{
    sMegaIndicatorGfx, sizeof(sMegaIndicatorGfx), 0xD778
};
static const struct SpritePalette sSpritePalette_MegaIndicator =
{
    sMegaIndicatorPal, 0xD778
};
...
u8 CreateBattlerHealthboxSprites(u8 battlerId)
{
    ...
    if (!IsDoubleBattle())
    {
        ...
        gSprites[healthboxRightSpriteId].data[5] = healthboxLeftSpriteId;
    ...
    CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_1), (void*)((0x6000000 + 0x10000) + healthBarSpritePtr->oam.tileNum * 32), 0x04000000 | ((64)/(32/8) & 0x1FFFFF));

    gSprites[healthboxLeftSpriteId].data[5] = healthbarSpriteId;
    gSprites[healthboxLeftSpriteId].data[6] = battlerId;
    gSprites[healthboxLeftSpriteId].invisible = 1;

    gSprites[healthboxRightSpriteId].invisible = 1;
    gSprites[healthboxRightSpriteId].data[6] = 0xFF;

    healthBarSpritePtr->data[5] = healthboxLeftSpriteId;
...
static void SpriteCB_HealthBar(struct Sprite *sprite)
{
    u8 healthboxSpriteId = sprite->data[5];
...
static void SpriteCB_HealthBoxOther(struct Sprite *sprite)
{
    u8 healthboxMainSpriteId = sprite->data[5];
    u8 megaSpriteId = sprite->data[6];
...
static void TryToggleHealboxVisibility(u8 priority, u8 healthboxLeftSpriteId, u8 healthboxRightSpriteId, u8 healthbarSpriteId, u8 indicatorSpriteId)
{
    ...
    case CONTROLLER_MOVEANIMATION:
        if (gBattleResources->bufferA[gBattleAnimAttacker][1] == 144)
            return;
        break;
    ...
    for (i = 0; i < (sizeof(spriteIds)/sizeof(*(spriteIds))); i++)
    {
        if (spriteIds[i] == 0xFF)
            continue;
...
void UpdateOamPriorityInAllHealthboxes(u8 priority)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        u8 healthbarSpriteId = gSprites[gHealthboxSpriteIds[i]].data[5];
...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl)
{
    ...
    u32 xPos, var1;
    void *objVram;
    u8 battler = gSprites[healthboxSpriteId].data[6];
    ...
    if (GetBattlerSide(battler) == 0)
    {
        objVram = (void*)((0x6000000 + 0x10000));
        if (!IsDoubleBattle())
...
void UpdateHpTextInHealthbox(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent)
{
    ...
    else
    {
        ...
        else if (gBattleSpritesDataPtr->battlerData[battler].hpNumbersNoBars)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                CpuSet(&gMonSpritesGfxPtr->barFontGfx[i * 64 + 32], (void*)(((0x6000000 + 0x10000)) + 32 * (gSprites[healthboxSpriteId].oam.tileNum + var + i)), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))
...
static void UpdateHpTextInHealthboxInDoubles(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent)
{
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) == 0)
    {
        if (gBattleSpritesDataPtr->battlerData[gSprites[healthboxSpriteId].data[6]].hpNumbersNoBars)
        {
            ...
            objVram = (void*)((0x6000000 + 0x10000)) + spriteTileNum;

            if (maxOrCurrent != HP_CURRENT)
            {
                ConvertIntToDecimalStringN(text, value, STR_CONV_MODE_RIGHT_ALIGN, 3);
                windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(text, 0, 5, 0, &windowId);
                HpTextIntoHealthboxObject((void*)((0x6000000 + 0x10000)) + spriteTileNum + 0xC0, windowTileData, 2);
                RemoveWindowOnHealthbox(windowId);
                CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_116), (void*)((0x6000000 + 0x10000) + 0x680) + (gSprites[healthboxSpriteId].oam.tileNum * 32), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))
            ...
            else
            {
                ...
                HpTextIntoHealthboxObject((void*)((0x6000000 + 0x10000) + 0x60) + spriteTileNum, windowTileData, 3);
                RemoveWindowOnHealthbox(windowId);
            }
        }
    }
    else
    {
        ...
        if (gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        {
            ...
            for (i = var; i < var + 3; i++)
            {
                if (i < 3)
                {
                    CpuSet(&gMonSpritesGfxPtr->barFontGfx[((i - var) * 64) + 32], (void*)(((0x6000000 + 0x10000)) + 32 * (1 + gSprites[r7].oam.tileNum + i)), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))

                               ;
                }
                else
                {
                    CpuSet(&gMonSpritesGfxPtr->barFontGfx[((i - var) * 64) + 32], (void*)(((0x6000000 + 0x10000) + 0x20) + 32 * (i + gSprites[r7].oam.tileNum)), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))
            ...
            if (maxOrCurrent == HP_CURRENT)
            {
                CpuSet(&gMonSpritesGfxPtr->barFontGfx[224], (void*)(((0x6000000 + 0x10000)) + ((gSprites[r7].oam.tileNum + 4) * 32)), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))

                               ;
                { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, (void*)(((0x6000000 + 0x10000)) + (gSprites[r7].oam.tileNum * 32)), 0x04000000 | 0x01000000 | ((0x20)/(32/8) & 0x1FFFFF)); };
            }
            else
            {
                if (GetBattlerSide(battlerId) == 0)
                {
                    CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_116), (void*)((0x6000000 + 0x10000)) + ((gSprites[healthboxSpriteId].oam.tileNum + 52) * 32), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF))
...
static void PrintSafariMonInfo(u8 healthboxSpriteId, struct Pokemon *mon)
{
 ...
 nature = GetNature(mon, 0);
    ...
    for (j = 6, i = 0; i < var; i++, j++)
    {
        ...
        CpuSet(GetHealthboxElementGfxPtr(elementId), barFontGfx + (i * 64), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
    }

    for (j = 1; j < var + 1; j++)
    {
        spriteTileNum = (gSprites[healthboxSpriteId].oam.tileNum + (j - (j / 8 * 8)) + (j / 8 * 64)) * 32;
        CpuSet(barFontGfx, (void*)((0x6000000 + 0x10000)) + (spriteTileNum), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        barFontGfx += 0x20;

        spriteTileNum = (8 + gSprites[healthboxSpriteId].oam.tileNum + (j - (j / 8 * 8)) + (j / 8 * 64)) * 32;
        CpuSet(barFontGfx, (void*)((0x6000000 + 0x10000)) + (spriteTileNum), 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        barFontGfx += 0x20;
    }

    healthBarSpriteId = gSprites[healthboxSpriteId].data[5];
    ...
    for (j = 0; j < 5; j++)
    {
        if (j <= 1)
        {
            CpuSet(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20], (void*)((0x6000000 + 0x10000)) + (gSprites[healthBarSpriteId].oam.tileNum + 2 + j) * 32, 0x04000000 | ((32)/(32/8) & 0x1FFFFF))

                         ;
        }
        else
        {
            CpuSet(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20], (void*)((0x6000000 + 0x10000) + 0xC0) + (j + gSprites[healthBarSpriteId].oam.tileNum) * 32, 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
         ...
         && (IsDoubleBattle() || GetBattlerSide(i) != 0))
        {
            ...
            if (GetBattlerSide(i) == 0)
            {
                ...
                if (noBars == 1)
                {
                    healthBarSpriteId = gSprites[gHealthboxSpriteIds[i]].data[5];

                    { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, (void*)((0x6000000 + 0x10000) + gSprites[healthBarSpriteId].oam.tileNum * 32), 0x04000000 | 0x01000000 | ((0x100)/(32/8) & 0x1FFFFF)); };
                    UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i], GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]], 57), HP_CURRENT);
                    UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i], GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]], 58), HP_MAX);
                }
                else
                {
                    UpdateStatusIconInHealthbox(gHealthboxSpriteIds[i]);
                    UpdateHealthboxAttribute(gHealthboxSpriteIds[i], &gPlayerParty[gBattlerPartyIndexes[i]], HEALTHBOX_HEALTH_BAR);
                    CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_117), (void*)((0x6000000 + 0x10000) + 0x680 + gSprites[gHealthboxSpriteIds[i]].oam.tileNum * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF));
                }
            }
            else
            {
                if (noBars == 1)
                {
                    ...
                    else
                    {
                        ...
                        { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, (void *)((0x6000000 + 0x10000) + gSprites[healthBarSpriteId].oam.tileNum * 32), 0x04000000 | 0x01000000 | ((0x100)/(32/8) & 0x1FFFFF)); };
                        UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i], GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]], 57), HP_CURRENT);
                        UpdateHpTextInHealthboxInDoubles(gHealthboxSpriteIds[i], GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]], 58), HP_MAX);
                    }
                }
                else
                {
...
void CreateMegaTriggerSprite(u8 battlerId, u8 palId)
{
    ...
    if (GetSpriteTileStartByTag(0xD777) == 0xFFFF)
    ...
    if (gBattleStruct->mega.triggerSpriteId == 0xFF)
    {
        if (gBattleTypeFlags & (1 << 0))
            gBattleStruct->mega.triggerSpriteId = CreateSprite(&sSpriteTemplate_MegaTrigger,
                                                             gSprites[gHealthboxSpriteIds[battlerId]].pos1.x - (15),
                                                             gSprites[gHealthboxSpriteIds[battlerId]].pos1.y - (-4), 0);
        else
            gBattleStruct->mega.triggerSpriteId = CreateSprite(&sSpriteTemplate_MegaTrigger,
                                                             gSprites[gHealthboxSpriteIds[battlerId]].pos1.x - (15),
                                                             gSprites[gHealthboxSpriteIds[battlerId]].pos1.y - (-11), 0);
    }
    gSprites[gBattleStruct->mega.triggerSpriteId].data[0] = battlerId;
    gSprites[gBattleStruct->mega.triggerSpriteId].data[1] = 0;
...
static void SpriteCb_MegaTrigger(struct Sprite *sprite)
{
    ...
    if (sprite->data[1])
    {
        if (sprite->pos1.x != gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos1.x - xSlide)
            sprite->pos1.x++;

        if (sprite->pos1.x >= gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos1.x - xPriority)
            sprite->oam.priority = 2;
        else
            sprite->oam.priority = 1;

        sprite->pos1.y = gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos1.y - yDiff;
        sprite->pos2.y = gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos2.y - yDiff;
        if (sprite->pos1.x == gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos1.x - xSlide)
            DestroyMegaTriggerSprite();
    }
    else
    {
        if (sprite->pos1.x != gSprites[gHealthboxSpriteIds[sprite->data[0]]].pos1.x - xOptimal)
            sprite->pos1.x--;
...
bool32 IsMegaTriggerSpriteActive(void)
{
    if (GetSpriteTileStartByTag(0xD777) == 0xFFFF)
        return 0;
    else if (IndexOfSpritePaletteTag(0xD777) != 0xFF)
        return 1;
    else
        return 0;
}
...
void HideMegaTriggerSprite(void)
{
    ...
    gSprites[gBattleStruct->mega.triggerSpriteId].data[1] = 1;
}

void DestroyMegaTriggerSprite(void)
{
    FreeSpritePaletteByTag(0xD777);
    FreeSpriteTilesByTag(0xD777);
    if (gBattleStruct->mega.triggerSpriteId != 0xFF)
...
u32 CreateMegaIndicatorSprite(u32 battlerId, u32 which)
{
    ...
    gSprites[spriteId].data[0] = battlerId;
    return spriteId;
}
...
{
    u32 i;
    s16 *spriteId = &gSprites[gSprites[healthboxSpriteId].oam.affineParam].data[6];
    ...
    for (i = 0; i < 4; i++)
    {
        if (gSprites[gSprites[gHealthboxSpriteIds[i]].oam.affineParam].data[6] != 0xFF)
            break;
    }

    if (i == 4)
    {
        FreeSpritePaletteByTag(0xD778);
        FreeSpriteTilesByTag(0xD778);
    }
}
...
u8 CreatePartyStatusSummarySprites(u8 battlerId, struct HpAndStatus *partyInfo, u8 arg2, bool8 isBattleStart)
{
    ...
    if (!arg2 || GetBattlerPosition(battlerId) != 3)
    {
        if (GetBattlerSide(battlerId) == 0)
        {
            isOpponent = 0;
    ...
    if (isOpponent)
    {
        ...
        gSprites[summaryBarSpriteId].oam.matrixNum = 0x08;
    }
    else
    {
        gSprites[summaryBarSpriteId].pos1.x += 96;
    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        ...
        else
        {
            for (i = 0, var = 5, j = 0; j < 6; j++)
            {
                ...
                else if (gBattleTypeFlags & (1 << 18) && gBattleStruct->arenaLostPlayerMons & gBitTable[j])
                {
                    gSprites[ballIconSpritesIds[i]].oam.tileNum += 3;
                }
    ...
    else
    {
        ...
        else
        {
            for (var = 0, i = 0, j = 0; j < 6; j++)
            {
                ...
                else if (gBattleTypeFlags & (1 << 18) && gBattleStruct->arenaLostOpponentMons & gBitTable[j])
                {
                    gSprites[ballIconSpritesIds[5 - var]].oam.tileNum += 3;
                }
                else if (partyInfo[j].status != 0)
    ...
    gTasks[taskId].data[1] = summaryBarSpriteId;
    ...
    PlaySE12WithPanning(114, 0);
    return taskId;
}

void Task_HidePartyStatusSummary(u8 taskId)
{
    u8 ballIconSpriteIds[6];
    bool8 isBattleStart;
    u8 summaryBarSpriteId;
    u8 battlerId;
    s32 i;

    isBattleStart = gTasks[taskId].data[10];
    ...
    if (isBattleStart && gSaveBlock2Ptr->optionsTransitionSpeed == 1)
    {
...
static void sub_8073E08(u8 taskId)
{
    if ((gTasks[taskId].data[11]++ % 2) == 0)
    {
        ...
        SetGpuReg(0x52, (((16 - gTasks[taskId].data[15]) << 8) | (gTasks[taskId].data[15])));
    }
    if (gTasks[taskId].data[15] == 0)
...
static void SpriteCB_StatusSummaryBallsOnBattleStart(struct Sprite *sprite)
{
    ...
    if (sprite->pos2.x == 0)
    {
        ...
        if (sprite->data[7] != 0)
            PlaySE2WithPanning(116, pan);
        else
            PlaySE1WithPanning(115, pan);
...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    u8 *windowTileData;
    u8 gender;
    struct Pokemon *illusionMon = GetIllusionMonPtr(gSprites[healthboxSpriteId].data[6]);
    ...
    if ((species == 29 || species == 32) && StringCompare(nickname, gSpeciesNames[species]) == 0)
        gender = 100;
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) == 0)
    {
        TextIntoHealthboxObject((void*)(0x6000000 + 0x10040 + spriteTileNum), windowTileData, 6);
        ptr = (void*)((0x6000000 + 0x10000));
        if (!IsDoubleBattle())
    ...
    {
        TextIntoHealthboxObject((void*)(0x6000000 + 0x10020 + spriteTileNum), windowTileData, 7);
    }
...
static void TryAddPokeballIconToHealthbox(u8 healthboxSpriteId, bool8 noStatus)
{
    ...
    if (GetBattlerSide(battlerId) == 0)
        return;
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]], 11)), FLAG_GET_CAUGHT))
        return;

    healthBarSpriteId = gSprites[healthboxSpriteId].data[5];

    if (noStatus)
        CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_70), (void*)((0x6000000 + 0x10000) + (gSprites[healthBarSpriteId].oam.tileNum + 8) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF));
    else
        { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, (void*)((0x6000000 + 0x10000) + (gSprites[healthBarSpriteId].oam.tileNum + 8) * 32), 0x04000000 | 0x01000000 | ((32)/(32/8) & 0x1FFFFF)); };
}
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        status = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], 55);
        if (!IsDoubleBattle())
            tileNumAdder = 0x1A;
        else
            tileNumAdder = 0x12;
    ...
    {
        status = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]], 55);
        tileNumAdder = 0x11;
    }
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
            CpuSet(statusGfxPtr, (void*)((0x6000000 + 0x10000) + (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder + i) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF));

        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
            CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_1), (void *)((0x6000000 + 0x10000) + gSprites[healthBarSpriteId].oam.tileNum * 32), 0x04000000 | ((64)/(32/8) & 0x1FFFFF));

        TryAddPokeballIconToHealthbox(healthboxSpriteId, 1);
        return;
    }
    ...
    CpuSet(gPlttBufferUnfaded + 0x100 + pltAdder, (void*)((0x5000000 + 0x200) + pltAdder * 2), 0x00000000 | ((2)/(16/8) & 0x1FFFFF));
    CpuSet(statusGfxPtr, (void*)((0x6000000 + 0x10000) + (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder) * 32), 0x04000000 | ((96)/(32/8) & 0x1FFFFF));
    if (IsDoubleBattle() == 1 || GetBattlerSide(battlerId) == 1)
    {
        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        {
            CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_0), (void*)((0x6000000 + 0x10000) + gSprites[healthBarSpriteId].oam.tileNum * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF));
            CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_65), (void*)((0x6000000 + 0x10000) + (gSprites[healthBarSpriteId].oam.tileNum + 1) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF));
        }
    }
    TryAddPokeballIconToHealthbox(healthboxSpriteId, 0);
}
...
static void UpdateSafariBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * 32;
    TextIntoHealthboxObject((void*)((0x6000000 + 0x10000) + 0x40) + spriteTileNum, windowTileData, 6);
    TextIntoHealthboxObject((void*)((0x6000000 + 0x10000) + 0x800) + spriteTileNum, windowTileData + 0xC0, 2);
    RemoveWindowOnHealthbox(windowId);
}
...
static void UpdateLeftNoOfBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * 32;
    SafariTextIntoHealthboxObject((void*)((0x6000000 + 0x10000) + 0x2C0) + spriteTileNum, windowTileData, 2);
    SafariTextIntoHealthboxObject((void*)((0x6000000 + 0x10000) + 0xA00) + spriteTileNum, windowTileData + 0x40, 4);
    RemoveWindowOnHealthbox(windowId);
}
...
void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon, u8 elementId)
{
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) == 0)
    {
        ...
        if (elementId == HEALTHBOX_LEVEL || elementId == HEALTHBOX_ALL)
            UpdateLvlInHealthbox(healthboxSpriteId, GetMonData(mon, 56));
        if (elementId == HEALTHBOX_CURRENT_HP || elementId == HEALTHBOX_ALL)
            UpdateHpTextInHealthbox(healthboxSpriteId, GetMonData(mon, 57), HP_CURRENT);
        if (elementId == HEALTHBOX_MAX_HP || elementId == HEALTHBOX_ALL)
            UpdateHpTextInHealthbox(healthboxSpriteId, GetMonData(mon, 58), HP_MAX);
        if (elementId == HEALTHBOX_HEALTH_BAR || elementId == HEALTHBOX_ALL)
        {
            LoadBattleBarGfx(0);
            maxHp = GetMonData(mon, 58);
            currHp = GetMonData(mon, 57);
...
static void MoveBattleBarGraphically(u8 battlerId, u8 whichBar)
{
    ...
    case HEALTH_BAR:
        ...
        for (i = 0; i < 6; i++)
        {
            u8 healthbarSpriteId = gSprites[gBattleSpritesDataPtr->battleBars[battlerId].healthboxSpriteId].data[5];
            if (i < 2)
                CpuSet(GetHealthboxElementGfxPtr(barElementId) + array[i] * 32, (void*)((0x6000000 + 0x10000) + (gSprites[healthbarSpriteId].oam.tileNum + 2 + i) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
                                                                                                                      ;
            else
                CpuSet(GetHealthboxElementGfxPtr(barElementId) + array[i] * 32, (void*)((0x6000000 + 0x10000) + 64 + (i + gSprites[healthbarSpriteId].oam.tileNum) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
                                                                                                                       ;
        }
        break;
    case EXP_BAR:
        CalcBarFilledPixels(gBattleSpritesDataPtr->battleBars[battlerId].maxValue,
                    ...
                    &gBattleSpritesDataPtr->battleBars[battlerId].currValue,
                    array, 64 / 8);
        level = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], 56);
        if (level == 100)
        {
            for (i = 0; i < 8; i++)
                array[i] = 0;
        ...
        {
            if (i < 4)
                CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_12) + array[i] * 32, (void*)((0x6000000 + 0x10000) + (gSprites[gBattleSpritesDataPtr->battleBars[battlerId].healthboxSpriteId].oam.tileNum + 0x24 + i) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
                                                                                                                                                                      ;
            else
                CpuSet(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_12) + array[i] * 32, (void*)((0x6000000 + 0x10000) + 0xB80 + (i + gSprites[gBattleSpritesDataPtr->battleBars[battlerId].healthboxSpriteId].oam.tileNum) * 32), 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
                                                                                                                                                                       ;
        }
        break;
    }
...
static s32 CalcNewBarValue(s32 maxValue, s32 oldValue, s32 receivedValue, s32 *currValue, u8 scale, u16 toAdd)
{
    ...
    if (maxValue < scale)
    {
        if (newValue == ((int)((*currValue) >> 8)) && (*currValue & 0xFF) == 0)
            return -1;
    }
    else
    {
    ...
    if (maxValue < scale)
    {
        s32 toAdd = ((s32)((maxValue) << 8)) / scale;
...
static u8* AddTextPrinterAndCreateWindowOnHealthbox(const u8 *str, u32 x, u32 y, u32 bgColor, u32 *windowId)
{
    ...
    FillWindowPixelBuffer(winId, ((bgColor) | ((bgColor) << 4)));
...
static void FillHealthboxObject(void *dest, u32 arg1, u32 arg2)
{
    { vu32 tmp = (vu32)(0x11111111 * arg1); CpuSet((void *)&tmp, dest, 0x04000000 | 0x01000000 | ((arg2 * 32)/(32/8) & 0x1FFFFF)); };
}

static void HpTextIntoHealthboxObject(void *dest, u8 *windowTileData, u32 windowWidth)
{
    CpuSet(windowTileData + 256, dest, 0x04000000 | ((windowWidth * 32)/(32/8) & 0x1FFFFF));
}
...
static void TextIntoHealthboxObject(void *dest, u8 *windowTileData, s32 windowWidth)
{
    ...
    if (windowWidth > 0)
    {
        do
        {
            CpuSet(windowTileData + 20, dest + 20, 0x04000000 | ((12)/(32/8) & 0x1FFFFF));
...
{
    sAbilityPopUpGfx, sizeof(sAbilityPopUpGfx), 0xD720
};
static const struct SpritePalette sSpritePalette_AbilityPopUp =
{
    sAbilityPopUpPalette, 0xD720
};
...
static const s16 sAbilityPopUpCoordsDoubles[4][2] =
{
    {29, 80},
    {204, 19},
    {29, 97},
...
static const s16 sAbilityPopUpCoordsSingles[4][2] =
{
    {29, 93},
    {204, 23},
};
...
{
    CpuSet(windowTileData + 256, dest + 256, 0x04000000 | ((arg2 * 32)/(32/8) & 0x1FFFFF));
    if (arg2 > 0)
    {
        do
        {
            if (arg3)
                CpuSet(windowTileData + 16, dest + 16, 0x04000000 | ((16)/(32/8) & 0x1FFFFF));
            else
                CpuSet(windowTileData + 20, dest + 20, 0x04000000 | ((12)/(32/8) & 0x1FFFFF));
            dest += 32, windowTileData += 32;
            arg2--;
...
static void PrintBattlerOnAbilityPopUp(u8 battlerId, u8 spriteId1, u8 spriteId2)
{
    PrintOnAbilityPopUp(gBattleMons[battlerId].nickname,
                        (void*)((0x6000000 + 0x10000)) + (gSprites[spriteId1].oam.tileNum * 32),
                        (void*)((0x6000000 + 0x10000)) + (gSprites[spriteId2].oam.tileNum * 32),
                        7, 0,
                        0,
                        2, 7, 1);
}
...
static void PrintAbilityOnAbilityPopUp(u32 ability, u8 spriteId1, u8 spriteId2)
{
    PrintOnAbilityPopUp(gAbilityNames[ability],
                        (void*)((0x6000000 + 0x10000)) + (gSprites[spriteId1].oam.tileNum * 32) + 256,
                        (void*)((0x6000000 + 0x10000)) + (gSprites[spriteId2].oam.tileNum * 32) + 256,
                        7, 1,
                        4,
                        7, 9, 1);
}
...
static const u16 sOverwrittenPixelsTable[][2] =
{
 ...
 {( ((1 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((1) - ((1 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((2 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((2) - ((2 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((3 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((3) - ((3 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((4 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((4) - ((4 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((5 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((5) - ((5 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((6 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((6) - ((6 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 5},
 {( ((7 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((7) - ((7 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((8 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((8) - ((8 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((9 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((9) - ((9 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((10 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((10) - ((10 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((11 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((11) - ((11 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((12 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((12) - ((12 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((13 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 8},

 {( ((13 / 8) * 32 * 8) + ((8 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((8) - ((8 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((16 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((16) - ((16 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((24 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((24) - ((24 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((32 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((32) - ((32 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((40 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((40) - ((40 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((48 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((48) - ((48 / 8) * 8)) / 2))), 8},
 {( ((13 / 8) * 32 * 8) + ((56 / 8) * 32) + ((((13) - ((13 / 8) * 8))) * 4) + ((((56) - ((56 / 8) * 8)) / 2))), 8},

    {( ((14 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((8 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((8) - ((8 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((16 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((16) - ((16 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((24 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((24) - ((24 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((32 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((32) - ((32 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((40 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((40) - ((40 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((48 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((48) - ((48 / 8) * 8)) / 2))), 8},
 {( ((14 / 8) * 32 * 8) + ((56 / 8) * 32) + ((((14) - ((14 / 8) * 8))) * 4) + ((((56) - ((56 / 8) * 8)) / 2))), 8},

 {( ((15 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((15) - ((15 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((16 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((16) - ((16 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((17 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((17) - ((17 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((18 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((18) - ((18 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((19 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((19) - ((19 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((20 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((20) - ((20 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((21 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((21) - ((21 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((22 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((22) - ((22 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((23 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((23) - ((23 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((24 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((24) - ((24 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((25 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((25) - ((25 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
 {( ((26 / 8) * 32 * 8) + ((0 / 8) * 32) + ((((26) - ((26 / 8) * 8))) * 4) + ((((0) - ((0 / 8) * 8)) / 2))), 3},
};
...
static void RestoreOverwrittenPixels(u8 *tiles)
{
 ...
 CpuSet(tiles, buffer, 0x04000000 | ((sizeof(sAbilityPopUpGfx))/(32/8) & 0x1FFFFF));

 for (i = 0; i < (size_t)(sizeof(sOverwrittenPixelsTable) / sizeof((sOverwrittenPixelsTable)[0])); i++)
 {
 ...
 CpuSet(buffer, tiles, 0x04000000 | ((sizeof(sAbilityPopUpGfx))/(32/8) & 0x1FFFFF));
 Free(buffer);
}
...
void CreateAbilityPopUp(u8 battlerId, u32 ability, bool32 isDoubleBattle)
{
    ...
    if ((battlerPosition & 1) == 0)
    {
        spriteId1 = CreateSprite(&sSpriteTemplate_AbilityPopUp1,
                                coords[battlerPosition][0] - 68,
                                coords[battlerPosition][1], 0);
        spriteId2 = CreateSprite(&sSpriteTemplate_AbilityPopUp2,
                                coords[battlerPosition][0] - 68 + 64,
                                coords[battlerPosition][1], 0);

        gSprites[spriteId1].data[0] = coords[battlerPosition][0];
        gSprites[spriteId2].data[0] = coords[battlerPosition][0] + 64;

        gSprites[spriteId1].data[3] = 1;
        gSprites[spriteId2].data[3] = 1;
    }
    else
    {
        spriteId1 = CreateSprite(&sSpriteTemplate_AbilityPopUp1,
        ...
        gSprites[spriteId1].data[3] = 0;
        gSprites[spriteId2].data[3] = 0;
    }
    ...
    gTasks[taskId].data[6] = spriteId1;
    gTasks[taskId].data[7] = spriteId2;

    gSprites[spriteId1].data[5] = 1;
    gSprites[spriteId1].data[4] = battlerId;
    gSprites[spriteId2].data[4] = battlerId;
    ...
    RestoreOverwrittenPixels((void*)((0x6000000 + 0x10000)) + (gSprites[spriteId1].oam.tileNum * 32));
}
...
static void SpriteCb_AbilityPopUp(struct Sprite *sprite)
{
    if (!sprite->data[1])
    {
        if (sprite->data[5] && ++sprite->data[2] == 4)
            PlaySE(5);
        if ((!sprite->data[3] && (sprite->pos1.x -= 4) <= sprite->data[0])
            || (sprite->data[3] && (sprite->pos1.x += 4) >= sprite->data[0])
           )
        {
    ...
    else
    {
        if (sprite->data[2] == 0)
        {
            if ((!sprite->data[3] && (sprite->pos1.x += 4) >= sprite->data[0] + 68)
                ||(sprite->data[3] && (sprite->pos1.x -= 4) <= sprite->data[0] - 68)
               )
            {
                gBattleStruct->activeAbilityPopUps &= ~(gBitTable[sprite->data[4]]);
...
void DestroyAbilityPopUp(u8 battlerId)
{
    gSprites[gBattleStruct->abilityPopUpSpriteIds[battlerId][0]].data[2] = 0;
    gSprites[gBattleStruct->abilityPopUpSpriteIds[battlerId][1]].data[2] = 0;
    gBattleScripting.fixedPopup = 0;
}

static void Task_FreeAbilityPopUpGfx(u8 taskId)
{
    if (!gSprites[gTasks[taskId].data[6]].inUse
        && !gSprites[gTasks[taskId].data[7]].inUse
        && !gBattleStruct->activeAbilityPopUps)
    {
        FreeSpriteTilesByTag(0xD720);
