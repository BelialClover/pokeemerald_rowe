...
# 2 "src/battle_ai_switch_items.c" 2
...
# 3 "src/battle_ai_switch_items.c" 2
...
# 4 "src/battle_ai_switch_items.c" 2
...
# 5 "src/battle_ai_switch_items.c" 2
...
# 6 "src/battle_ai_switch_items.c" 2
...
# 7 "src/battle_ai_switch_items.c" 2
# 1 "include/pokemon.h" 1
# 8 "src/battle_ai_switch_items.c" 2
...
# 9 "src/battle_ai_switch_items.c" 2
# 1 "include/util.h" 1
...
# 10 "src/battle_ai_switch_items.c" 2
# 1 "include/constants/abilities.h" 1
# 11 "src/battle_ai_switch_items.c" 2
# 1 "include/constants/item_effects.h" 1
# 12 "src/battle_ai_switch_items.c" 2
# 1 "include/constants/items.h" 1
# 13 "src/battle_ai_switch_items.c" 2
# 1 "include/constants/moves.h" 1
# 14 "src/battle_ai_switch_items.c" 2
...
void GetAIPartyIndexes(u32 battlerId, s32 *firstId, s32 *lastId)
{
    if (((gBattleTypeFlags & (1 << 22) && gTrainerBattleOpponent_B == 0xFFFF)) && (battlerId & 1) == 1)
    {
        *firstId = 0, *lastId = 6;
    }
    else if (gBattleTypeFlags & ((1 << 15) | (1 << 22) | (1 << 23)))
    {
        if ((battlerId & 2) == 0)
            *firstId = 0, *lastId = 3;
        else
...
static bool8 ShouldSwitchIfWonderGuard(void)
{
    ...
    for (opposingBattler = GetBattlerAtPosition(opposingPosition), i = 0; i < 4; i++)
    {
        ...
        if (move != 0)
        {
            if (AI_GetTypeEffectiveness(move, gActiveBattler, opposingBattler) >= ((u16)((2.0) * 4096)))
                return 0;
        }
    }
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (GetMonData(&party[i], 65) == 0)
            continue;
        if (GetMonData(&party[i], 65) == 898 + 308 + 1)
            continue;
        ...
        {
            move = GetMonData(&party[i], 13 + j);
            if (move != 0)
            {
                if (AI_GetTypeEffectiveness(move, gActiveBattler, opposingBattler) >= ((u16)((2.0) * 4096)) && Random() % 3 < 2)
                {
...
static bool8 FindMonThatAbsorbsOpponentsMove(void)
{
    ...
    if (HasSuperEffectiveMoveAgainstOpponents(1) && Random() % 3 != 0)
        return 0;
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        species = GetMonData(&party[i], 11);
        if (GetMonData(&party[i], 46) != 0)
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        if (gMoveResultFlags & flags)
        {
            ...
            for (j = 0; j < 4; j++)
            {
                ...
                if (AI_GetTypeEffectiveness(move, gActiveBattler, battlerIn1) >= ((u16)((2.0) * 4096)) && Random() % moduloPercent == 0)
                {
                    *(gBattleStruct->AI_monToSwitchIntoId + gActiveBattler) = i;
                    BtlController_EmitTwoReturnValues(1, 2, 0);
                    return 1;
...
static bool8 ShouldSwitch(void)
{
    ...
    if (gBattleMons[gActiveBattler].status2 & ((1 << 13) | (1 << 26)))
        return 0;
...
void AI_TrySwitchOrUseItem(void)
{
        ...
        if (ShouldSwitch())
        {
            if (*(gBattleStruct->AI_monToSwitchIntoId + gActiveBattler) == 6)
            {
                ...
                if (monToSwitchId == 6)
                {
                    ...
                    {
                        if (GetMonData(&party[monToSwitchId], 57) == 0)
                            continue;
    ...
    BtlController_EmitTwoReturnValues(1, 0, (gActiveBattler ^ 1) << 8);
}
...
static u32 GetBestMonBatonPass(struct Pokemon *party, int firstId, int lastId, u8 invalidMons, int aliveCount)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            if (GetMonData(&party[i], 13 + j, ((void *)0)) == 226)
            {
                bits |= gBitTable[i];
                break;
            }
...
static u32 GestBestMonOffensive(struct Pokemon *party, int firstId, int lastId, u8 invalidMons, u32 opposingBattler)
{
    ...
    while (bits != 0x3F)
    {
        ...
        if (bestMonId != 6)
        {
            for (i = 0; i < 4; i++)
            {
                u32 move = GetMonData(&party[bestMonId], 13 + i);
                if (move != 0 && AI_GetTypeEffectiveness(move, gActiveBattler, opposingBattler) >= ((u16)((2.0) * 4096)))
                    break;
            }
...
static u32 GetBestMonDmg(struct Pokemon *party, int firstId, int lastId, u8 invalidMons, u32 opposingBattler)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            u32 move = GetMonData(&party[i], 13 + j);
...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    for (i = firstId; i < lastId; i++)
    {
        if (GetMonData(&party[i], 11) == 0
            ...
            || (GetMonAbility(&party[i]) == 54 && IsTruantMonVulnerable(gActiveBattler, opposingBattler)))
...
static bool8 ShouldUseItem(void)
{
    ...
    if (gBattleTypeFlags & (1 << 22) && GetBattlerPosition(gActiveBattler) == 2)
        return 0;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (item == 0)
            continue;
        if (gItemEffectTable[item] == ((void *)0))
            continue;
        ...
        *(gBattleStruct->AI_itemType + gActiveBattler / 2) = GetAI_ItemType(item, itemEffects);
        ...
        case AI_ITEM_CURE_CONDITION:
            *(gBattleStruct->AI_itemFlags + gActiveBattler / 2) = 0;
            ...
            if (itemEffects[3] & 0x10 && (gBattleMons[gActiveBattler].status1 & (1 << 3) || gBattleMons[gActiveBattler].status1 & (1 << 7)))
            {
