...
const struct CompressedSpriteSheet gBallSpriteSheets[POKEBALL_COUNT] =
{
    ...
    {gInterfaceGfx_LevelBall, 384, 55012},
    {gInterfaceGfx_LureBall, 384, 55013},
    {gInterfaceGfx_MoonBall, 384, 55014},
    {gInterfaceGfx_FriendBall, 384, 55015},
    {gInterfaceGfx_LoveBall, 384, 55016},
    {gInterfaceGfx_HeavyBall, 384, 55017},
    {gInterfaceGfx_FastBall, 384, 55018},
    {gInterfaceGfx_SportBall, 384, 55019},
    {gInterfaceGfx_DuskBall, 384, 55020},
    {gInterfaceGfx_QuickBall, 384, 55021},
    {gInterfaceGfx_HealBall, 384, 55022},
    {gInterfaceGfx_CherishBall, 384, 55023},
    {gInterfaceGfx_ParkBall, 384, 55024},
    {gInterfaceGfx_DreamBall, 384, 55025},
    {gInterfaceGfx_BeastBall, 384, 55026},
};
...
const struct CompressedSpritePalette gBallSpritePalettes[POKEBALL_COUNT] =
{
    ...
    {gInterfacePal_LevelBall, 55012},
    {gInterfacePal_LureBall, 55013},
    {gInterfacePal_MoonBall, 55014},
    {gInterfacePal_FriendBall, 55015},
    {gInterfacePal_LoveBall, 55016},
    {gInterfacePal_HeavyBall, 55017},
    {gInterfacePal_FastBall, 55018},
    {gInterfacePal_SportBall, 55019},
    {gInterfacePal_DuskBall, 55020},
    {gInterfacePal_QuickBall, 55021},
    {gInterfacePal_HealBall, 55022},
    {gInterfacePal_CherishBall, 55023},
    {gInterfacePal_ParkBall, 55024},
    {gInterfacePal_DreamBall, 55025},
    {gInterfacePal_BeastBall, 55026},
};
...
u8 DoPokeballSendOutAnimation(s16 pan, u8 kindOfThrow)
{
    ...
    gTasks[taskId].data[2] = kindOfThrow;
...
static void Task_DoPokeballSendOutAnim(u8 taskId)
{
    ...
    throwCaseId = gTasks[taskId].data[2];
    battlerId = gTasks[taskId].data[3];
    ...
    gSprites[ballSpriteId].data[6] = gBattlerTarget;
    if (!notSendOut)
    {
        DestroyTask(taskId);
        return;
...
static void SpriteCB_TestBallThrow(struct Sprite *sprite)
{
    if (TranslateAnimHorizontalArc(sprite))
    {
        ...
        u8 opponentBattler = gTasks[taskId].data[4];
        u8 noOfShakes = gTasks[taskId].data[2];
        ...
        sprite->data[0] = LaunchBallFadeMonTask(0, opponentBattler, 14, ballId);
...
static void sub_807574C(struct Sprite *sprite)
{
    ...
    if (gSprites[gBattlerSpriteIds[sprite->data[6]]].affineAnimEnded)
    {
        ...
        gSprites[gBattlerSpriteIds[sprite->data[6]]].invisible = 1;
        sprite->data[5] = 0;
...
{
    u8 wantedCry = gTasks[taskId].data[2];
    s8 pan = gTasks[taskId].data[1];
    u16 species = gTasks[taskId].data[0];
    u8 battlerId = gTasks[taskId].data[3];
    u8 monSpriteId = gTasks[taskId].data[4];
    struct Pokemon *mon = (void*)(u32)((gTasks[taskId].data[5] << 0x10) | (u16)(gTasks[taskId].data[6]));
    ...
    default:
        if (gSprites[monSpriteId].affineAnimEnded)
            gTasks[taskId].data[15] = wantedCry + 1;
        break;
    case 1:
        if (ShouldPlayNormalMonCry(mon) == 1)
...
static void SpriteCB_ReleaseMonFromBall(struct Sprite *sprite)
{
    u8 battlerId = sprite->data[6];
    u32 ballId;

    StartSpriteAnim(sprite, 1);
    ballId = ItemIdToBallId(GetBattlerPokeballItemId(battlerId));
    AnimateBallOpenParticles(sprite->pos1.x, sprite->pos1.y - 5, 1, 0x1C, ballId);
    sprite->data[0] = LaunchBallFadeMonTask(1, sprite->data[6], 14, ballId);
    ...
    if (gMain.inBattle)
    {
        ...
        formSpeciesId = GetFormSpeciesId(GetMonData(mon, 11), GetMonData(mon, 89));
        if ((battlerId == GetBattlerAtPosition(0) || battlerId == GetBattlerAtPosition(1))
        ...
        if (!IsDoubleBattle() || !gBattleSpritesDataPtr->animationData->field_9_x1)
            wantedCryCase = 0;
        else if (battlerId == GetBattlerAtPosition(0) || battlerId == GetBattlerAtPosition(1))
            wantedCryCase = 1;
        else
            wantedCryCase = 2;
        ...
        if (illusionMon != ((void *)0))
            gTasks[taskId].data[0] = GetFormSpeciesId(GetMonData(illusionMon, 11), GetMonData(illusionMon, 89));
        else
            gTasks[taskId].data[0] = formSpeciesId;

        gTasks[taskId].data[1] = pan;
        gTasks[taskId].data[2] = wantedCryCase;
        gTasks[taskId].data[3] = battlerId;
        gTasks[taskId].data[4] = gBattlerSpriteIds[sprite->data[6]];
        gTasks[taskId].data[5] = (u32)(mon) >> 0x10;
        gTasks[taskId].data[6] = (u32)(mon);
        gTasks[taskId].data[15] = 0;
    }

    StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[sprite->data[6]]], 1);

    if (GetBattlerSide(sprite->data[6]) == 1)
        gSprites[gBattlerSpriteIds[sprite->data[6]]].callback = SpriteCb_OpponentMonFromBall;
    else
        gSprites[gBattlerSpriteIds[sprite->data[6]]].callback = SpriteCb_PlayerMonFromBall;
...
void CreatePokeballSpriteToReleaseMon(u8 monSpriteId, u8 battlerId, u8 x, u8 y, u8 oamPriority, u8 subpriortiy, u8 g, u32 h, u16 species)
{
    ...
    gSprites[monSpriteId].invisible = 1;
}
...
static void sub_80765E0(struct Sprite *sprite)
{
    ...
    {
        if (gSprites[monSpriteId].data[7] == 898 + 308 + 1)
            DoMonFrontSpriteAnimation(&gSprites[monSpriteId], gSprites[monSpriteId].data[7], 1, 0);
        else
            DoMonFrontSpriteAnimation(&gSprites[monSpriteId], gSprites[monSpriteId].data[7], 0, 0);
...
void sub_8076918(u8 battlerId)
{
    ...
    if (GetBattlerPosition(battlerId) == 2)
...
void LoadBallGfx(u8 ballId)
{
    ...
    default:
        ...
        LZDecompressVram(gOpenPokeballGfx, (void *)(0x6000000 + 0x10100 + var * 32));
        break;
    }
}
