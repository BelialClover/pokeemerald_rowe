...
# 18 "src/battle_factory.c" 2
# 1 "include/constants/moves.h" 1
# 19 "src/battle_factory.c" 2
...
{
    14, 74, 96, 97, 104, 106,
    107, 110, 111, 112, 116, 133,
    151, 159, 160, 176, 187, 244,
    268, 289, 294, 322, 334, 336, 339, 347, 349,
    0
};

static const u16 sMoves_ImpossibleToPredict[] =
{
    102, 118, 119, 144, 164, 166, 174,
    217, 266, 271, 272, 274, 285, 293,
    0
};

static const u16 sMoves_WeakeningTheFoe[] =
{
    28, 39, 43, 45, 81, 103, 108, 134,
    148, 178, 180, 184, 204, 282, 230, 297,
    313, 319, 321,
    0
};
...
{
    12, 32, 38, 63, 68, 90,
    117, 120, 143, 153, 175, 179, 194,
    195, 220, 243, 262, 288, 263, 264,
    307, 308, 315, 338, 354, 344,
    0
};

static const u16 sMoves_Endurance[] =
{
    54, 105, 113, 114, 115, 135, 156, 182,
    197, 203, 208, 215, 219, 226, 234,
    235, 236, 256, 273, 275, 277, 278, 287,
    300, 303, 312, 346,
    0
};
...
{
    47, 48, 50, 73, 77, 78, 79,
    86, 92, 95, 109, 137, 139, 142, 147,
    169, 186, 191, 207, 212, 213, 227, 259,
    260, 261, 269, 281, 286, 289, 298, 320, 335,
    0
};
...
static const u16 *const sMoveStyles[8 - 1] =
{
    [1 - 1] = sMoves_TotalPreparation,
    [2 - 1] = sMoves_SlowAndSteady,
    [3 - 1] = sMoves_Endurance,
    [4 - 1] = sMoves_HighRiskHighReturn,
    [5 - 1] = sMoves_WeakeningTheFoe,
    [6 - 1] = sMoves_ImpossibleToPredict,
    [7 - 1] = sMoves_DependsOnTheBattlesFlow,
};

static void (* const sBattleFactoryFunctions[])(void) =
{
    [0] = InitFactoryChallenge,
    [1] = GetBattleFactoryData,
    [2] = SetBattleFactoryData,
    [3] = SaveFactoryChallenge,
    [4] = nullsub_75,
    [5] = nullsub_123,
    [6] = SelectInitialRentalMons,
    [7] = SwapRentalMons,
    [8] = SetPerformedRentalSwap,
    [9] = SetRentalsToOpponentParty,
    [10] = SetPlayerAndOpponentParties,
    [11] = SetOpponentGfxVar,
    [12] = GenerateOpponentMons,
    [13] = GenerateInitialRentalMons,
    [14] = GetOpponentMostCommonMonType,
    [15] = GetOpponentBattleStyle,
    [16] = RestorePlayerPartyHeldItems,
};
...
static void GenerateOpponentMons(void)
{
    ...
    while (i != 3)
    {
        u16 monId = GetFactoryMonId(lvlMode, challengeNum, 0);
    ...
    if (gSpecialVar_0x8005 < 2)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            CreateMon(&gPlayerParty[i],
                      ...
                      monLevel,
                      ivs,
                      1, gSaveBlock2Ptr->frontier.rentalMons[i].personality,
                      0, 0, 0);
            ...
            for (j = 0; j < 6; bits <<= 1, j++)
            {
                if (gFacilityTrainerMons[monId].evSpread & bits)
                    SetMonData(&gPlayerParty[i], 26 + j, &evs);
            }
    ...
    case 2:
        for (i = 0; i < 3; i++)
        {
            ...
            CreateMon(&gEnemyParty[i],
                      ...
                      monLevel,
                      ivs,
                      1, gSaveBlock2Ptr->frontier.rentalMons[i + 3].personality,
                      0, 0, 0);
            ...
            {
                if (gFacilityTrainerMons[monId].evSpread & bits)
                    SetMonData(&gEnemyParty[i], 26 + j, &evs);
            }
...
static void GenerateInitialRentalMons(void)
{
    ...
    {
        if (i < rentalRank)
            monId = GetFactoryMonId(factoryLvlMode, challengeNum, 1);
        else
            monId = GetFactoryMonId(factoryLvlMode, challengeNum, 0);
...
static u8 GetMoveBattleStyle(u16 move)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sMoveStyles) / sizeof((sMoveStyles)[0])); i++)
    {
...
bool8 InBattleFactory(void)
{
    return gMapHeader.mapLayoutId == 347
        || gMapHeader.mapLayoutId == 348;
}
...
void FillFactoryBrainParty(void)
{
    ...
    while (i != 3)
    {
        ...
        for (k = 0; k < 4; k++)
            SetMonMoveAvoidReturn(&gEnemyParty[i], gFacilityTrainerMons[monId].moves[k], k);
        SetMonData(&gEnemyParty[i], 32, &friendship);
        SetMonData(&gEnemyParty[i], 12, &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
        i++;
    }
}
