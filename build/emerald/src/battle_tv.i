...
void BattleTv_SetDataBasedOnString(u16 stringId)
{
    ...
    case 221:
        ...
        if (!(gBattleTypeFlags & (1 << 1)) && GetMonData(defMon, 57, ((void *)0)) != 0)
            TrySetBattleSeminarShow();
        break;
    case 222:
...
void TryPutLinkBattleTvShowOnAir(void)
{
    ...
    s16 playerBestSum = 0, opponentBestSum = 32767;
    u8 playerBestMonId = 0, opponentBestMonId = 0;
    struct BattleTvMovePoints *movePoints = ((void *)0);
    ...
    for (i = 0; i < 6; i++)
    {
        if (GetMonData(&gPlayerParty[i], 11, ((void *)0)) != 0)
            countPlayer++;
        if (GetMonData(&gEnemyParty[i], 11, ((void *)0)) != 0)
            countOpponent++;
    }
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (species != 0 && !GetMonData(&gPlayerParty[i], 45, ((void *)0)))
        {
            for (sum = 0, j = 0; j < 4; j++)
        ...
        species = GetMonData(&gEnemyParty[i], 11, ((void *)0));
        if (species != 0 && !GetMonData(&gEnemyParty[i], 45, ((void *)0)))
        {
            for (sum = 0, j = 0; j < 4; j++)
            ...
            {
                if (GetMonData(&gEnemyParty[i], 25, ((void *)0)) > GetMonData(&gEnemyParty[opponentBestMonId], 25, ((void *)0)))
                {
                    opponentBestMonId = i;
                    opponentBestSum = sum;
    ...
    moveId = GetMonData(&gPlayerParty[playerBestMonId], 13 + i, ((void *)0));
