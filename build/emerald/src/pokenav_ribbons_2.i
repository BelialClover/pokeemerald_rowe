# 0 "src/pokenav_ribbons_2.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokenav_ribbons_2.c"
...
# 13 "src/pokenav_ribbons_2.c" 2

# 1 "include/constants/songs.h" 1
# 15 "src/pokenav_ribbons_2.c" 2
...
# 1 "src/data/text/ribbon_descriptions.h" 1
# 22 "src/data/text/ribbon_descriptions.h"
...
# 127 "src/pokenav_ribbons_2.c" 2
# 1 "src/data/text/gift_ribbon_descriptions.h" 1
...
{
    [RIBBONS_SUMMARY_FUNC_NONE] = ((void *)0),
...
u32 RibbonsSummaryHandleInput(struct PokenavSub13 *structPtr)
{
    if (({(gMain.newAndRepeatedKeys) & (0x0040);}) && structPtr->monList->currIndex != 0)
    {
    ...
    if (({(gMain.newAndRepeatedKeys) & (0x0080);}) && structPtr->monList->currIndex < structPtr->monList->listCount - 1)
    {
        structPtr->monList->currIndex++;
        structPtr->field_C = 0;
        sub_81D0814(structPtr);
...
u32 HandleExpandedRibbonInput(struct PokenavSub13 *structPtr)
{
    if (({(gMain.newAndRepeatedKeys) & (0x0040);}) && sub_81D05DC(structPtr))
        return RIBBONS_SUMMARY_FUNC_EXPANDED_CURSOR_MOVE;
    if (({(gMain.newAndRepeatedKeys) & (0x0080);}) && sub_81D061C(structPtr))
        return RIBBONS_SUMMARY_FUNC_EXPANDED_CURSOR_MOVE;
    if (({(gMain.newAndRepeatedKeys) & (0x0020);}) && sub_81D0664(structPtr))
        return RIBBONS_SUMMARY_FUNC_EXPANDED_CURSOR_MOVE;
    if (({(gMain.newAndRepeatedKeys) & (0x0010);}) && sub_81D0688(structPtr))
        return RIBBONS_SUMMARY_FUNC_EXPANDED_CURSOR_MOVE;
    if (({(gMain.newKeys) & (0x0002);}))
    {
        structPtr->callback = RibbonsSummaryHandleInput;
...
static void GetCurrMonInfo2(u16 *species, u32 *personality, u32 *otId)
{
    ...
    {
        struct BoxPokemon *boxMon = GetBoxedMonPtr(monInfo->boxId, monInfo->monId);
        *species = GetBoxMonData(boxMon, 11);
        *personality = GetBoxMonData(boxMon, 0);
        *otId = GetBoxMonData(boxMon, 1);
    }
}
...
static u32 GetCurrMonRibbonCount(void)
{
    ...
    if (monInfo->boxId == 14)
        return GetMonData(&gPlayerParty[monInfo->monId], 82);
    else
        return GetBoxMonDataAt(monInfo->boxId, monInfo->monId, 82);
}
...
void sub_81D0814(struct PokenavSub13 *structPtr)
{
    ...
    if (monInfo->boxId == 14)
        ribbons = GetMonData(&gPlayerParty[monInfo->monId], 83);
    else
        ribbons = GetBoxMonDataAt(monInfo->boxId, monInfo->monId, 83);

    structPtr->field_10 = 0;
    structPtr->field_12 = 0;
    for (i = 0; i < (size_t)(sizeof(gUnknown_086237F8) / sizeof((gUnknown_086237F8)[0])); i++)
    {
...
u32 LoopedTask_OpenRibbonsSummaryMenu(s32 state)
{
    ...
    {
    case 0:
        InitBgTemplates(gUnknown_08624B98, (size_t)(sizeof(gUnknown_08624B98) / sizeof((gUnknown_08624B98)[0])));
...
void PrintCurrentMonRibbonCount(struct PokenavSub14 *structPtr)
{
    ...
    FillWindowPixelBuffer(structPtr->ribbonCountWindowId, ((4) | ((4) << 4)));
...
void PrintRibbbonsSummaryMonInfo(struct PokenavSub14 *structPtr)
{
    ...
    AddTextPrinterParameterized(windowId, 1, gStringVar3, 0, 1, 0xFF, ((void *)0));
    switch (gender)
    {
    case 0x00:
    ...
    *(txtPtr++) = 0x05;
    ...
    AddTextPrinterParameterized(windowId, 1, gStringVar1, 60, 1, 0xFF, ((void *)0));
...
void AddRibbonListIndexWindow(struct PokenavSub14 *structPtr)
{
    ...
    FillWindowPixelBuffer(structPtr->listIdxWindowId, ((1) | ((1) << 4)));
...
void PrintRibbonsMonListIndex(struct PokenavSub14 *structPtr)
{
    ...
    *(txtPtr++) = 0xBA;
    ...
    AddTextPrinterParameterized(structPtr->listIdxWindowId, 1, gStringVar1, x, 1, 0xFF, ((void *)0));
...
u16 DrawRibbonsMonFrontPic(s32 unused0, s32 unused1)
{
    ...
    GetCurrMonInfo2(&species, &personality, &otId);
    spriteId = CreateMonPicSprite_HandleDeoxys(species, otId, personality, 1, 40, 104, 15, 0xFFFF);
...
{
    {.frame = {.xScale = 128, .yScale = 128, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = 32, .yScale = 32, .rotation = 0, .duration = 4}},
    {.type = 0x7FFF}
};
...
void sub_81D13FC(struct PokenavSub14 *structPtr)
{
    ...
    structPtr->field_14->invisible = 1;
}
...
void sub_81D1448(struct PokenavSub14 *structPtr)
{
    ...
    structPtr->field_14->invisible = 0;
