...
__attribute__((section("ewram_data"))) struct ObjectEvent gObjectEvents[16] = {};
...
static bool8 ForcedMovement_MuddySlope(void)
{
    ...
    if (playerObjEvent->movementDirection != 2 || GetPlayerSpeed() <= 3)
    {
...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    if (gPlayerAvatar.flags & (1 << 3))
    {
        ...
        else if (FlagGet((((0x500 + 864 - 1) + 1) + 0x22)) && (heldKeys & 0x0001))
        {
 ...
 if (!(gPlayerAvatar.flags & (1 << 4)) && (heldKeys & 0x0002 || FlagGet(0x35)) && FlagGet(0x33)
    ...
    else if (FlagGet((((0x500 + 864 - 1) + 1) + 0x22)))
    {
        if (heldKeys & 0x0001)
        {
            gPlayerAvatar.creeping = 1;
...
{
    if (FlagGet((((0x500 + 864 - 1) + 1) + 0x29)))
    {
        u8 objectEventId = GetObjectEventIdByXY(x, y);

        if (objectEventId != 16 && gObjectEvents[objectEventId].graphicsId == 87)
        {
...
static void CheckAcroBikeCollision(s16 x, s16 y, u8 metatileBehavior, u8 *collision)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sAcroBikeTrickMetatiles) / sizeof((sAcroBikeTrickMetatiles)[0])); i++)
    {
...
bool8 IsPlayerCollidingWithFarawayIslandMew(u8 direction)
{
    ...
    mewObjectId = GetObjectEventIdByLocalIdAndMap(1, ((57 | (26 << 8)) & 0xFF), ((57 | (26 << 8)) >> 8));
    if (mewObjectId == 16)
        return 0;
...
static void DoPlayerAvatarTransition(void)
{
    ...
    if (flags != 0)
    {
        for (i = 0; i < (size_t)(sizeof(sPlayerAvatarTransitionFuncs) / sizeof((sPlayerAvatarTransitionFuncs)[0])); i++, flags >>= 1)
        {
            if (flags & 1)
...
{
    return 1;
# 1354 "src/field_player_avatar.c"
}

bool8 IsPlayerSurfingNorth(void)
{
    if (GetPlayerMovementDirection() == 2 && TestPlayerAvatarFlags((1 << 3)))
        return 1;
    else
        return 0;
}
...
{
    gPlayerAvatar.flags &= ((1 << 7) | (1 << 6) | (1 << 5));
...
static u8 GetPlayerAvatarStateTransitionByGraphicsId(u8 graphicsId, u8 gender)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sPlayerAvatarGfxToStateFlag[0]) / sizeof((sPlayerAvatarGfxToStateFlag[0])[0])); i++)
    {
...
void InitPlayerAvatar(s16 x, s16 y, u8 direction, u8 gender)
{
    ...
    playerObjEventTemplate.movementType = 0xB;
    ...
    playerObjEventTemplate.script = ((void *)0);
    ...
    SetPlayerAvatarStateMask((1 << 5) | (1 << 0));
}
...
static u8 PlayerAvatar_DoSecretBaseMatJump(struct Task *task, struct ObjectEvent *objectEvent)
{
    ...
    if (ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
        ...
        {
            gPlayerAvatar.preventStep = 0;
            gPlayerAvatar.transitionFlags |= (1 << 5);
...
static void Task_WaitStopSurfing(u8 taskId)
{
    ...
    if (ObjectEventClearHeldMovementIfFinished(playerObjEvent))
    {
        ...
        playerObjEvent->triggerGroundEffectsOnMove = 1;
        DestroyTask(taskId);
    }
}
# 1742 "src/field_player_avatar.c"
...
static bool8 Fishing_GetRodOut(struct Task *task)
{
    ...
    task->data[14] = gObjectEvents[gPlayerAvatar.objectEventId].graphicsId;
...
static bool8 Fishing_ShowDots(struct Task *task)
{
    ...
    if (task->data[1] >= 20)
    {
        ...
        {
            AddTextPrinterParameterized(0, 1, dot, task->data[2] * 12, 1, 0, ((void *)0));
            task->data[2]++;
        }
    }
    return 0;
...
static bool8 Fishing_StartEncounter(struct Task *task)
{
    ...
    if (task->data[1] == 0)
    {
        if (!IsTextPrinterActive(0))
        {
            ...
            ObjectEventSetGraphicsId(playerObjEvent, task->data[14]);
...
static bool8 CanStartSurfing(s16 x, s16 y, u8 direction)
{
    ...
    if ((gPlayerAvatar.flags & (1 << 0))
     && MapGridGetZCoordAt(x, y) == 1
     && GetObjectEventIdByXYZ(x, y, 1) == 16
