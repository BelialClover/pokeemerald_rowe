# 0 "src/palette_util.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/palette_util.c"
...
{
    if (id >= (size_t)(sizeof(flash->palettes) / sizeof((flash->palettes)[0])) || flash->palettes[id].available)
        return 0xFF;
...
{
    if (id >= (size_t)(sizeof(flash->palettes) / sizeof((flash->palettes)[0])))
        return 0xFF;
...
void RouletteFlash_Run(struct RouletteFlashUtil *flash)
{
    ...
    if (flash->enabled)
    {
        for (i = 0; i < (size_t)(sizeof(flash->palettes) / sizeof((flash->palettes)[0])); i++)
        {
            if ((flash->flags >> i) & 1)
            {
...
{
    u8 i = 0;
    struct PulseBlendPalette *pulseBlendPalette = ((void *)0);
