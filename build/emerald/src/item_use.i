...
__attribute__((section("ewram_data"))) static void(*sItemUseOnFieldCB)(u8 taskId) = ((void *)0);
...
static const u8 sClockwiseDirections[] = {2, 4, 1, 3};
...
void ItemUseOutOfBattle_Bike(u8 taskId)
{
    ...
    if (FlagGet((((0x500 + 864 - 1) + 1) + 0x2B)) == 1 || MetatileBehavior_IsVerticalRail(behavior) == 1 || MetatileBehavior_IsHorizontalRail(behavior) == 1 || MetatileBehavior_IsIsolatedVerticalRail(behavior) == 1 || MetatileBehavior_IsIsolatedHorizontalRail(behavior) == 1)
        DisplayCannotDismountBikeMessage(taskId, data[3]);
    else
    {
        if (Overworld_IsBikingAllowed() == 1 && IsBikingDisallowedByPlayer() == 0)
        {
...
static bool32 CanFish(void)
{
    ...
    else
    {
        if (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehavior) && !MapGridIsImpassableAt(x, y))
            return 1;
        if (MetatileBehavior_8089510(tileBehavior) == 1)
            return 1;
    }
...
{
    if (ItemfinderCheckForHiddenItems(gMapHeader.events, taskId) == 1)
    ...
    if (data[3] == 0)
    {
        if (data[4] == 4)
        {
            ...
            else
            {
                ...
                for (i = 0; i < (size_t)(sizeof(sClockwiseDirections) / sizeof((sClockwiseDirections)[0])); i++)
                {
...
static bool8 IsHiddenItemPresentAtCoords(const struct MapEvents *events, s16 x, s16 y)
{
    ...
    {
        if (bgEvent[i].kind == 7 && x == (u16)bgEvent[i].x && y == (u16)bgEvent[i].y)
        {
    ...
    for (x = playerX - 7; x <= playerX + 7; x++)
    {
        for (y = playerY - 5; y <= playerY + 5; y++)
        {
             ...
             || y >= height)
            {
                ...
                if (conn && IsHiddenItemPresentInConnection(conn, x, y) == 1)
...
static void SetDistanceOfClosestHiddenItem(u8 taskId, s16 itemDistanceX, s16 itemDistanceY)
{
    ...
    if (data[2] == 0)
    {
        ...
        data[0] = itemDistanceX;
        data[1] = itemDistanceY;
        data[2] = 1;
    }
    else
    {
        ...
        if (data[0] < 0)
            oldItemAbsX = data[0] * -1;
        else
            oldItemAbsX = data[0];


        if (data[1] < 0)
            oldItemAbsY = data[1] * -1;
        else
            oldItemAbsY = data[1];
        ...
        {
            if (oldItemAbsX + oldItemAbsY == newItemAbsX + newItemAbsY
            && (oldItemAbsY > newItemAbsY || (oldItemAbsY == newItemAbsY && data[1] < itemDistanceY)))
            {
...
static void Task_HiddenItemNearby(u8 taskId)
{
    if (ObjectEventCheckHeldMovementStatus(&gObjectEvents[GetObjectEventIdByLocalIdAndMap(0xFF, 0, 0)]) == 1)
...
{
    if (IsPlayerFacingEmptyBerryTreePatch() == 1)
    {
...
{
    if (gSpecialVar_ItemId >= 682)
        DisplayItemMessage(taskId, 1, gText_BootedUpHM, BootUpSoundTMHM);
    else
        DisplayItemMessage(taskId, 1, gText_BootedUpTM, BootUpSoundTMHM);
}
...
static void Task_UseLure(u8 taskId)
{
    if (!IsSEPlaying())
    {
        VarSet(0x404E, ItemId_GetHoldEffectParam(gSpecialVar_ItemId));
        RemoveUsedItem();
        if (!InBattlePyramid())
            DisplayItemMessage(taskId, 1, gStringVar4, BagMenu_InitListsMenu);
        else
...
{
    if (CanUseDigOrEscapeRopeOnCurMap() == 1)
    {
...
u32 CanThrowBall(void)
{
    ...
    else if (gBattlerInMenuId == GetBattlerAtPosition(2)
             && IsBattlerAlive(GetBattlerAtPosition(0)))
    {
        return 2;
    }
    else if (IsPlayerPartyAndPokemonStorageFull() == 1)
    {
        return 3;
    }
...
void ItemUseInBattle_StatIncrease(u8 taskId)
{
    ...
    if (ExecuteTableBasedItemEffect(&gPlayerParty[partyId], gSpecialVar_ItemId, partyId, 0) != 0)
    {
        if (!InBattlePyramid())
