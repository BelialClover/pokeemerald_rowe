# 0 "src/pokenav_region_map.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokenav_region_map.c"
...
# 13 "src/pokenav_region_map.c" 2
# 1 "include/task.h" 1
# 14 "src/pokenav_region_map.c" 2
...
# 15 "src/pokenav_region_map.c" 2

# 1 "include/constants/rgb.h" 1
# 17 "src/pokenav_region_map.c" 2
# 1 "include/constants/songs.h" 1
# 18 "src/pokenav_region_map.c" 2
# 1 "include/constants/region_map_sections.h" 1
# 19 "src/pokenav_region_map.c" 2
...
# 1 "src/data/region_map/city_map_tilemaps.h" 1
...
# 1 "src/data/region_map/city_map_entries.h" 1
static const struct CityMapEntry sPokenavCityMaps[22] =
{
    {
        .mapSecId = 0x00,
        .index = 0,
...
# 149 "src/pokenav_region_map.c" 2
...
static u32 LoopedTask_OpenRegionMap(s32 taskState)
{
    ...
    {
    case 0:
        SetVBlankCallback_(((void *)0));
        HideBg(1);
        HideBg(2);
        HideBg(3);
        SetBgMode(1);
        InitBgTemplates(sRegionMapBgTemplates, (size_t)(sizeof(sRegionMapBgTemplates) / sizeof((sRegionMapBgTemplates)[0])) - 1);
...
{
    int i;
    for (i = 0; i < (size_t)(sizeof(sCityZoomTextSpriteSheet) / sizeof((sCityZoomTextSpriteSheet)[0])); i++)
...
static void FreeCityZoomViewGfx(void)
{
    ...
    for (i = 0; i < (int)(size_t)(sizeof(state->cityZoomTextSprites) / sizeof((state->cityZoomTextSprites)[0])); i++)
        DestroySprite(state->cityZoomTextSprites[i]);
}

static void LoadPokenavRegionMapGfx(struct Pokenav5Struct_2 *state)
{
    sub_8199DF0(1, ((0) | ((0) << 4)), 0x40, 1);
    sub_8199DF0(1, ((1) | ((1) << 4)), 0x41, 1);
    { vu16 tmp = (vu16)(0x1040); CpuSet((void *)&tmp, state->tilemapBuffer, 0x00000000 | 0x01000000 | ((0x800)/(16/8) & 0x1FFFFF)); };
    ...
    FillWindowPixelBuffer(state->infoWindowId, ((1) | ((1) << 4)));
...
static void UpdateMapSecInfoWindow(struct Pokenav5Struct_2 *state)
{
    ...
    case MAPSECTYPE_CITY_CANFLY:
        ...
        AddTextPrinterParameterized(state->infoWindowId, 7, regionMap->mapSecName, 0, 1, 0xFF, ((void *)0));
        DrawCityMap(state, regionMap->mapSecId, regionMap->posWithinMapSec);
        CopyWindowToVram(state->infoWindowId, 3);
        SetCityZoomTextInvisibility(0);
        break;
...
static void PrintLandmarkNames(struct Pokenav5Struct_2 *state, int mapSecId, int pos)
{
    ...
    while (1)
    {
        ...
        StringCopyPadded(gStringVar1, landmarkName, 0x00, 12);
        AddTextPrinterParameterized(state->infoWindowId, 7, gStringVar1, 0, i * 16 + 17, 0xFF, ((void *)0));
        i++;
    }
}
