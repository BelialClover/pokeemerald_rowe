...
# 35 "src/cable_club.c" 2
# 1 "include/constants/songs.h" 1
# 36 "src/cable_club.c" 2
...
static void CreateLinkupTask(u8 minPlayers, u8 maxPlayers)
{
    if (FindTaskIdByFunc(Task_LinkupStart) == 0xFF)
    {
        ...
        gTasks[taskId1].data[1] = minPlayers;
        gTasks[taskId1].data[2] = maxPlayers;
    }
}
...
static void UpdateLinkPlayerCountDisplay(u8 taskId, u8 numPlayers)
{
    ...
    if (numPlayers != data[3])
    {
        if (numPlayers <= 1)
            ...
            PrintNumPlayersInLink(data[5], numPlayers);
        data[3] = numPlayers;
    }
}
...
static void Task_LinkupAwaitConnection(u8 taskId)
{
    ...
    if (CheckLinkCanceledBeforeConnection(taskId) == 1
     || CheckLinkCanceled(taskId) == 1
     || playerCount < 2)
        return;
...
static void Task_LinkupConfirm(u8 taskId)
{
    ...
    if (CheckLinkErrored(taskId) == 1
     || TryLinkTimeout(taskId) == 1)
        return;
    ...
    if (*linkupStatus == 1)
    {
        if (gLinkType == 0x2266 || gLinkType == 0x2277)
        {
            ...
            else
            {
                ...
                ClearLinkPlayerCountWindow(gTasks[taskId].data[5]);
...
u8 CreateTask_ReestablishCableClubLink(void)
{
    if (FuncIsActiveTask(Task_ReestablishLink) != 0)
        return 0xFF;
