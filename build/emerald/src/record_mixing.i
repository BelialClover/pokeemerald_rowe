# 0 "src/record_mixing.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/record_mixing.c"
...
# 4 "src/record_mixing.c" 2
# 1 "include/constants/items.h" 1
# 5 "src/record_mixing.c" 2
# 1 "gflib/text.h" 1
# 272 "gflib/text.h"
enum
{
...
# 10 "src/record_mixing.c" 2
# 1 "include/pokemon.h" 1
# 11 "src/record_mixing.c" 2
...
# 21 "src/record_mixing.c" 2
# 1 "include/constants/songs.h" 1
# 22 "src/record_mixing.c" 2
# 1 "include/menu.h" 1
# 26 "include/menu.h"
enum
{
...
# 35 "src/record_mixing.c" 2
# 1 "include/constants/battle_frontier.h" 1
# 36 "src/record_mixing.c" 2
...
{
    struct RankingHall1P hallRecords1P[9][2][6];
...
static __attribute__((section("ewram_data"))) struct RecordMixingDayCareMail gUnknown_02039F9C = {0};
static __attribute__((section("ewram_data"))) union PlayerRecords *sReceivedRecords = ((void *)0);
static __attribute__((section("ewram_data"))) union PlayerRecords *sSentRecord = ((void *)0);
...
static void PrintTextOnRecordMixing(const u8 *src)
{
    ...
    AddTextPrinterParameterized(0, 1, src, 0, 1, 0, ((void *)0));
...
static void Task_RecordMixing_Main(u8 taskId)
{
    ...
    {
    case 0:
        sSentRecord = Alloc(sizeof(union PlayerRecords));
        sReceivedRecords = Alloc(sizeof(union PlayerRecords) * 4);
        ...
        data[15] = CreateTask(Task_RecordMixing_SoundEffect, 81);
        break;
    case 1:
...
static void Task_SendPacket(u8 taskId)
{
    ...
    case 0:
        {
            void *recordData = LoadPtrFromTaskData(&task->data[2]) + task->data[4] * 200;
...
static void Task_CopyReceiveBuffer(u8 taskId)
{
    ...
    if (status == GetLinkPlayerCountAsBitFlags())
    {
        ...
        for (i = 0; i < GetLinkPlayerCount(); i++)
        {
            ...
            if ((status >> i) & 1)
            {
                dest = LoadPtrFromTaskData((u16 *)&task->data[5]) + task->data[i + 1] * 200 + sRecordStructSize * i;
                src = GetPlayerRecvBuffer(i);
                if ((task->data[i + 1] + 1) * 200 > sRecordStructSize)
                    memcpy(dest, src, sRecordStructSize - task->data[i + 1] * 200);
                else
                    memcpy(dest, src, 200);
                ResetBlockReceivedFlag(i);
                task->data[i + 1]++;
                if (task->data[i + 1] == sRecordStructSize / 200 + 1)
                    handledPlayers++;
            }
        }
...
static void ReceiveBattleTowerData(void *battleTowerRecord, size_t recordSize, u8 which)
{
    ...
    if (Link_AnyPartnersPlayingRubyOrSapphire())
    {
        if (RubyBattleTowerRecordToEmerald((void *)battleTowerRecord + recordSize * mixIndices[which], (void *)battleTowerRecord + recordSize * which) == 1)
        {
...
static void ReceiveLilycoveLadyData(LilycoveLady *lilycoveLady, size_t recordSize, u8 which)
{
    ...
    {
        dest = Alloc(sizeof(LilycoveLady));
        if (dest == ((void *)0))
            return;
    ...
    for (i = 0; i < GetLinkPlayerCount(); i++)
    {
        ...
        for (j = 0; j < _src->numDaycareMons; j ++)
        {
            ...
            if (anyRS)
            {
                ...
                if (recordMixingMail->monName[0] == 0xFC && recordMixingMail->monName[1] == 0x15)
                {
            ...
            else if (language == 1)
            {
                if (IsStringJapanese(recordMixingMail->OT_name))
                    recordMixingMail->gameLanguage = 1;
                else
                    recordMixingMail->gameLanguage = (2);

                if (IsStringJapanese(recordMixingMail->monName))
                    recordMixingMail->monLanguage = 1;
                else
                    recordMixingMail->monLanguage = (2);
            }
        }
    }
...
static void ReceiveGiftItem(u16 *item, u8 which)
{
    if (which != 0 && *item != 0 && GetPocketByItemId(*item) == 10)
    {
...
static void ReceiveApprenticeData(struct Apprentice *mixApprentice, size_t recordSize, u32 multiplayerId)
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (mixApprenticePtr[i].playerName[0] != 0xFF && !IsApprenticeAlreadySaved(&mixApprenticePtr[i], &gSaveBlock2Ptr->apprentices[0]))
        {
            numApprentices++;
            apprenticeId = i;
        }
