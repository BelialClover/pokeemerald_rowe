# 0 "src/contest_util.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/contest_util.c"
...
# 4 "src/contest_util.c" 2
# 1 "include/battle_gfx_sfx_util.h" 1
# 5 "src/contest_util.c" 2
# 1 "gflib/bg.h" 1
...
# 8 "src/contest_util.c" 2
# 1 "include/contest_link.h" 1
...
# 22 "src/contest_util.c" 2
# 1 "include/main.h" 1
# 23 "src/contest_util.c" 2
...
# 24 "src/contest_util.c" 2
# 1 "include/palette.h" 1
# 25 "src/contest_util.c" 2
...
# 26 "src/contest_util.c" 2
# 1 "include/pokemon.h" 1
# 27 "src/contest_util.c" 2
...
# 36 "src/contest_util.c" 2
# 1 "include/task.h" 1
# 37 "src/contest_util.c" 2
...
# 41 "src/contest_util.c" 2

# 1 "include/constants/event_objects.h" 1
# 43 "src/contest_util.c" 2
# 1 "include/constants/field_specials.h" 1
# 44 "src/contest_util.c" 2
# 1 "include/constants/game_stat.h" 1
# 45 "src/contest_util.c" 2
# 1 "include/constants/rgb.h" 1
# 46 "src/contest_util.c" 2
# 1 "include/constants/songs.h" 1
# 47 "src/contest_util.c" 2
# 1 "include/constants/tv.h" 1
# 48 "src/contest_util.c" 2
...
struct ContestResults
{
    ...
    struct ContestMonResults (*monResults)[4];
    u8 *unusedBg;
    u8 *tilemapBuffers[4];
    u8 *unused;
};

static __attribute__((section("ewram_data"))) struct ContestResults *sContestResults = ((void *)0);
...
static const u8 sContestLinkTextColors[4] = {0x1, 0xF, 0xE};
...
static void InitContestResultsDisplay(void)
{
    ...
    for (i = 0; i < (int)(size_t)(sizeof(sContestResults->tilemapBuffers) / sizeof((sContestResults->tilemapBuffers)[0])); i++)
    ...
    SetGpuReg(0x4a, (1 << 1) | (1 << 2) | (1 << 3) | (1 << 5) | ((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13));
    SetGpuReg(0x40, 0);
    SetGpuReg(0x44, 0);
    SetGpuReg(0x42, 0);
    SetGpuReg(0x46, 0);
    ...
    SetGpuReg(0x1c, 0);
    SetGpuReg(0x1e, 0);
    SetGpuRegBits(0x0, 0x0F00 | 0x1000 | 0x2000 | 0x4000 | 0x8000);
    gBattle_BG0_X = 0;
    gBattle_BG0_Y = 0;
    gBattle_BG1_X = 0;
    gBattle_BG1_Y = 0;
...
static void LoadContestResultsBgGfx(void)
{
    ...
    LZDecompressVram(gContestResults_Gfx, (void *)(0x6000000 + (0x4000 * (0))));
    ...
    for (i = 0; i < 4; i++)
    {
        numStars = GetNumPreliminaryPoints(i, 1);
        round2Points = GetNumRound2Points(i, 1);
        for (j = 0; j < 10; j++)
        {
            tile1 = 0x60B2;
            if (j < numStars)
...
static void Task_ShowWinnerMonBanner(u8 taskId)
{
    ...
    case 5:
        if (sContestResults->data->winnerMonSlidingState == SLIDING_MON_EXITED)
        {
            sContestResults->data->destroyConfetti = 1;
            gTasks[taskId].data[0] = 0;
...
static void Task_EndShowContestResults(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            BlendPalettes(0x0000FFFF, 16, ((0) | ((0) << 5) | ((0) << 10)));
            gTasks[taskId].data[1]++;
        }
        else if (gTasks[taskId].data[1] == 1)
        {
            BlendPalettes(0xFFFF0000, 16, ((0) | ((0) << 5) | ((0) << 10)));
            gTasks[taskId].data[1]++;
        }
        else
        {
...
static void Task_FlashStarsAndHearts(u8 taskId)
{
    if (++gTasks[taskId].data[0] == 2)
    {
        ...
        BlendPalette(0x6B, 1, gTasks[taskId].data[1], ((30) | ((22) << 5) | ((11) << 10)));
        BlendPalette(0x68, 1, gTasks[taskId].data[1], ((31) | ((31) << 5) | ((31) << 10)));
        BlendPalette(0x6E, 1, gTasks[taskId].data[1], ((30) | ((29) << 5) | ((29) << 10)));
    }

    if (gTasks[taskId].data[1] == 0)
        sContestResults->data->pointsFlashing = 0;
    else
        sContestResults->data->pointsFlashing = 1;
}
...
static void LoadContestMonIcon(u16 species, u8 monIndex, u8 srcOffset, u8 useDmaNow, u32 personality, u8 formId)
{
    ...
    iconPtr = GetMonIconPtr(species, personality, frameNum, formId);
    iconPtr += srcOffset * 0x200 + 0x80;
    if (useDmaNow)
    {
        RequestDma3Copy(iconPtr, (void *)(0x6000000 + (0x4000 * (1))) + monIndex * 0x200, 0x180, 1);
        var0 = ((monIndex + 10) << 12);
...
static void TryCreateWirelessSprites(void)
{
    ...
    if (gLinkContestFlags & (1 << 1))
    {
        ...
        RequestDma3Fill(0xFFFFFFFF, (void *)(0x6000000 + (0x4000 * (4))) + sheet * 0x20, 0x80, 1);
...
static s32 DrawResultsTextWindow(const u8 *text, u8 spriteId)
{
    ...
    AddTextPrinterParameterized3(windowId, 1, (strWidth * 8 - origWidth) / 2, 1, sContestLinkTextColors, -1, text);
    {
        ...
        spriteTilePtrs[0] = (u8 *)(sprite->oam.tileNum * 32 + (0x6000000 + 0x10000));

        for (i = 1; i < 4; i++)
            spriteTilePtrs[i] = (void*)(gSprites[sprite->data[i - 1]].oam.tileNum * 32 + (0x6000000 + 0x10000));

        for (i = 0; i < 4; i++)
            { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, spriteTilePtrs[i], 0x04000000 | 0x01000000 | ((0x400)/(32/8) & 0x1FFFFF)); };

        dst = spriteTilePtrs[0];
        CpuSet(src, dst, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 128, dst + 0x100, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 128, dst + 0x200, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 64, dst + 0x300, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));

        for (i = 0; i < strWidth; i++)
        {
            dst = &spriteTilePtrs[(i + 1) / 8][((i + 1) % 8) * 32];
            CpuSet(src + 192, dst, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            CpuSet(windowTilesPtr, dst + 0x100, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            CpuSet(windowTilesPtr + 960, dst + 0x200, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            CpuSet(src + 224, dst + 0x300, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
            windowTilesPtr += 0x20;
        }

        dst = &spriteTilePtrs[(i + 1) / 8][((i + 1) % 8) * 32];
        CpuSet(src + 32, dst, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 160, dst + 0x100, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 160, dst + 0x200, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
        CpuSet(src + 96, dst + 0x300, 0x04000000 | ((0x20)/(32/8) & 0x1FFFFF));
    }
...
static void LoadContestResultSprites(void)
{
    ...
    u8 spriteIds[(size_t)(sizeof(sUnknown_0858D810) / sizeof((sUnknown_0858D810)[0]))];

    template = sSpriteTemplate_858D7F8;
    for (i = 0; i < (int)(size_t)(sizeof(sUnknown_0858D810) / sizeof((sUnknown_0858D810)[0])); i++)
...
static void StartTextBoxSlideIn(s16 x, u16 y, u16 slideOutTimer, u16 slideIncrement)
{
    ...
    sprite->pos2.y = 0;
    sprite->data[4] = x + 32;
    sprite->data[5] = slideOutTimer;
    sprite->data[6] = slideIncrement;
    sprite->data[7] = 0;
...
static void Task_HighlightWinnersBox(u8 taskId)
{
    if (++gTasks[taskId].data[11] == 1)
    {
        ...
        BlendPalette(0x91, 1, gTasks[taskId].data[12], ((13) | ((28) << 5) | ((27) << 10)));
...
static void Task_BounceMonIconInBox(u8 taskId)
{
    ...
    {
        gTasks[taskId].data[10] = 0;
        LoadContestMonIcon(gTasks[taskId].data[2], monIndex, gTasks[taskId].data[11], 0, gContestMons[monIndex].personality, gContestMons[monIndex].formId);
...
static void CalculateContestantsResultData(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (gContestMonRound2Points[i] < 0)
            (*sContestResults->monResults)[i].lostPoints = 1;
        ...
        (*sContestResults->monResults)[i].numStars = GetNumPreliminaryPoints(i, 1);
        round2Points = GetNumRound2Points(i, 1);
        ...
        if (gContestFinalStandings[i])
        {
            ...
            if (barLengthPreliminary + barLengthRound2 == 87 + 1)
            {
    ...
    if (!isRound2)
    {
        for (i = 0; i < 4; i++)
        {
            ...
            if (numUpdates < numStars)
            {
                ...
                gTasks[taskId].data[1] = target >> 16;
    ...
    else
    {
        for (i = 0; i < 4; i++)
        {
            ...
            if (numUpdates < numHearts)
            {
                ...
                if ((*sContestResults->monResults)[i].lostPoints)
                    gTasks[taskId].data[1] = -(target >> 16) + (*sContestResults->monResults)[i].barLengthPreliminary;
                else
                    gTasks[taskId].data[1] = (target >> 16) + (*sContestResults->monResults)[i].barLengthPreliminary;
...
{
    int i;
    bool32 minMaxReached = 0;
    bool32 targetReached = 0;
    u8 monId = gTasks[taskId].data[0];
    s16 target = gTasks[taskId].data[1];
    s16 decreasing = gTasks[taskId].data[2];
    ...
    {
        if (sContestResults->data->unkC[monId] > 87)
            minMaxReached = 1;
    }
...
static void AllocContestResults(void)
{
    ...
    sContestResults->unusedBg = AllocZeroed(0x800);
    sContestResults->tilemapBuffers[0] = AllocZeroed(0x800);
    sContestResults->tilemapBuffers[1] = AllocZeroed(0x800);
    sContestResults->tilemapBuffers[2] = AllocZeroed(0x800);
    sContestResults->tilemapBuffers[3] = AllocZeroed(0x800);
...
static void FreeContestResults(void)
{
    { Free(sContestResults->data); sContestResults->data = ((void *)0); };
    { Free(sContestResults->monResults); sContestResults->monResults = ((void *)0); };
    { Free(sContestResults->unusedBg); sContestResults->unusedBg = ((void *)0); };
    { Free(sContestResults->tilemapBuffers[0]); sContestResults->tilemapBuffers[0] = ((void *)0); };
    { Free(sContestResults->tilemapBuffers[1]); sContestResults->tilemapBuffers[1] = ((void *)0); };
    { Free(sContestResults->tilemapBuffers[2]); sContestResults->tilemapBuffers[2] = ((void *)0); };
    { Free(sContestResults->tilemapBuffers[3]); sContestResults->tilemapBuffers[3] = ((void *)0); };
    { Free(sContestResults->unused); sContestResults->unused = ((void *)0); };
    { Free(sContestResults); sContestResults = ((void *)0); };
    FreeMonSpritesGfx();
}
...
u16 HasMonWonThisContestBefore(void)
{
    ...
    {
    case 0:
        if (GetMonData(mon, 50) > gSpecialVar_ContestRank)
            hasRankRibbon = 1;
        break;
    case 1:
        if (GetMonData(mon, 51) > gSpecialVar_ContestRank)
            hasRankRibbon = 1;
        break;
    case 2:
        if (GetMonData(mon, 52) > gSpecialVar_ContestRank)
            hasRankRibbon = 1;
        break;
    case 3:
        if (GetMonData(mon, 53) > gSpecialVar_ContestRank)
            hasRankRibbon = 1;
        break;
    case 4:
        if (GetMonData(mon, 54) > gSpecialVar_ContestRank)
            hasRankRibbon = 1;
        break;
    }
...
void GiveMonContestRibbon(void)
{
    ...
    {
    case 0:
        ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex], 50);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData < 4)
        {
            ribbonData++;
            SetMonData(&gPlayerParty[gContestMonPartyIndex], 50, &ribbonData);
            if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
                TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 50);
        }
        break;
    case 1:
        ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex], 51);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData < 4)
        {
            ribbonData++;
            SetMonData(&gPlayerParty[gContestMonPartyIndex], 51, &ribbonData);
            if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
                TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 51);
        }
        break;
    case 2:
        ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex], 52);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData < 4)
        {
            ribbonData++;
            SetMonData(&gPlayerParty[gContestMonPartyIndex], 52, &ribbonData);
            if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
                TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 52);
        }
        break;
    case 3:
        ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex], 53);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData < 4)
        {
            ribbonData++;
            SetMonData(&gPlayerParty[gContestMonPartyIndex], 53, &ribbonData);
            if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
                TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 53);
        }
        break;
    case 4:
        ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex], 54);
        if (ribbonData <= gSpecialVar_ContestRank && ribbonData < 4)
        {
            ribbonData++;
            SetMonData(&gPlayerParty[gContestMonPartyIndex], 54, &ribbonData);
            if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
                TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 54);
        }
        break;
    }
}
...
void sub_80F8970(void)
{
    ...
    for (i = 0; i < 4 - 1; i++)
    {
        for (j = 4 - 1; j > i; j--)
        {
            ...
            {
                int temp;
                { temp = conditions[j]; conditions[j] = conditions[j - 1]; conditions[j - 1] = temp; }
            }
        }
    }
...
void LoadLinkContestPlayerPalettes(void)
{
    ...
    int version;
    struct Sprite *sprite;
    static const u8 sContestantLocalIds[4] = { 3, 4, 5, 14 };
...
bool8 GiveMonArtistRibbon(void)
{
    ...
    hasArtistRibbon = GetMonData(&gPlayerParty[gContestMonPartyIndex], 70);
    if (!hasArtistRibbon
    ...
    {
        hasArtistRibbon = 1;
        SetMonData(&gPlayerParty[gContestMonPartyIndex], 70, &hasArtistRibbon);
        if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) > 4)
            TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex], 70);
...
void ShowContestEntryMonPic(void)
{
    ...
    if (FindTaskIdByFunc(Task_ShowContestEntryMonPic) == 0xFF)
    {
        ...
        {
            if (!(gLinkContestFlags & (1 << 2)))
                DoMonFrontSpriteAnimation(&gSprites[spriteId], formSpeciesId, 0, 0);
        }
        else
        {
            DoMonFrontSpriteAnimation(&gSprites[spriteId], formSpeciesId, 0, 0);
