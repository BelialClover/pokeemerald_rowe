...
static const struct BikeHistoryInputInfo sAcroBikeTricksList[] =
{
    ...
    {1, 0x0002, 0xF, 0xF, sAcroBikeJumpTimerList, sAcroBikeJumpTimerList, 1},
    {2, 0x0002, 0xF, 0xF, sAcroBikeJumpTimerList, sAcroBikeJumpTimerList, 2},
    {3, 0x0002, 0xF, 0xF, sAcroBikeJumpTimerList, sAcroBikeJumpTimerList, 3},
    {4, 0x0002, 0xF, 0xF, sAcroBikeJumpTimerList, sAcroBikeJumpTimerList, 4},
};
...
static u8 AcroBikeHandleInputNormal(u8 *newDirection, u16 newKeys, u16 heldKeys)
{
    ...
    if (*newDirection == direction && (heldKeys & 0x0002) && gPlayerAvatar.bikeSpeed == SPEED_STANDING)
    {
...
static void AcroBikeTransition_SideJump(u8 direction)
{
    ...
    if (collision)
    {
        ...
        if (WillPlayerCollideWithCollision(collision, direction) == 0)
        {
            AcroBikeTransition_TurnDirection(direction);
            return;
        }
...
static void AcroBike_TryHistoryUpdate(u16 newKeys, u16 heldKeys)
{
    ...
    direction = heldKeys & (0x0001 | 0x0002 | 0x0004 | 0x0008);
...
static u8 AcroBike_GetJumpDirection(void)
{
    ...
    for (i = 0; i < (size_t)(sizeof(sAcroBikeTricksList) / sizeof((sAcroBikeTricksList)[0])); i++)
    {
...
static void Bike_UpdateDirTimerHistory(u8 dir)
{
    ...
    for (i = (size_t)(sizeof(gPlayerAvatar.dirTimerHistory) / sizeof((gPlayerAvatar.dirTimerHistory)[0])) - 1; i != 0; i--)
...
static void Bike_UpdateABStartSelectHistory(u8 input)
{
    ...
    for (i = (size_t)(sizeof(gPlayerAvatar.abStartSelectTimerHistory) / sizeof((gPlayerAvatar.abStartSelectTimerHistory)[0])) - 1; i != 0; i--)
...
bool8 RS_IsRunningDisallowed(u8 tile)
{
    if (IsRunningDisallowedByMetatile(tile) != 0 || gMapHeader.mapType == 8)
        return 1;
    else
        return 0;
}
...
{
    if (gBikeCyclingChallenge != 0 && gBikeCollisions < 100)
        gBikeCollisions++;
}
...
bool8 player_should_look_direction_be_enforced_upon_movement(void)
{
    if (TestPlayerAvatarFlags((1 << 2)) != 0 && MetatileBehavior_IsBumpySlope(gObjectEvents[gPlayerAvatar.objectEventId].currentMetatileBehavior) != 0)
        return 0;
    else
        return 1;
}
...
void GetOnOffBike(u8 transitionFlags)
{
    gUnusedBikeCameraAheadPanback = 0;
    ...
    else
    {
        ...
        Overworld_SetSavedMusic(374);
        Overworld_ChangeMusicTo(374);
    }
}
...
void BikeClearState(int newDirHistory, int newAbStartHistory)
{
    ...
    for (i = 0; i < (size_t)(sizeof(gPlayerAvatar.dirTimerHistory) / sizeof((gPlayerAvatar.dirTimerHistory)[0])); i++)
        gPlayerAvatar.dirTimerHistory[i] = 0;

    for (i = 0; i < (size_t)(sizeof(gPlayerAvatar.abStartSelectTimerHistory) / sizeof((gPlayerAvatar.abStartSelectTimerHistory)[0])); i++)
