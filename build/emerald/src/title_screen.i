# 0 "src/title_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/title_screen.c"
...
# 6 "src/title_screen.c" 2
# 1 "include/clear_save_data_menu.h" 1
...
static void SpriteCB_VersionBannerLeft(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        if (sprite->data[0] != 0)
            sprite->data[0]--;
        SetGpuReg(0x52, gIntroWaterDropAlphaBlend[sprite->data[0]]);
    }
}
...
static void SpriteCB_PokemonLogoShine(struct Sprite *sprite)
{
    if (sprite->pos1.x < 272)
    {
        if (sprite->data[0])
        {
            ...
            backgroundColor = ((((sprite->data[1]) & 0x1F) << 10) + (((sprite->data[1]) & 0x1F) << 5) + ((sprite->data[1]) & 0x1F));
            if (sprite->pos1.x == 240 / 2 + 12
                || sprite->pos1.x == 240 / 2 + 16
                || sprite->pos1.x == 240 / 2 + 20
                || sprite->pos1.x == 240 / 2 + 24)
                gPlttBufferFaded[0] = ((24) | ((31) << 5) | ((12) << 10));
            else
...
void CB2_InitTitleScreen(void)
{
    ...
    case 4:
        ...
        SetGpuReg(0x28, -29 * 256);
        SetGpuReg(0x2a, -1);
        SetGpuReg(0x2c, -32 * 256);
        SetGpuReg(0x2e, -1);
        SetGpuReg(0x40, 0);
        SetGpuReg(0x44, 0);
        SetGpuReg(0x42, 0);
        SetGpuReg(0x46, 0);
        SetGpuReg(0x48, 0x1F1F);
        SetGpuReg(0x4a, 0x3F1F);
        SetGpuReg(0x50, (1 << 2) | (2 << 6));
        SetGpuReg(0x52, 0);
        SetGpuReg(0x54, 0xC);
        SetGpuReg(0x8, (3) | ((2) << 2) | ((26) << 8) | 0x0000 | 0x0000);
        SetGpuReg(0xa, (2) | ((3) << 2) | ((27) << 8) | 0x0000 | 0x0000);
        SetGpuReg(0xc, (1) | ((0) << 2) | ((9) << 8) | 0x0080 | 0x4000);
    ...
    case 5:
        if (!UpdatePaletteFade())
        {
            ...
            ScanlineEffect_InitWave(0, 160, 4, 4, 0, ((0x4000000 + 0x14) - (0x4000000 + 0x10)), 1);
...
static void Task_TitleScreenPhase1(u8 taskId)
{
    ...
    if ((gMain.newKeys & (0x0001 | 0x0002 | 0x0008 | 0x0004)) || gTasks[taskId].data[1] != 0)
    {
  gTasks[taskId].data[1] = 1;
    ...
    {
        u16 frameNum = gTasks[taskId].data[0];
        if (frameNum == 176)
    ...
    else
    {
        ...
        spriteId = CreateSprite(&sVersionBannerLeftSpriteTemplate, 98, 2, 0);
        gSprites[spriteId].data[0] = 64;
        gSprites[spriteId].data[1] = taskId;


        spriteId = CreateSprite(&sVersionBannerRightSpriteTemplate, 162, 2, 0);
        gSprites[spriteId].data[1] = taskId;
...
static void Task_TitleScreenPhase2(u8 taskId)
{
    ...
    if ((gMain.newKeys & (0x0001 | 0x0002 | 0x0008 | 0x0004)) || gTasks[taskId].data[1])
    {
        gTasks[taskId].data[1] = 1;
        gTasks[taskId].data[0] = 0;
    }
    ...
    {
        gTasks[taskId].data[1] = 1;
        SetGpuReg(0x50, (1 << 1) | (1 << 6) | (1 << 8) | (1 << 13));
        SetGpuReg(0x52, (((15) << 8) | (6)));
        SetGpuReg(0x54, 0);
        SetGpuReg(0x0, 0x0001
                                    | 0x0040
                                    | 0x0100
    ...
    if (!(gTasks[taskId].data[0] & 3) && gTasks[taskId].data[2] != 0)
...
static void Task_TitleScreenPhase3(u8 taskId)
{
    ...
    {
  PlayCryInternal(384, 0, 120, 10, 0);

        FadeOutBGM(4);
        BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 0x10, (((31) | ((31) << 5) | ((31) << 10)) | 0x8000));
        SetMainCallback2(CB2_GoToMainMenu);
    }
    else if (({(gMain.heldKeys) & ((0x0002 | 0x0004 | 0x0040));}) == (0x0002 | 0x0004 | 0x0040))
    {
        SetMainCallback2(CB2_GoToClearSaveDataScreen);
    }
    else if (({(gMain.heldKeys) & ((0x0002 | 0x0004 | 0x0020));}) == (0x0002 | 0x0004 | 0x0020)
      && CanResetRTC() == 1)
    {
        FadeOutBGM(4);
    ...
    else
    {
        ...
        UpdateLegendaryMarkingColor(gTasks[taskId].data[0]);
