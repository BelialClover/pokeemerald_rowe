...
static u8 FindFirstActiveTask(void)
{
    ...
    for (taskId = 0; taskId < 16; taskId++)
        if (gTasks[taskId].isActive == 1 && gTasks[taskId].prev == 0xFE)
            break;
...
void SetTaskFuncWithFollowupFunc(u8 taskId, TaskFunc func, TaskFunc followupFunc)
{
    { u32 tasksAddr = (u32)gTasks; u32 addr = taskId * sizeof(struct Task) + 28; u32 dataAddr = tasksAddr + ((size_t)&((struct Task *)0)->data); addr += dataAddr; *((u16 *)addr) = (u32)followupFunc; }
    { u32 tasksAddr = (u32)gTasks; u32 addr = taskId * sizeof(struct Task) + 30; u32 dataAddr = tasksAddr + ((size_t)&((struct Task *)0)->data); addr += dataAddr; *((u16 *)addr) = (u32)followupFunc >> 16; }
...
void SwitchTaskToFollowupFunc(u8 taskId)
{
    ...
    gTasks[taskId].func = ((void *)0);

    { u32 tasksAddr = (u32)gTasks; u32 addr = taskId * sizeof(struct Task) + 28; u32 dataAddr = tasksAddr + ((size_t)&((struct Task *)0)->data); addr += dataAddr; func = *((u16 *)addr); }
    { u32 tasksAddr = (u32)gTasks; u32 addr = taskId * sizeof(struct Task) + 30; u32 dataAddr = tasksAddr + ((size_t)&((struct Task *)0)->data); addr += dataAddr; func |= *((s16 *)addr) << 16; }
...
bool8 FuncIsActiveTask(TaskFunc func)
{
    ...
    for (i = 0; i < 16; i++)
        if (gTasks[i].isActive == 1 && gTasks[i].func == func)
            return 1;
