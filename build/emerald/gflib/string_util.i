# 0 "gflib/string_util.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "gflib/string_util.c"
...
__attribute__((section("ewram_data"))) u8 gStringVar4[0x3E8] = {0};
__attribute__((section("ewram_data"))) static u8 sUnknownStringVar[16] = {0};
...
const u8 *GetExpandedPlaceholder(u32 id)
{
    ...
    {
        [0x0] = ExpandPlaceholder_UnknownStringVar,
        [0x1] = ExpandPlaceholder_PlayerName,
        [0x2] = ExpandPlaceholder_StringVar1,
        [0x3] = ExpandPlaceholder_StringVar2,
        [0x4] = ExpandPlaceholder_StringVar3,
        [0x5] = ExpandPlaceholder_KunChan,
        [0x6] = ExpandPlaceholder_RivalName,
        [0x7] = ExpandPlaceholder_Version,
        [0x8] = ExpandPlaceholder_Aqua,
        [0x9] = ExpandPlaceholder_Magma,
        [0xA] = ExpandPlaceholder_Archie,
        [0xB] = ExpandPlaceholder_Maxie,
        [0xC] = ExpandPlaceholder_Kyogre,
        [0xD] = ExpandPlaceholder_Groudon,
    };

    if (id >= (size_t)(sizeof(funcs) / sizeof((funcs)[0])))
...
u8 GetExtCtrlCodeLength(u8 code)
{
    ...
    u8 length = 0;
    if (code < (size_t)(sizeof(lengths) / sizeof((lengths)[0])))
        length = lengths[code];
    return length;
}
...
char *ConvertToAscii(const u8 *str)
{
    ...
    for (i = 0; *str != 0xFF; i++, str++)
    {
        char modifiedCode = '?';
  if(*str >= 0xD5 && *str <= 0xEE)
        {
            modifiedCode = *str-(0xBB -'a');
        }
        else if(*str >= 0xBB && *str <= 0xD4)
        {
            modifiedCode = *str-(0xBB -'A');
        }
        else if (*str >= 0xA1 && *str <= 0xAA)
        {
            modifiedCode = *str-(0xA1 -'0');
        }
        else if (*str == 0x00)
        {
            modifiedCode = ' ';
