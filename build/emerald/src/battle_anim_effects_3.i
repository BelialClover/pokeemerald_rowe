...
# 1 "include/task.h" 1
# 19 "src/battle_anim_effects_3.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 20 "src/battle_anim_effects_3.c" 2
# 1 "include/util.h" 1
...
# 21 "src/battle_anim_effects_3.c" 2
# 1 "include/constants/battle_anim.h" 1
# 22 "src/battle_anim_effects_3.c" 2
# 1 "include/constants/rgb.h" 1
# 23 "src/battle_anim_effects_3.c" 2
# 1 "include/constants/songs.h" 1
# 24 "src/battle_anim_effects_3.c" 2
# 1 "include/constants/weather.h" 1
# 25 "src/battle_anim_effects_3.c" 2
...
void AnimTask_StockpileDeformMon(u8 taskId)
{
    ...
    {
        PrepareAffineAnimInTaskData(&gTasks[taskId], GetAnimBattlerSpriteId(0), gStockpileDeformMonAffineAnimCmds);
        gTasks[taskId].data[0]++;
    }
    else
    {
...
void AnimTask_SpitUpDeformMon(u8 taskId)
{
    if (!gTasks[taskId].data[0])
    {
        PrepareAffineAnimInTaskData(&gTasks[taskId], GetAnimBattlerSpriteId(0), gSpitUpDeformMonAffineAnimCmds);
        gTasks[taskId].data[0]++;
    }
    else
    {
...
void AnimTask_SwallowDeformMon(u8 taskId)
{
    if (!gTasks[taskId].data[0])
    {
        PrepareAffineAnimInTaskData(&gTasks[taskId], GetAnimBattlerSpriteId(0), gSwallowDeformMonAffineAnimCmds);
        gTasks[taskId].data[0]++;
    }
    else
    {
...
void AnimTask_TransformMon(u8 taskId)
{
    ...
    case 2:
        ...
        dest = animBg.bgTiles;
        CpuSet(src, dest, 0x04000000 | ((0x800)/(32/8) & 0x1FFFFF));
...
{
    HandleSpeciesGfxDataChange(gBattleAnimAttacker, gBattleAnimTarget, 1, 0);
    DestroyAnimVisualTask(taskId);
}
...
void AnimTask_MorningSunLightBeam(u8 taskId)
{
    ...
    case 0:
        ...
        gTasks[taskId].data[0]++;
        PlaySE12WithPanning(228, BattleAnimAdjustPanning(-64));
        break;
    case 1:
...
void AnimTask_StrongFrustrationGrowAndShrink(u8 taskId)
{
    ...
    {
        PrepareAffineAnimInTaskData(&gTasks[taskId], GetAnimBattlerSpriteId(0), gStrongFrustrationAffineAnimCmds);
        gTasks[taskId].data[0]++;
    }
    else
    {
...
void AnimTask_RolePlaySilhouette(u8 taskId)
{
        ...
        if (GetBattlerSide(gBattleAnimAttacker) != 0)
        {
            ...
            if (gBattleSpritesDataPtr->battlerData[gBattleAnimTarget].transformSpecies == 0)
            {
                if (GetBattlerSide(gBattleAnimTarget) == 0)
                {
                    ...
                    formId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimTarget]], 89);
                }
                else
                {
                    species = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimTarget]], 11);
                    formId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimTarget]], 89);
                }
            }
            else
            {
    ...
    spriteId = sub_80A8394(species, isBackPic, 0, coord1 + xOffset, coord2, 5, personality, otId, gBattleAnimTarget, 1, formId);
...
static void AnimMeteorMashStar(struct Sprite *sprite)
{
    ...
    if (GetBattlerSide(gBattleAnimTarget) == 0 || IsContest())
    {
...
static void AnimBlockX_Step(struct Sprite *sprite)
{
    ...
    case 0:
        ...
        if (sprite->pos2.y >= 0)
        {
            PlaySE12WithPanning(205, BattleAnimAdjustPanning(63));
            sprite->pos2.y = 0;
            sprite->data[0]++;
        }
        break;
...
void AnimTask_SnatchOpposingMonMove(u8 taskId)
{
    ...
    case 1:
        ...
        else
        {
            if (GetBattlerSide(gBattleAnimAttacker) == 0)
            {
                ...
                if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies == 0)
                {
                    ...
                    formId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]], 89);
                }
                else
                {
            ...
            else
            {
                ...
                if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies == 0)
                {
                    ...
                    formId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimAttacker]], 89);
                }
                else
                {
                    species = gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies;
        ...
        spriteId2 = sub_80A8394(species, isBackPic, 0, x, GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y), subpriority, personality, otId, gBattleAnimAttacker, 0, formId);
        if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies != 0)
...
static void sub_815FE80(struct Sprite *sprite)
{
    ...
    case 0:
        ...
        {
            PlaySE12WithPanning(122, BattleAnimAdjustPanning(63));
            sprite->pos1.y = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y) + 16;
            sprite->data[0] = -32;
            sprite->data[7]++;
            sprite->invisible = 0;
            if (GetBattlerSide(gBattleAnimAttacker) == 1 && !IsContest())
...
void AnimTask_GetWeather(u8 taskId)
{
    ...
    if (gWeatherMoveAnim & ((1 << 5) | (1 << 6)))
        ...
        gBattleAnimArgs[7] = 3;
    else if (gWeatherMoveAnim & ((1 << 7) | (1 << 8)))
        gBattleAnimArgs[7] = 4;
