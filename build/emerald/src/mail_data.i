...
bool8 MonHasMail(struct Pokemon *mon)
{
    u16 heldItem = GetMonData(mon, 12);
    if (ItemIsMail(heldItem) && GetMonData(mon, 64) != 0xFF)
        return 1;
    else
        return 0;
}
...
u8 TakeMailFromMon2(struct Pokemon *mon)
{
    ...
    for (i = 6; i < 16; i++)
    {
        ...
        {
            memcpy(&gSaveBlock1Ptr->mail[i], &gSaveBlock1Ptr->mail[GetMonData(mon, 64)], sizeof(struct MailStruct));
            gSaveBlock1Ptr->mail[GetMonData(mon, 64)].itemId = 0;
            SetMonData(mon, 64, &newMailId);
            SetMonData(mon, 12, newHeldItem);
            return i;
        }
    }
