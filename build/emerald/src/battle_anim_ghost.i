# 0 "src/battle_anim_ghost.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_anim_ghost.c"
...
# 6 "src/battle_anim_ghost.c" 2
# 1 "include/constants/rgb.h" 1
# 7 "src/battle_anim_ghost.c" 2
...
# 8 "src/battle_anim_ghost.c" 2
# 1 "include/constants/songs.h" 1
# 9 "src/battle_anim_ghost.c" 2
# 1 "include/sound.h" 1
...
# 12 "src/battle_anim_ghost.c" 2
# 1 "include/constants/moves.h" 1
# 13 "src/battle_anim_ghost.c" 2
...
{
    {.frame = {.xScale = 0x1E, .yScale = 0x1E, .rotation = 10, .duration = 5}},
    {.frame = {.xScale = 0xFFE2, .yScale = 0xFFE2, .rotation = 10, .duration = 5}},
...
static void sub_8111764(struct Sprite *sprite)
{
    ...
    if ((r0 & 0xFF) == 0)
    {
        ...
        SetGpuReg(0x52, (((16 - sprite->data[6]) << 8) | (sprite->data[6])));
...
static void AnimTask_NightShadeClone_Step1(u8 taskId)
{
    ...
    if (gTasks[taskId].data[10] == 3)
    {
        ...
        SetGpuReg(0x52, (((gTasks[taskId].data[3]) << 8) | (gTasks[taskId].data[2])));
...
void AnimTask_NightmareClone(u8 taskId)
{
    ...
    task->data[4] = 0;
    SetGpuReg(0x50, ((1 << 6) | (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13))));
    SetGpuReg(0x52, (((task->data[3]) << 8) | (task->data[2])));
...
static void AnimTask_SpiteTargetShadow_Step1(u8 taskId)
{
    ...
    case 0:
        ...
        else
        {
            ...
            else
            {
                ...
                gSprites[task->data[0]].oam.objMode = 0;
    ...
    case 1:
        ...
        BlendPalette(task->data[4], 16, 10, ((13) | ((0) << 5) | ((15) << 10)));
        task->data[15]++;
        break;
    case 2:
    ...
    case 3:
        if (position == 1)
            SetGpuReg(0x50, ((1 << 6) | (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13)) | (1 << 1)));
        else
            SetGpuReg(0x50, ((1 << 6) | (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13)) | (1 << 2)));
...
static void AnimTask_SpiteTargetShadow_Step3(u8 taskId)
{
    ...
    case 0:
        ...
        break;
    case 1:
        BlendPalette(task->data[4], 16, 0, ((13) | ((0) << 5) | ((15) << 10)));
        break;
    case 2:
...
void AnimTask_DestinyBondWhiteShadow(u8 taskId)
{
    ...
    if (!IsContest())
    {
        for (battler = 0; battler < 4; battler++)
        {
            ...
            {
                if (gAnimMoveIndex == 464)
                    spriteId = CreateSprite(&gDarkVoidBlackHoleTemplate, baseX, baseY, 55);
                else
    ...
    case 0:
        if (task->data[6] == 0)
        {
            if (++task->data[5] > 1)
            {
                ...
                SetGpuReg(0x52, (((task->data[9]) << 8) | (task->data[8])));
                if (task->data[7] >= 24)
                {
                    task->data[7] = 0;
                    task->data[6] = 1;
...
static void AnimTask_CurseStretchingBlackBg_Step1(u8 taskId)
{
    ...
    else
    {
        ...
        BeginNormalPaletteFade(selectedPalettes, 0, 16, 16, ((0) | ((0) << 5) | ((0) << 10)));
...
static void AnimCurseNail_Step2(struct Sprite *sprite)
{
    ...
    else
    {
        sprite->data[1] = 0;
        sprite->data[2]++;
        SetGpuReg(0x52, (16 - sprite->data[2]) | (sprite->data[2] << 8));
        if (sprite->data[2] == 16)
        {
            sprite->invisible = 1;
...
void AnimGhostStatusSprite(struct Sprite *sprite)
{
    ...
    else if (sprite->data[7] > 30)
    {
        ...
        SetGpuReg(0x52, (((coeffB) << 8) | (coeffA)));
