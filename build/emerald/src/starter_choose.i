...
static const u8 sTextColors[] = {0x0, 0x1, 0x3};
...
{
    {.frame = {.xScale = 20, .yScale = 20, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = 20, .yScale = 20, .rotation = 0, .duration = 15}},
    {.type = 0x7FFF},
};
...
void CB2_ChooseStarter(void)
{
    ...
    LZ77UnCompVram(gBirchBagTilemap, (void *)((0x6000000 + (0x800 * (6)))));
    ...
    SetGpuReg(0x40, 0);
    SetGpuReg(0x44, 0);
    SetGpuReg(0x50, (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5) | (3 << 6));
    SetGpuReg(0x52, 0);
    SetGpuReg(0x54, 7);
...
static void Task_StarterChoose(u8 taskId)
{
    CreateStarterPokemonLabel(gTasks[taskId].data[0]);
    DrawStdFrameWithCustomTileAndPalette(0, 0, 0x2A8, 0xD);
    AddTextPrinterParameterized(0, 1, gText_BirchInTrouble, 0, 1, 0, ((void *)0));
    PutWindowTilemap(0);
...
static void Task_HandleStarterChooseInput(u8 taskId)
{
    ...
    {
        u8 spriteId;
        u8 formId = GetFormIdFromFormSpeciesId(GetStarterPokemon(gTasks[taskId].data[0]));
        u16 baseSpecies = GetFormSpeciesId(GetStarterPokemon(gTasks[taskId].data[0]), 0);
    ...
    else if (({(gMain.newKeys) & (0x0020);}) && selection > 0)
    {
...
static void Task_WaitForStarterSprite(u8 taskId)
{
    if (gSprites[gTasks[taskId].data[2]].affineAnimEnded &&
        gSprites[gTasks[taskId].data[2]].pos1.x == (240 / 2) &&
...
static void Task_AskConfirmStarter(u8 taskId)
{
    PlayCry1(GetStarterPokemon(gTasks[taskId].data[0]), 0);
    FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
    AddTextPrinterParameterized(0, 1, gText_ConfirmStarterChoice, 0, 1, 0, ((void *)0));
    ScheduleBgCopyTilemapToVram(0);
...
static void CreateStarterPokemonLabel(u8 selection)
{
    ...
    FillWindowPixelBuffer(sStarterLabelWindowId, ((0) | ((0) << 4)));
    ...
    SetGpuReg(0x40, (((labelLeft) << 8) | (labelRight)));
    SetGpuReg(0x44, (((labelTop) << 8) | (labelBottom)));
}
...
static u8 CreatePokemonFrontSprite(u16 species, u8 x, u8 y, u8 formId)
{
    ...
    spriteId = CreatePicSprite2(species, 8, 0, 1, x, y, 0xE, 0xFFFF, formId);
...
static void SpriteCB_SelectionHand(struct Sprite *sprite)
{
    ...
    sprite->pos1.x = sCursorCoords[gTasks[sprite->data[0]].data[0]][0];
    sprite->pos1.y = sCursorCoords[gTasks[sprite->data[0]].data[0]][1];
