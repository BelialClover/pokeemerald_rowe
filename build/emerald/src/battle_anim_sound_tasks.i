# 0 "src/battle_anim_sound_tasks.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_anim_sound_tasks.c"
...
# 2 "src/battle_anim_sound_tasks.c" 2
...
# 3 "src/battle_anim_sound_tasks.c" 2
...
# 4 "src/battle_anim_sound_tasks.c" 2
...
# 5 "src/battle_anim_sound_tasks.c" 2
# 1 "include/m4a.h" 1
...
# 6 "src/battle_anim_sound_tasks.c" 2
# 1 "include/sound.h" 1
...
# 7 "src/battle_anim_sound_tasks.c" 2
# 1 "include/task.h" 1
# 8 "src/battle_anim_sound_tasks.c" 2
# 1 "include/constants/battle_anim.h" 1
# 9 "src/battle_anim_sound_tasks.c" 2
...
void SoundTask_PlayCryHighPitch(u8 taskId)
{
    ...
    else
    {
        ...
        if (gBattleAnimArgs[0] == 0)
            ...
            battlerId = ((gBattleAnimAttacker) ^ 2);
        else
            battlerId = ((gBattleAnimTarget) ^ 2);


        if ((gBattleAnimArgs[0] == 1 || gBattleAnimArgs[0] == 3) && !IsBattlerSpriteVisible(battlerId))
        {
            DestroyAnimVisualTask(taskId);
            return;
        }
...
void SoundTask_PlayCryWithEcho(u8 taskId)
{
    ...
    pan = BattleAnimAdjustPanning(-64);
...
# 406 "src/battle_anim_sound_tasks.c"
void SoundTask_PlaySeChangingVolume(u8 taskId)
{
    gTasks[taskId].data[0] = gBattleAnimArgs[0];
    gTasks[taskId].data[1] = BattleAnimAdjustPanning(gBattleAnimArgs[1]);
    gTasks[taskId].data[2] = gTasks[taskId].data[7] = gBattleAnimArgs[2];
    gTasks[taskId].data[3] = gBattleAnimArgs[3];
    gTasks[taskId].data[4] = gBattleAnimArgs[4];
    gTasks[taskId].data[5] = gBattleAnimArgs[5];

    PlaySE1WithPanning(gTasks[taskId].data[0], gTasks[taskId].data[1]);
    m4aMPlayVolumeControl(&gMPlayInfo_SE1, 0xFFFF, gTasks[taskId].data[2]);
...
static void SoundTask_SeVolumeChange(u8 taskId)
{
    ...
    if (gTasks[taskId].data[6]++ >= gTasks[taskId].data[5])
    {
        ...
        if (gTasks[taskId].data[2] > 256)
            gTasks[taskId].data[2] = 256;

        if (gTasks[taskId].data[7] < gTasks[taskId].data[4])
        {
            if (gTasks[taskId].data[2] >= gTasks[taskId].data[4])
                destroyTask = 1;
        }
        else
        {
