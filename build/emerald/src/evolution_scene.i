# 0 "src/evolution_scene.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/evolution_scene.c"
...
# 4 "src/evolution_scene.c" 2
# 1 "include/battle_message.h" 1
# 5 "src/evolution_scene.c" 2
# 1 "gflib/bg.h" 1
...
# 14 "src/evolution_scene.c" 2
# 1 "include/main.h" 1
# 15 "src/evolution_scene.c" 2
# 1 "include/menu.h" 1
...
# 19 "src/evolution_scene.c" 2
# 1 "include/pokemon.h" 1
# 20 "src/evolution_scene.c" 2
...
# 26 "src/evolution_scene.c" 2
# 1 "include/task.h" 1
# 27 "src/evolution_scene.c" 2
...
# 32 "src/evolution_scene.c" 2
# 1 "include/constants/battle_string_ids.h" 1
# 33 "src/evolution_scene.c" 2
# 1 "include/constants/songs.h" 1
# 34 "src/evolution_scene.c" 2
# 1 "include/constants/rgb.h" 1
# 35 "src/evolution_scene.c" 2
...
static __attribute__((section("ewram_data"))) struct EvoInfo *sEvoStructPtr = ((void *)0);
static __attribute__((section("ewram_data"))) u16 *sEvoMovingBgPtr = ((void *)0);
...
static void Task_BeginEvolutionScene(u8 taskID)
{
    ...
    case 1:
        if (!gPaletteFade.active)
        {
            ...
            mon = &gPlayerParty[gTasks[taskID].data[10]];
            speciesToEvolve = gTasks[taskID].data[2];
            canStopEvo = gTasks[taskID].data[3];
            partyID = gTasks[taskID].data[10];
            formIdToEvolve = gTasks[taskID].data[12];
...
void BeginEvolutionScene(struct Pokemon* mon, u16 speciesToEvolve, bool8 canStopEvo, u8 partyID, u8 formIdToEvolve)
{
    ...
    gTasks[taskID].data[2] = speciesToEvolve;
    gTasks[taskID].data[3] = canStopEvo;
    gTasks[taskID].data[10] = partyID;
    gTasks[taskID].data[12] = formIdToEvolve;
...
void EvolutionScene(struct Pokemon* mon, u16 speciesToEvolve, bool8 canStopEvo, u8 partyID, u8 formIdToEvolve)
{
    ...
    trainerId = GetMonData(mon, 1);
    personality = GetMonData(mon, 0);
    currentFormId = GetMonData(mon, 89);
    ...
    gTasks[ID].data[2] = speciesToEvolve;
    gTasks[ID].data[3] = canStopEvo;
    gTasks[ID].data[4] = 1;
    gTasks[ID].data[9] = 0;
    gTasks[ID].data[10] = partyID;
    gTasks[ID].data[11] = currentFormId;
    gTasks[ID].data[12] = formIdToEvolve;
...
static void CB2_EvolutionSceneLoadGraphics(void)
{
    ...
    postEvoFormId = gTasks[sEvoStructPtr->evoTaskID].data[12];
...
static void CB2_TradeEvolutionSceneLoadGraphics(void)
{
    ...
    u8 postEvoFormId = gTasks[sEvoStructPtr->evoTaskID].data[12];
    ...
    case 4:
        {
            const struct CompressedSpritePalette* pokePal;
            u32 trainerId = GetMonData(Mon, 1);
            u32 personality = GetMonData(Mon, 0);
            DecompressPicFromTable_DontHandleDeoxys(&gMonFrontPicTable[postEvoFormSpecies],
                                     gMonSpritesGfxPtr->sprites[3],
                                     postEvoFormSpecies);
            pokePal = GetMonSpritePalStructFromOtIdPersonality(postEvoFormSpecies, trainerId, personality);
...
static void CreateShedinja(u16 preEvoSpecies, struct Pokemon* mon)
{
    ...
    if (gEvolutionTable[preEvoSpecies][0].method == 13 && gPlayerPartyCount < 6)
    {
        ...
        SetMonData(&gPlayerParty[gPlayerPartyCount], 11, (&gEvolutionTable[preEvoSpecies][1].targetSpecies));
        SetMonData(&gPlayerParty[gPlayerPartyCount], 2, (gSpeciesNames[gEvolutionTable[preEvoSpecies][1].targetSpecies]));
        SetMonData(&gPlayerParty[gPlayerPartyCount], 12, (&data));
        SetMonData(&gPlayerParty[gPlayerPartyCount], 8, (&data));
        SetMonData(&gPlayerParty[gPlayerPartyCount], 10, (&data));
        ...
        SetMonData(&gPlayerParty[gPlayerPartyCount], 55, (&data));
        data = 0xFF;
        SetMonData(&gPlayerParty[gPlayerPartyCount], 64, (&data));
        ...
        if (GetMonData(shedinja, 11) == 292
            && GetMonData(shedinja, 3) == 1
            && GetMonData(mon, 11) == 291)
                SetMonData(shedinja, 2, Text_ShedinjaJapaneseName);
    }
}
...
{
    u32 var;
    struct Pokemon* mon = &gPlayerParty[gTasks[taskID].data[10]];


    if (gMain.heldKeys == 0x0002
        && gTasks[taskID].data[0] == 8
        && gTasks[gBattleCommunication[2]].isActive
        && gTasks[taskID].data[3] & 0x1)
    {
    ...
    case 2:
        ...
        {
            EvoScene_DoMonAnimation(sEvoStructPtr->preEvoSpriteID, GetFormSpeciesId(gTasks[taskID].data[1], gTasks[taskID].data[11]));
            gTasks[taskID].data[0]++;
        }
        break;
    case 3:
    ...
    case 4:
        if (!IsSEPlaying())
        {
            PlayNewMapMusic(348);
            gTasks[taskID].data[0]++;
            BeginNormalPaletteFade(0x1C, 4, 0, 0x10, ((0) | ((0) << 5) | ((0) << 10)));
        }
        break;
    case 5:
        if (!gPaletteFade.active)
        {
            InitMovingBackgroundTask(0);
    ...
    case 7:
        if (!gTasks[gBattleCommunication[2]].isActive)
        {
            gBattleCommunication[2] = sub_817C3A0(sEvoStructPtr->preEvoSpriteID, sEvoStructPtr->postEvoSpriteID);
            gTasks[taskID].data[0]++;
        }
        break;
    case 8:
    ...
    case 10:
        if (!gTasks[gBattleCommunication[2]].isActive)
        {
            gBattleCommunication[2] = LaunchTask_PostEvoSparklesSet2AndFlash(gTasks[taskID].data[2]);
            gTasks[taskID].data[0]++;
        }
        break;
    case 11:
    ...
    case 12:
        if (IsSEPlaying())
        {
            ...
            sub_8140174();
            BeginNormalPaletteFade(0x1C, 0, 0x10, 0, ((0) | ((0) << 5) | ((0) << 10)));
            gTasks[taskID].data[0]++;
        }
        break;
    case 13:
        if (!gPaletteFade.active)
        {
            EvoScene_DoMonAnimation(sEvoStructPtr->postEvoSpriteID, GetFormSpeciesId(gTasks[taskID].data[2],gTasks[taskID].data[12]));
            gTasks[taskID].data[0]++;
        }
        break;
    case 14:
        if (IsCryFinished())
        {
            ...
            PlayBGM(342);
            gTasks[taskID].data[0]++;
            SetMonData(mon, 11, (void*)(&gTasks[taskID].data[2]));
            SetMonData(mon, 89, (void*)(&gTasks[taskID].data[12]));
            CalculateMonStats(mon);
            EvolutionRenameMon(mon, gTasks[taskID].data[1], gTasks[taskID].data[2]);
            GetSetPokedexFlag(SpeciesToNationalPokedexNum(gTasks[taskID].data[2]), FLAG_SET_SEEN);
            GetSetPokedexFlag(SpeciesToNationalPokedexNum(gTasks[taskID].data[2]), FLAG_SET_CAUGHT);
   ...
   var = MonTryLearningNewMoveEvolution(mon, gTasks[taskID].data[4]);
            if (var != 0 && !gTasks[taskID].data[9])
            {
                u8 text[20];

                if (!(gTasks[taskID].data[3] & 0x80))
                {
                    StopMapMusic();
                    Overworld_PlaySpecialMapMusic();
                }

                gTasks[taskID].data[3] |= 0x80;
    ...
    case 16:
        if (!gPaletteFade.active)
        {
            ...
            if (!gTasks[taskID].data[9])
                CreateShedinja(gTasks[taskID].data[1], mon);

            DestroyTask(taskID);
            FreeMonSpritesGfx();
            Free(sEvoStructPtr);
            sEvoStructPtr = ((void *)0);
    ...
    case 17:
        ...
        {
            m4aMPlayAllStop();
            BeginNormalPaletteFade(0x6001C, 0, 0x10, 0, ((31) | ((31) << 5) | ((31) << 10)));
            gTasks[taskID].data[0]++;
        }
        break;
    case 18:
    ...
    case 20:
        ...
        break;
    case 21:
        if (!IsTextPrinterActive(0) && !IsSEPlaying() && --gTasks[taskID].data[4] == 0)
            gTasks[taskID].data[0] = 15;
        break;
    case 22:
        ...
        case 5:
            ...
            {
                FreeAllWindowBuffers();
                ShowSelectMovePokemonSummaryScreen(gPlayerParty, gTasks[taskID].data[10],
            ...
            if (!gPaletteFade.active && gMain.callback2 == CB2_EvolutionSceneUpdate)
            {
                ...
                else
                {
                    ...
                    if (IsHMMove2(move))
                    {
                        ...
                        BattlePutTextOnWindow(gDisplayedStringBattle, 0);
                        gTasks[taskID].data[6] = 12;
                    }
                    else
                    {
                        { gBattleTextBuff2[0] = 0xFD; gBattleTextBuff2[1] = 2; gBattleTextBuff2[2] = (move & 0xFF); gBattleTextBuff2[3] = (move & 0xFF00) >> 8; gBattleTextBuff2[4] = 0xFF; }
...
static void Task_TradeEvolutionScene(u8 taskID)
{
    ...
    case 1:
        if (!IsTextPrinterActive(0))
        {
            PlayCry1(GetFormSpeciesId(gTasks[taskID].data[1], gTasks[taskID].data[11]), 0);
            gTasks[taskID].data[0]++;
        }
        break;
    case 2:
    ...
    case 4:
        if (!gPaletteFade.active)
        {
            InitMovingBackgroundTask(1);
            var = gSprites[sEvoStructPtr->preEvoSpriteID].oam.paletteNum + 16;
            gBattleCommunication[2] = LaunchTask_PreEvoSparklesSet1(var);
            gTasks[taskID].data[0]++;
            SetGpuReg(0xe, 0x603);
        }
        break;
    ...
    case 9:
        if (!gTasks[gBattleCommunication[2]].isActive)
        {
            gBattleCommunication[2] = LaunchTask_PostEvoSparklesSet2AndFlash_Trade(gTasks[taskID].data[2]);
            gTasks[taskID].data[0]++;
        }
        break;
    case 10:
    ...
    case 12:
        if (IsCryFinished())
        {
            ...
            PlayFanfare(342);
            gTasks[taskID].data[0]++;
            SetMonData(mon, 11, (&gTasks[taskID].data[2]));
            CalculateMonStats(mon);
            EvolutionRenameMon(mon, gTasks[taskID].data[1], gTasks[taskID].data[2]);
            GetSetPokedexFlag(SpeciesToNationalPokedexNum(gTasks[taskID].data[2]), FLAG_SET_SEEN);
            GetSetPokedexFlag(SpeciesToNationalPokedexNum(gTasks[taskID].data[2]), FLAG_SET_CAUGHT);
        ...
        break;
    case 13:
        if (!IsTextPrinterActive(0) && IsFanfareTaskInactive() == 1)
        {
   ...
   var = MonTryLearningNewMoveEvolution(mon, gTasks[taskID].data[4]);
    ...
    case 15:
        ...
        {
            m4aMPlayAllStop();
            BeginNormalPaletteFade((1 << (gSprites[sEvoStructPtr->preEvoSpriteID].oam.paletteNum + 16)) | (0x4001C), 0, 0x10, 0, ((31) | ((31) << 5) | ((31) << 10)));
            gTasks[taskID].data[0]++;
        }
        break;
    case 16:
    ...
    case 18:
        ...
        break;
    case 19:
        if (!IsTextPrinterActive(0) && IsFanfareTaskInactive() == 1 && --gTasks[taskID].data[4] == 0)
            gTasks[taskID].data[0] = 13;
        break;
    case 20:
        switch (gTasks[taskID].data[6])
...
static void InitMovingBackgroundTask(bool8 isLink)
{
    ...
    DecompressAndLoadBgGfxUsingHeap(1, sUnknown_085B4134, 0, 0, 0);
    ...
    if (!isLink)
    {
        SetGpuReg(0x50, (1 << 1) | (1 << 6) | (1 << 10));
        SetGpuReg(0x52, (((8) << 8) | (8)));
        SetGpuReg(0x0, 0x1000 | 0x0400 | 0x0200 | 0x0100 | 0x0040);
    ...
    {
        SetGpuReg(0x50, (1 << 1) | (1 << 6) | (1 << 11));
        SetGpuReg(0x52, (((8) << 8) | (8)));
        SetGpuReg(0x0, 0x1000 | 0x0800 | 0x0200 | 0x0100 | 0x0040);
    }
...
static void sub_8140174(void)
{
    ...
    SetGpuReg(0x0, 0x1000 | 0x0800 | 0x0100 | 0x0040);
    Free(sEvoMovingBgPtr);
}

static void EvoScene_DoMonAnimation(u8 monSpriteId, u16 speciesId)
{
    DoMonFrontSpriteAnimation(&gSprites[monSpriteId], speciesId, 0, 0);
}
