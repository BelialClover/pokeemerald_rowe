# 0 "src/lottery_corner.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/lottery_corner.c"
...
# 4 "src/lottery_corner.c" 2
# 1 "include/pokemon.h" 1
# 5 "src/lottery_corner.c" 2
# 1 "include/constants/items.h" 1
# 6 "src/lottery_corner.c" 2
...
# 10 "src/lottery_corner.c" 2

static __attribute__((section("ewram_data"))) u16 sWinNumberDigit = 0;
static __attribute__((section("ewram_data"))) u16 sOtIdDigit = 0;
...
void PickLotteryCornerTicket(void)
{
    ...
    for (i = 0; i < 14; i++)
    {
        for (j = 0; j < (6 * 5); j++)
        {
            if (GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], 11) != 0 &&
            !GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], 45))
            {
                u32 otId = GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], 1);
                u8 numMatchingDigits = GetMatchingDigits(gSpecialVar_Result, otId);
    ...
    if (gSpecialVar_0x8004 != 0)
    {
        ...
        if (box == 14)
        {
            ...
            GetMonData(&gPlayerParty[slot], 2, gStringVar1);
        }
        else
        {
            gSpecialVar_0x8006 = 1;
            GetBoxMonData(&gPokemonStoragePtr->boxes[box][slot], 2, gStringVar1);
        }
